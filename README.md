...
* **원본 대조 재료의 기계 확정**: 원본 DDL에서 AST가 확정할 수 있는 사실 — DML 문장별 적용 범위(독립 SELECT·`IF` 술어까지), 대상 행을 가르는 리터럴 집합, 파생 테이블의 컬럼 산식, 잠금 힌트, 함수 선언 옵션, `CASE` 분기의 조건·결과, 원본 주석, 세션 옵션, 식별자 표기 — 을 프롬프트에 **표로** 실어 주고, 명세서가 그 표를 **행 단위로, 문장 번호까지** 그대로 옮겼는지 기계가 대조합니다. 표가 담지 못하는 문장(지금은 `MERGE`)은 거짓 행 대신 기계 공지로 알립니다. 산문으로 요구하면 요약되거나 그럴듯한 대체물이 채워지므로, 옮겨 적어야 할 값 자체를 미리 확정해 넘기는 방식입니다.
