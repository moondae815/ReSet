...
* **원본 대조 재료의 기계 확정**: 원본 DDL에서 AST가 확정할 수 있는 사실 — DML 문장별 적용 범위, 대상 행을 가르는 리터럴 집합, 파생 테이블의 컬럼 산식, 잠금 힌트, 함수 선언 옵션, `CASE` 분기의 조건·결과, 원본 주석, 세션 옵션, 식별자 표기 — 을 프롬프트에 **표로** 실어 주고, 명세서가 그 표를 그대로 옮겼는지 기계가 대조합니다. 산문으로 요구하면 요약되거나 그럴듯한 대체물이 채워지므로, 옮겨 적어야 할 값 자체를 미리 확정해 넘기는 방식입니다.
* **실행 의미의 기계 확정**: DDL 원문이 말하지 않는 실행 시점의 사실 — 참조 객체의 DB 소속, 결과가 없을 때 집계 대입이 남기는 값, `IF` 사이에서 `@@ROWCOUNT`가 리셋되는 경계, 커서 수명, `CAST(... AS INT)`가 반올림인지 절사인지 — 도 같은 방식으로 표에 실어 대조합니다. 잎 타입을 하나라도 모르면 그 행은 아예 싣지 않습니다. 추측이 한 줄 섞이면 표 전체의 신뢰가 무너지기 때문입니다.
