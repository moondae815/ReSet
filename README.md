...
* **원본 대조 재료의 기계 확정**: 원본 DDL에서 AST가 확정할 수 있는 사실 — DML 문장별 적용 범위(독립 SELECT·`IF` 술어까지), 대상 행을 가르는 리터럴 집합, 파생 테이블의 컬럼 산식, 잠금 힌트, 함수 선언 옵션, `CASE` 분기의 조건·결과, 트랜잭션 경계, 변수 대입, 지역 변수 선언, 원본 주석, 세션 옵션, 식별자 표기 — 을 프롬프트에 **표로** 실어 주고, 명세서가 그 표를 **행 단위로, 문장 번호까지** 그대로 옮겼는지 기계가 대조합니다. 표가 담지 못하는 문장(지금은 `MERGE`)은 거짓 행 대신 기계 공지로 알립니다. 산문으로 요구하면 요약되거나 그럴듯한 대체물이 채워지므로, 옮겨 적어야 할 값 자체를 미리 확정해 넘기는 방식입니다.
...
* **순차적 배치 현대화 계획 수립**: 분석된 여러 SP들의 명세서를 사용자가 선택한 순서대로 조합하여, 워크플로우 제어, 대용량 페이징, 오류 처리 정책이 설계된 통합 배치 계획서(`BatchMigrationPlan.md`)를 작성합니다. 사용자가 고르는 것은 **진입점**이며, 그 SP들이 호출하는 **프로시저**의 명세는 도구가 의존 관계를 따라 재료에 자동으로 더합니다(호출한 자리 바로 뒤에 놓이며, 추가된 것은 화면과 로그에 알립니다). 함수는 더하지 않습니다 — 부모 명세가 이미 호출식 전문을 담고 있어 계획서가 함수를 재구현할 일이 없기 때문입니다. (배치 작업 단위는 `--job-name` 옵션을 통해 식별 관리됩니다.)
...
### 6. 명세서 기반 요구사항 도출 (PRD Derivation)
* **`Spec.md` 단일 근거의 요구사항 문서화**: 이미 완성된 명세서(`Spec.md`) 하나만 읽어 업무 요구사항 문서(`Prd.md`)를 도출합니다. DB에 다시 접속하지도, 재분석을 수행하지도 않으므로 이미 쌓인 산출물에 즉시 소급 적용할 수 있고, 그래서 1번 분석 실행에 이어 붙이지 않고 별도 메뉴로 기동합니다. 각 요구 항목은 `<Spec 헤딩> > "<원문 구절>"` 형태의 근거를 반드시 달고 `도출`(직접 지지)·`추정`(재구성) 확신도를 표기해, 코드에는 없는 「원래 업무 요구」를 문서가 스스로 지어낸 것처럼 오독되지 않게 합니다.
* **귀속 검사와 정직한 한계 표기**: L1/L2/L3 검증 파이프라인에는 들어가지 않고, 근거 인용이 원본 명세서에 **실재하는지**만 기계로 대조합니다(불일치 시 1회 재시도 후에도 남은 결함은 문서 상단 배너에 기록하고 그대로 저장). 이 검사가 증명하는 것은 인용의 실재뿐이며 요구와 근거의 대응까지 보장하지는 않으므로, 문서 상단 배너가 그 한계를 항상 함께 적어 독자가 문서를 실제보다 강하게 믿지 않게 합니다.

### 7. 통합 정산 정책 문서 도출 (Policy Extraction)
* **명세서 근거 + 사람이 짠 업무 순서**: DB를 다시 긁지 않고 이미 완성된 `Spec.md` 코퍼스와 사람이 소유하는 정산 프로세스 명부(`settlement-process.md`)를 근거로 정책서를 도출합니다. 업무 순서는 코드 호출 그래프로 복원되지 않으므로(EXEC 간선이 극소수) 명부가 단계 제목과 소속 SP를 정하고, 명부가 없으면 초안을 자동 생성한 뒤 사람이 채우기를 기다립니다.
* **코드값 사전(선택적 마스터 데이터 결합)**: DDL의 비교식 상수(예: `WHERE Status = 'S02'`)는 명세서에 등장하는 것만 채택해 코드값 사전 좌변을 만들고, DB 연결이 있으면 그 우변(업무 의미, 예: `S02 = 정산보류`)을 작은 마스터 테이블에서 채웁니다. DB가 없거나 매칭이 안 되면 「의미 미상」으로 표기된 채 문서가 완주합니다.

### 8. 실시간 병렬 태스크 진행률 시각화 (CLI Progress)
...
### 9. 영속적인 실행 로깅 시스템 (Serilog File Sink & Clean Logging)
...
└── output/                          # [산출물 폴더] 역공학 명세서 및 마이그레이션 생성물 보관
    ...
    │       ├── docs/
    │       │   ├── Spec.md                 # 최종 비즈니스 명세서
    │       │   ├── Prd.md                  # 명세서 기반 요구사항 문서 (메뉴 5, DB 연결 없이 사후 도출)
    ...
    ├── settlement-process.md        # 사람이 소유·편집하는 정산 프로세스 명부 (단계·순서 지정, 없으면 초안 자동 생성)
    ├── Policy/
    │   ├── SettlementPolicy.md      # 정산 정책 문서 (L1/L2를 거치지 않는 미검증 산출물)
    │   ├── settlement-codebook.json # 코드값 사전 (좌변: DDL 상수, 우변: 마스터 데이터 매칭 결과)
    │   └── steps/                   # 단계별 정책 본문 원본 (NN-슬러그.md)
...
```json
{
  ...
  "AiSettings": {
    ...
    // 있어, 위반이 특정 자리로 귀속되면 그 자리만 다시 만들고 채점 회차를 소모하지 않습니다. 자리는
    // 단계 섹션과 골격 둘 다입니다 — 단계 섹션 밖(개요·흐름도·공통 규약 절·검증 SQL 세트)의 위반은
    // 골격만 직전 골격 위의 패치로 다시 만들고 단계 본문은 동결합니다. 귀속에 실패한 위반은 종전대로
    // 전량 재생성이며 채점 예산을 씁니다 (1 이상의 정수, 기본값: 2)
...
2. 로그인 성공 시 아래 **메인 메뉴**가 화면에 표시됩니다:
   ...
   * **`4. 통합 정산 정책 문서 도출 (Policy Extraction)`**:
     이미 쌓인 명세서(`Spec.md`)와 정산 프로세스 명부(`settlement-process.md`)를 근거로 비즈니스 관점의 통합 정책 정의서(`Policy/SettlementPolicy.md`)를 도출합니다. SP를 선택하거나 Job 이름을 입력하지 않으며, 명부 파일이 없으면 초안을 만들고 안내한 뒤 중단합니다.
   * **`5. 명세서 기반 요구사항 도출 (PRD Derivation)`**:
     `output/Procedures/` 아래 이미 `Spec.md`가 생성된 대상만 목록으로 보여주고, 다중 선택한 대상마다 그 `Spec.md`를 근거로 요구사항 문서(`Prd.md`)를 같은 폴더에 저장합니다. DB 연결이 필요 없으며, 기존 `Prd.md`가 있으면 덮어쓰기를 먼저 확인받습니다.
...
> [!NOTE]
> `AiSettings:Provider`(또는 `Critic`/`Consolidator`)에 `claude-cli`나 `codex-cli`가 지정된 배치는 기본적으로 DB 연결 전에 중단됩니다. 두 CLI가 헤드리스 호출을 지원해도 권한 프롬프트나 구독 쿼터 소진으로 장시간 실행이 중단될 수 있기 때문입니다(이미 만든 단계는 저장되어 같은 Job 이름으로 다시 실행하면 이어지도록 설계돼 있습니다 — 실물 확인은 진행 중입니다). `agy-cli`는 도구 권한을 끌 수 없어 배치 옵트인 여부와 무관하게 차단됩니다.
>
