...
> [!TIP]
> **💡 재귀 분석 산출물 모드**
> * `AnalyzeReferencedCodeObjects`는 기본적으로 `false`입니다. 활성화하면 하위 SP/UDF도 각각 검증 파이프라인을 거쳐 객체별 `Spec.md`를 생성합니다. 끄든 켜든 분석과 저장은 같은 경로를 타므로 산출물의 자리와 구성은 동일하며, 끄면 의존성 매니페스트가 루트 한 노드짜리로 나옵니다(루트 SP만 분석하므로).
