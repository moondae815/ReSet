...
> [!NOTE]
> `AiSettings:Provider`(또는 `Critic`/`Consolidator`)에 `claude-cli`나 `codex-cli`가 지정된 배치는 기본적으로 DB 연결 전에 중단됩니다. 두 CLI가 헤드리스 호출을 지원해도 권한 프롬프트나 구독 쿼터 소진으로 장시간 실행이 중단될 수 있기 때문입니다(이미 만든 단계는 저장되어 같은 Job 이름으로 다시 실행하면 이어집니다). `agy-cli`는 도구 권한을 끌 수 없어 배치 옵트인 여부와 무관하게 차단됩니다.
>
