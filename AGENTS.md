...
2.  **전방위적 소프트 페일(Soft Fail) 및 예외 격리 정책을 준수하십시오.**
    ...
    *   **정합성 검증 DB 실행**: [SpExecutionService.cs](./src/ReSet.Validator.Core/Services/SpExecutionService.cs)의 Legacy SQL 실행 수집 시 연결 실패나 쿼리 수행 오류가 나면 크래시하지 말고, 결과 DTO의 테스트 케이스를 `FAIL`로 처리하고 예외 메시지를 `ErrorCode` 필드에 기재하여 직렬화 내보내야 합니다. (`SpExecutionService_ShouldSoftFail_OnInvalidConnectionString`가 결과에 `FAIL`과 프로시저명이 포함되는지 검사 — `ErrorCode` 필드 자체는 직접 단언하지 않음)
    ...
    *   **재귀 코드 객체 분석 — 상태 표기**: [DependencyAnalysisOrchestrator.cs](./src/ReSet.Core/Services/DependencyAnalysisOrchestrator.cs)에서 하위 SP/UDF 실패는 해당 노드만 `Failed`로 격리하고 계속 진행하십시오. 깊이 제한은 `SkippedDepth`, 크로스 DB 미진입은 `SkippedExternal`(단 활성 중 접근 실패는 `Failed`)로 표기하고, 최소 깊이 경로를 우선하십시오. (`AnalyzeAsync_ChildFailureDoesNotFailRoot`/`..._ExternalMetadataFailureIsSurfacedAsFailedNode`/`..._ShallowDiscoveryWinsOverLaterDepthExceededPath`가 검사)
    *   **재귀 코드 객체 분석 — 성공 산출물**: 성공하지 않은 객체에는 명세서 링크를 만들지 말고, 성공한 하위 객체의 Critic 점수와 `Thinking.md`는 보존하며, 객체명은 호출부가 아니라 카탈로그 표기를 따르십시오. (`AnalyzeAsync_SpecWriteFailureMarksChildFailedAndParentDoesNotLinkIt`/`..._PersistsChildReviewScoreAndThinkingArtifacts`/`..._NormalizesGraphKeysToCatalogObjectNameCasing`가 검사)
    *   **재귀 코드 객체 분석 — 경로 인코딩**: 객체 키와 출력 경로는 구분자·파일명 문자를 충돌 없이 인코딩해야 합니다.
    ...
    *   **취소는 소프트 페일 대상이 아님**: `OperationCanceledException`은 실패가 아니라 사용자의 지시입니다. 취소 토큰을 넘기는 `await`를 감싸는 광범위 `catch`에는 반드시 `when (ex is not OperationCanceledException)` 필터를 달아 취소가 경고 목록으로 흡수되거나 다른 예외로 세탁되지 않게 하십시오. (`CancellationPolicyTests`가 Roslyn 구문 트리로 자동 검사)
    *   **취소 이후의 부분 완료 보존**: 취소 이후에는 순회나 후속 생성을 계속하지 말고 즉시 되돌아가되, 이미 완료된 객체의 산출물은 보존하고 미분석 참조는 문서에 표기하십시오. (`DependencyAnalysisOrchestratorTests`의 `AnalyzeAsync_CancelledMidGraph_PersistsCompletedObjectsAndReportsPartialCompletion`과 `..._CyclicGraphCancelled_AddsTheUnresolvedReferenceBannerToTheSurvivingDocument`가 검사)
...
3.  **AI API 응답 널 가드(TryGetProperty) 및 모델 파라미터 매핑을 준수하십시오.**
    ...
    *   반드시 `TryGetProperty`를 활용해 JSON 필드 유무를 안전하게 확인하고, 비정상 수신 시 `InvalidOperationException`을 던져 투명하게 거절 사유를 노출하십시오. (Claude/OpenAI/Zai는 `ChatAsync_WithErrorResponse_ShouldThrowInvalidOperationException`, Google은 이름은 다르지만 같은 취지의 `ChatAsync_WithMissingCandidates_ShouldThrowInvalidOperationException`이 검사; Ollama는 같은 가드가 구현돼 있으나 대응 테스트가 없습니다)
    *   **모델별 전송 규격은 기존 클라이언트 배선을 따를 것**: OpenAI 추론 모델(o1/o3)은 `temperature` 대신 `reasoning_effort`를, gpt-5 Responses API는 explicit `prompt_cache_breakpoint`를, Claude는 `system` 블록 `cache_control`과 재생성 회차의 user 블록 두 번째 중단점을 각각 씁니다. 새 포맷을 임의로 만들거나 표현만 바꾸지 마십시오 — 접미사가 빈 호출에서 캐시 접두사가 깨지거나 400 위험만 늘어납니다. (근거: `architecture.md §4.5`, `§4.13`)
    *   **OpenAI Responses 추론 보존**: gpt-5 Responses API의 `output`에는 여러 `reasoning` 항목과 빈 `summary`가 섞여 올 수 있습니다. 모든 비어 있지 않은 `summary_text`를 누적하고, 뒤따르는 빈 summary가 앞서 수집한 추론을 지우지 않도록 하여 `Thinking.md`에 보존하십시오. (`ChatAsync_WithGpt5MixedReasoningSummaries_ShouldPreserveNonEmptyReasoningText`가 검사)
...
6.  **3단계 검증 파이프라인의 역할 분리 및 L2 Actor-Critic을 운용하십시오.**
    ...
    *   **검증 종료 상태 정직성**: 파이프라인이 어디서 끝났는지는 [VerificationOutcome.cs](./src/ReSet.Core/Models/VerificationOutcome.cs)의 네 값으로만 표현하고, `bool` 플래그나 `ReviewResult`의 널 여부로 대체 판정하지 마십시오. 표기 단일화·점수 노출 게이팅·캐시 경계 규칙은 `architecture.md §4.4.4` 참고.
...
8.  **지시서 번들 생성 및 코딩 에이전트 CLI 프로세스 제어를 적용하십시오.**
    *   **번들 분할 제공**: 지시서를 마크다운 하나로 묶지 마십시오 — 진입점·공통 문서·단계 본문·회차별 지시서로 나누고, 한 회차의 지시서는 그 회차가 읽을 것만 가리키게 하십시오(`architecture.md §4.11`, `InstructionBundleWriterTests`). 폴더가 없으면 선행 생성하고, 개별 SP 분석 시에는 번들을 만들지 않으며 통합 배치 시에만 문서(`docs/`)·소스코드(`src/`)를 `output/Jobs/{JobName}/`에 엄격히 격리하십시오.
    *   **분할 실패 시에도 지침은 앞으로**: 단일 파일 폴백에도 지침을 문서 앞으로 옮기는 순서 교정은 반드시 적용하십시오. 부분 분할은 만들지 마십시오 — 단계 하나라도 경계를 못 찾으면 전체를 단일 파일로 되돌리십시오(`architecture.md §4.11`, `InstructionEntryPointComposerTests.Compose_ShouldPlaceGuidelinesBeforePlanLink_EvenInFallback`, `PlanBoundaryResolverTests.ResolveSteps_ShouldFailWholly_WhenOneStepCannotBeLocated`).
    ...
    *   **동적 코드 생성 시점 제약**: 개별 SP 분석 완료 직후에는 에이전트 자동 기동을 금지하며, 가급적 복수 SP가 엮인 통합 배치 전환 계획서 수립 완료 시점에만 외부 에이전트를 기동하십시오. 단, 사용자가 메인 메뉴에서 스탠드얼론 메뉴(기작성된 지시서 기반 구동)를 선택한 경우에는 기존 출력 디렉터리의 `agent/MigrationInstructions.md`를 스캔하여 에이전트를 독립적으로 재기동(Resume)할 수 있도록 허용합니다. 이때 고른 지시서가
        레거시 단일 문서인지 회차 번들인지 **먼저 판정**하십시오. 번들이면 디스크에서 회차 목록을 복원해 회차 경로로 보내고, 복원이 성립하지 않으면 전체 Job 경로로 떨어뜨리지 말고 사유를 설명하며 거부하십시오 — 회차용 문서를 전체 Job 경로에 먹이는 조합만은 만들지 마십시오.
    ...
    *   **대화형/배치 인자 분리**: 엔진 인자는 `Arguments`(대화형)/`BatchArguments`(무인)로 나뉩니다. 대화형 TUI는 무인 실행에서 TTY를 못 열어 종료 코드 0으로 조용히 실패하므로 폴백하지 마십시오. `BatchArguments`가 비면 `CodingEngineFactory`가 명시 거부하고, 지시서는 `{jobDir}`·원본 명세서(Job 형제 경로)는 `{specRoot}`로 엽니다 — 출력 루트 전체는 열지 마십시오(`CodingEngineTests`, `ArgumentTemplateResolverTests.ResolveSpecRoot_ShouldNotGrantTheWholeOutputRoot`).
...
9.  **메타데이터 정화 및 정책 문서 수립 가이드를 준수하십시오.**
    *   **클렌징 스크립트 및 동기화**: AI 분석 완료 시 보완 스크립트 파일(`*_MetadataCleansing.sql`) 생성 기능은 현재 로컬 LLM의 스키마 환각(Hallucination) 방지를 위해 기본 제거되어 있습니다. 향후 수동 태그 삽입 등으로 해당 쿼리 파일이 물리적으로 존재할 때만 조건부로 TUI 최종 승인 및 동의를 묻고 DB 동기화를 실행하십시오. 크로스 DB 분석으로 수집된 다른 DB 소유 객체의 정화 스크립트는 파일명에 DB를 접두해 구분하고, 연결된 DB가 아닌
        대상에는 절대 실행하지 마십시오.
    ...
    *   **UPDATE 매핑표는 정적 파서가 확정합니다**: SET 절 타겟 컬럼·원천 표현식은 `SqlStaticParser`가 `AstUpdateMappings`로 추출해 프롬프트 표를 채웁니다(`architecture.md §4.3`). `AiService`의 fill-in-the-blank 표를 AI가 채우도록 되돌리지 마십시오 — 되돌리면 명세서가 산문으로 뭉개지는데 코드만 봐서는 알 수 없습니다. `MechanicalValidator`가 대조하므로(`MechanicalValidatorTests.Validate_WhenAnExpectedUpdateColumnIsMissing_ShouldReportIt`), 프롬프트만 지우면 L1이 영원히 실패합니다.
    ...
    *   **의존 스키마 덤프 필터링**: 상세 스키마 덤프는 AST가 감지한 실제 참조 컬럼·PK/FK·인덱스 구성 컬럼만 선별 출력(KeepCols 필터링)해 토큰을 절약합니다. 판정 근거(3-part 정확 일치, DB 컨텍스트 없을 때 베이스 이름 폴백, 과다 포함이 과소 포함보다 나은 이유, 14개 명세서를 망가뜨린 실측 결함)는 `SchemaPromptColumnSelector`/`KeyMatchesDependency`의 `<summary>`에 있고, `SchemaPromptColumnSelectorTests`가 정합성을 검사합니다.
    *   **통합 배치 도메인 5대 핵심 제약 (NOLOCK / INSERT-only / Chunk Key / 멱등성 / 예외 처리)**: 통합 배치 전환 계획 수립 시 다음 사항을 엄격히 강제하십시오. 1) **NOLOCK 힌트 전면 금지**: 타겟 프레임워크가 Session-level SNAPSHOT 격리를 사용할 경우 `WITH (NOLOCK)` 힌트는 READ UNCOMMITTED를 유발하여 정책을 위반하므로 마이그레이션 의사코드에서 즉시 전면 제거해야 합니다. 2) **INSERT-only 롤백**: INSERT-only 단계는 섀도우(Shadow) 백업 테이블을 생성해
        롤백하지 말고, 단순 `ROLLBACK TRAN`이나 `DELETE WHERE [ChunkKey]`를 통한 보상 트랜잭션으로 설계하십시오. 3) **청킹 키 검증, 원본 필터 및 에러 트래킹**: 청킹용 임시 분기 변수(예: `CLIENTID`)를 지정할 때는 반드시 타겟 테이블 내에 존재하는 실제 컬럼(또는 PK 기반 해시)만을 사용하고, 청킹 필터 적용 시에도 원본 비즈니스 필터(예: `WHERE Status = 'P'`)를 절대 누락 없이 보존하십시오. 에러 코드는 임의로 변조(Remapping)하지 말고 원본 에러
        코드를 정확히 트래킹하도록 제한하십시오. 추가로 Chunking `WHILE` 루프 내부에는 부분 커밋 및 격리를 위해 `BEGIN TRAN/COMMIT TRAN` 경계를 명시해야 합니다. 4) **멱등성 보장 및 섀도우 복원**: 체크포인트 기반 스킵 로직을 추가하여 이미 성공한 단계가 재시작 시 실패 처리되지 않도록 설계하고, Shadow 테이블을 활용해 롤백(Restore)할 경우 데이터 중복을 방지하기 위해 반드시 선행 `DELETE` 구문을 실행한 뒤 데이터를 복원하도록
        강제하십시오. 5) **XACT_ABORT 및 TRY...CATCH 결합**: `XACT_ABORT ON`을 선언할 경우 반드시 `BEGIN TRY...CATCH` 블록과 결합해야 하며, 구형 `GOTO` 예외 처리 패턴 사용을 원천 차단하십시오.
    ...
    *   **Mermaid flowchart 생성 규칙화**: 화살표/연결선 조건 라벨에는 절대 큰따옴표를 쓰지 말고(예: `N1 -->|예| N2`, `N1 -- 에러 --> N2`), 노드 내부 텍스트에는 SQL 변수 기호(`@`)를 쓰지 말고 자연어로 나타내십시오(단, `@@ERROR`는 예외 허용 및 전체 이중 따옴표 래핑). 노드 ID 공백·언더스코어 제거 등 정화기 동작은 `MechanicalValidator.CleanseMermaidCode`가 담당합니다(`MechanicalValidatorTests.PostProcessMarkdown_ShouldCleanseMermaidCode`).
