...
*   **비즈니스 서비스 ([Services](./src/ReSet.Core/Services))**
    ...
    *   [SqlObjectTypeClassifier.cs](./src/ReSet.Core/Services/SqlObjectTypeClassifier.cs): `sys` 카탈로그의 타입 문자열이 테이블/뷰인지 코드 객체인지 판정하는 단일 판정기. 부분 문자열 판정을 여기 모으십시오 — `SQL_TABLE_VALUED_FUNCTION`이 `TABLE`을 포함하므로, 호출부마다 따로 판정하면 한쪽만 가드를 갖게 되고 실제로 그렇게 되어 TVF의 DDL이 수집되지 않았습니다.
