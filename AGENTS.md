...
10. **모든 코드 변경 작업 시 `git worktree`를 적극 활용하십시오.**
    ...
    *   ⚠️ **`output.bak-2026-08-22/`은 스냅샷이 아니라 테스트 재료입니다.** `CorpusPaths.PriorEdition`이 그 이름을 상수로 들고 있고 `CoverageMapGoldenTests`가 그 존재를 요구합니다 — **과거 판 대조의 기준 세대**입니다. 재생성 대상으로 삼거나 덮어쓰면 코퍼스 테스트가 **다른 세대를 대조하게 됩니다.** 스냅샷을 새로 뜬다면 `output.bak-<작업이름>-<날짜>` 같은 **새 이름**을 쓰십시오(`.git/info/exclude`의 `output.bak-*`가 함께 덮습니다). 이 사실이 문서에 없어서, 한 세션이 이 링크를 걸고 하루 종일 작업하면서도 그것이 재료인 줄 몰랐습니다.

    *   ⚠️ **심링크를 건 워크트리에서 CLI 재생성을 돌리면 공용 코퍼스에 직접 씁니다.** `src/ReSet.Cli/appsettings.json`의 `OutputSettings.Directory`가 `./output`, `LogDirectory`가 `./output/logs`로 **cwd 상대**입니다. 링크 전에는 워크트리 안에 자기 `output/`이 생기지만, 링크 후에는 같은 명령이 **메인의 941M 공용 코퍼스를 고칩니다.** 테스트는 읽기만 하므로 안전하고, 문제는 **재생성**입니다.

        재생성 전에는 반드시 이렇게 하십시오.

        1.  **스냅샷을 먼저 뜹니다** — `output.bak-<작업이름>-<날짜>`. 관례는 실물로 존재하나 지금까지 절차가 문서에 없었습니다.
        2.  **한 Job으로 먼저 돌려 비용과 동작을 잽니다.**
        3.  **다른 세션에 알립니다.** 이 저장소의 여러 축이 **코퍼스 스윕 전후 대조**를 증거로 씁니다 — 재생성이 분모를 바꾸면 그 대조가 무의미해집니다. 전례: 다른 세션이 버전 11로 재생성하는 중에 버전 10 코드로 스윕해 `SetPredicateMismatch` 105건이 났고 전부 세대 차이였습니다.
        4.  **워크트리가 아니라 메인 저장소에서 돌리거나, 링크를 먼저 뗍니다.**

        되돌릴 수 없다는 점이 특히 중요합니다. `InstructionBundleWriter`가 **이번 회차가 덜 만들었을 때 하위 트리를 통째로 지웁니다**(`verification/`·`steps/`). 의도된 동작이지만("낡은 파일만 보고 작업을 시작하는 에이전트가 더 나쁘다"), **폴백 한 번에 이전 회차의 멀쩡한 산출물이 사라집니다.** 스냅샷 없이 돌리지 마십시오.

        (쓰기 경로 분리는 미결 항목입니다 — 분리가 없어서 재생성과 스윕이 직렬화됩니다.)
