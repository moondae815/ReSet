...
3.  **AI API 응답 널 가드(TryGetProperty) 및 모델 파라미터 매핑을 준수하십시오.**
    ...
    *   **모델별 전송 규격 매핑**: OpenAI 추론 모델(o1/o3) 호출 시 `temperature`를 제외하고 `reasoning_effort`를 표준 매핑하십시오. 또한 gpt-5 Responses API 사용 시 명시적인 `prompt_cache_key`를 주입하여 프롬프트 캐시를 활성화합니다. gpt-5.6 이후 모델은 캐시 접두사를 **cache breakpoint** 단위로 비교하며, breakpoint는 기본값(implicit)이 **마지막 메시지**에 놓입니다 — 그러므로 요청마다 달라지는 지시는 공통 메시지에 이어 붙이지 말고 별개 메시지로 분리해야 하고, 접미사가 비면 메시지를 만들지 마십시오(빈 메시지 하나가 느는 것 자체가 접두사를 바꿉니다). Claude 통신 시 4/5세대 모델의 빈 생각 블록 누수를 방지하기 위해 옵션을 조율하고, `system` 프롬프트 블록 내에 `cache_control: { type: "ephemeral" }`을 부여하여 캐싱을 활성화합니다.
...
- [ ] `dotnet test` 명령어를 실행하여 843개의 단위 테스트가 모두 예외 없이 100% 통과(Passed)하였는가?
