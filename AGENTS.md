...
*   **비즈니스 서비스 ([Services](./src/ReSet.Core/Services))**
    ...
    *   [IAiClient.cs](./src/ReSet.Core/Services/IAiClient.cs): AI 모델 간의 공통 텍스트 통신 계약 정의 인터페이스 및 프로바이더별 클라이언트 팩토리([AiClientFactory.cs](./src/ReSet.Core/Services/Clients/AiClientFactory.cs)). `ChatAsync`의 `volatileUserSuffix`는 요청마다 달라지는 짧은 지시를 받습니다 — `userPrompt`에 이어 붙여 넘기지 마십시오. Responses API 경로는 이것을 별개 메시지로 떼어 캐시 접두사를 지키고, 그 외 경로는 [PromptComposition](./src/ReSet.Core/Services/PromptComposition.cs)으로 이어 붙여 모델이 받는 내용을 같게 유지합니다.
    *   [PromptComposition.cs](./src/ReSet.Core/Services/PromptComposition.cs): 가변 접미사를 사용자 프롬프트에 합치는 규칙을 단독 소유하는 정적 클래스. 메시지를 나눌 수 없는 경로(Chat Completions·Claude·Google·Ollama·CLI)가 전부 이곳을 씁니다. 각자 이어 붙이면 구분자가 갈라져 같은 작업인데 제공자마다 프롬프트가 미묘하게 어긋납니다.
    ...
    *   [PlanStructureEnricher.cs](./src/ReSet.Core/Services/PlanStructureEnricher.cs): `SpecReturnCodeExtractor`가 뽑은 코드로 목차의 빈 `ErrorCodes`를 채워, 단계 하한 검사에 대조 기준을 주는 정적 클래스. 파싱된 객체가 아니라 목차 마크다운 문자열을 받고 돌려줍니다 — 그 문자열이 파일 기록·파싱·프롬프트의 단일 출처이기 때문입니다. `LegacyProcedures`가 빈 단계는 보존할 원본 코드가 애초에 없으므로 그대로 비워 둡니다.
...
2.  **전방위적 소프트 페일(Soft Fail) 및 예외 격리 정책을 준수하십시오.**
    ...
    *   **SQL 객체 타입 판정은 반드시 분류기를 거칠 것**: `sys` 카탈로그의 타입 문자열을 `Contains("TABLE")` 같은 부분 문자열로 직접 판정하지 마십시오. `SQL_TABLE_VALUED_FUNCTION`이 `TABLE`을 포함하므로 TVF가 테이블로 오분류되고, 그 함수의 DDL이 수집되지 않은 채 이를 호출하는 SP의 명세서가 로직을 블랙박스로 남긴 채 작성됩니다. 실제로 정산일을 계산하는 `UIF_SettleYMD`가 그렇게 누락됐습니다. [SqlObjectTypeClassifier](./src/ReSet.Core/Services/SqlObjectTypeClassifier.cs)의 `IsTableOrView` / `IsCodeObject` / `ResolveCodeObjectType`을 쓰고, 사본을 만들지 마십시오. **`TypeClassificationPolicyTests`가 자동 검사하는 범위는 정확히 "타입 문자열에 대한 원시 부분 문자열/접두·접미 판정"(`Contains`/`IndexOf`/`StartsWith`/`EndsWith`, 정규화 호출로 감싼 수신자 포함)이며, 이것이 분류 권위의 전부라는 뜻은 아닙니다.** `DependencyAnalysisOrchestrator.TryParseCodeObjectType`과 `MetadataExporter.NormalizeCodeObjectDdlFolder`는 스캐너가 못 보는 정확 일치 `switch` 테이블로 분류기 밖에 남아 있고, 분류기와 가장자리에서 어긋납니다(`"P"`/`"FN"`/`"TF"`를 분류기는 `Unresolved`로 보지만 두 테이블은 Procedure/Function으로 봅니다). 오늘 오작동하지는 않습니다 — 실제 `Type` 값은 전부 `type_desc`에서 오기 때문입니다 — 하지만 이 두 테이블을 새로 만들거나 베끼지 마십시오. 통합은 별도 후속 과제입니다.
...
- [ ] `dotnet test` 명령어를 실행하여 1135개의 단위 테스트가 모두 예외 없이 100% 통과(Passed)하였는가?
