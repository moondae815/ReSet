...
*   **비즈니스 서비스 ([Services](./src/ReSet.Core/Services))**
    ...
    *   [PlanStructureEnricher.cs](./src/ReSet.Core/Services/PlanStructureEnricher.cs): `SpecReturnCodeExtractor`가 뽑은 코드로 목차의 빈 `ErrorCodes`를 채워, 단계 하한 검사에 대조 기준을 주는 정적 클래스. 파싱된 객체가 아니라 목차 마크다운 문자열을 받고 돌려줍니다 — 그 문자열이 파일 기록·파싱·프롬프트의 단일 출처이기 때문입니다. `LegacyProcedures`가 빈 단계는 보존할 원본 코드가 애초에 없으므로 그대로 비워 둡니다. 보강할 블록은 파서에게 물어 고르고, 그 블록의 보강에 실패하면 뒤 블록으로 넘어가지 말고 보강 자체를 포기합니다 — 다른 블록을 고치면 두 목차가 갈라집니다.
