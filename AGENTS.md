...
*   **비즈니스 서비스 ([Services](./src/ReSet.Core/Services))**
    ...
    *   [StepDefect.cs](./src/ReSet.Core/Services/StepDefect.cs): 단계 하한 검사 판정을 `QualityFloor`(본문 미달, 재생성으로 고쳐짐), `Unverifiable`(대조 재료가 목차에 없어 검사가 못 돎, 재생성으로 고쳐지지 않음), `GenerationFailed`(재시도까지 빈 응답이라 본문 자체가 없음)로 가르는 열거형과 레코드. 셋을 가르는 이유는 검증률 집계가 이 구분에 기대기 때문입니다 — `QualityFloor`만 "검사가 실제로 돌았다"에 속하고 나머지 둘은 빠집니다. 새 종류를 더할 때는 [VerificationCoverage.cs](./src/ReSet.Core/Models/VerificationCoverage.cs)의 차감 목록도 함께 판단하십시오.
    ...
    *   [VerificationCoverage.cs](./src/ReSet.Core/Models/VerificationCoverage.cs): 산출물이 실제로 받은 기계 검증의 양. `PlanLayout`의 형제로 `ConsolidatedPipelineResult`에 실려, 문서 헤더의 `단계 검증: N/M`과 지시서 §0이 같은 값을 읽습니다. **검증률에서 빼는 결함 종류를 바꿀 때는 [StepDefect.cs](./src/ReSet.Core/Services/StepDefect.cs)의 구분과 함께 판단하십시오** — `QualityFloor`만 "검사가 실제로 돌았다"에 속하고 `Unverifiable`·`GenerationFailed`는 빠집니다. `StepsTotal`은 분할 미실행일 때 `null`이며 `0`으로 적으면 안 됩니다.
