...
| 파일 | 원본 | 역할 |
| :--- | :--- | :--- |
| `UP_UTIL_SETTLE_EXCEPTION_PROC.sql` | `output/Procedures/dbo.UP_UTIL_SETTLE_EXCEPTION_PROC/raw/metadata.json` 의 `DdlText` 를 그대로(줄바꿈만 LF 로 — 그 밖의 바이트는 동일) | 오라클. UPDATE 3(109~119 행)의 최상위 술어에 `(A.TXAMT-ISNULL(A.NonSettleAmt,0))` 로 인자를 감싼 괄호가 있다 |
