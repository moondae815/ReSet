...
namespace ReSet.Core.Tests
{
    ...
    public async Task RunPipelineAsync_OllamaMode_WithL3Feedback_PerformsPartialRegeneration()
    {
        ...
        public async Task RunConsolidatedPipelineAsync_L1AttributionSuccess_DoesNotConsumeScoringBudget()
        {
            ...
            var s01Calls = 0;
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
                {
            ...
            await aiService.Received(2).GenerateBatchStepSectionAsync(
                ...
                Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>());
        ...
        /// <summary>
        /// 최종 whole-branch 리뷰(Important 5): 규칙 3-1 위반(`BEGIN TRY`/`END CATCH`)이
        /// S01·S02 두 섹션 모두에 나타나면 <b>둘 다</b> 열려 지목 재생성돼야 한다.
        /// `L1ViolationAttribution.AttributeByLexeme`가 첫 발견에서 멈추면(고치기 전
        /// 동작) S02는 <c>StepFreezeState</c>에 의해 얼어붙어 다음 회차에도 같은
        /// 위반으로 L1이 다시 실패하고, Job 전체 예산인 <c>l1RepairAttempt</c>만
        /// 태우다 소진된다 - 세 번째 위반 단계가 있었다면 그 시점에 `L1Exhausted`로
        /// 끝났을 것이다.
        ///
        /// maxL2Attempts=0으로 채점 예산을 1회(attempt=1 고정)로 조인다 - 두 단계의
        /// 지목 재생성이 모두 L1 자기 예산만 쓰고 채점 예산을 안 건드려야 한다.
        /// </summary>
        [Fact]
        public async Task RunConsolidatedPipelineAsync_L1AttributionSuccess_OpensAllMatchingSteps()
        {
            var aiService = Substitute.For<IAiService>();
            aiService.BrainstormBatchPlanAsync(Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = "Brainstorm" });
            ...
            // S01·S02 둘 다 1회차 본문에 규칙 3-1 위반 고정 토큰(`BEGIN TRY`·`END CATCH`)을
            // 심는다 - ViolationLexemes가 뽑는 백틱 토큰과 정확히 같다(위 테스트 주석 참고).
            static string BadSection(BatchStepPlan step) =>
                $"### {step.Code} 단계\n\n대상은 {step.TargetTables[0]}이고 오류코드는 {step.ErrorCodes[0]}이다.\n\n" +
                "```sql\nBEGIN TRY\n    SELECT 1;\nEND TRY\nBEGIN CATCH\nEND CATCH\n```";
            ...
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                ...
                {
                    var step = call.Arg<BatchStepPlan>();
                    if (step.Code == "S01" && ++s01Calls == 1) return new AiResult { Content = BadSection(step) };
                    if (step.Code == "S02" && ++s02Calls == 1) return new AiResult { Content = BadSection(step) };
                    return new AiResult { Content = HealthyStepSection(step.Code, step.TargetTables[0], step.ErrorCodes[0]) };
                });
            aiService.ReviewConsolidatedPlanAsync(Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
                .Returns(new ReviewResult { HasDefects = false, ScoreAccuracy = 10, ScoreCrud = 10, ScoreInterface = 10, ScoreException = 10, ScoreReadability = 10 });
            ...
            // 골격은 한 번만 - 전량 재생성이 아니라 지목 재생성이라는 직접 증거.
            await aiService.Received(1).GenerateBatchPlanSkeletonAsync(
                Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(),
                Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>());

            // 핵심 단언: S01·S02 둘 다 2회씩(최초 + 수리) 불린다. 첫 발견에서 멈췄다면
            // (고치기 전 동작) S02는 1회만 불려(동결) 이 어서션이 실패했을 것이다.
            await aiService.Received(2).GenerateBatchStepSectionAsync(
                Arg.Is<BatchStepPlan>(s => s.Code == "S01"), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(),
                Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(),
                Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>());
            ...
            // 좁힌 단계 안내 메시지에 S01·S02 둘 다 실린다 - pendingDefectiveSteps가
            // 두 코드를 모두 담았다는 오케스트레이터 레벨의 직접 증거.
            ui.Received(1).NotifyStatus(Arg.Is<string>(s => s.Contains("S01") && s.Contains("S02") && s.Contains("좁혀")));
        ...
        /// <summary>
        /// 재개가 실제로 비용을 줄였다는 <b>유일한 직접 증거</b>는 AI 호출 수다(설계 §5-2).
        /// 재사용한 단계는 GenerateBatchStepSectionAsync가 불리지 않아야 하고,
        /// 브레인스토밍·목차 생성도 통째로 건너뛰어야 한다.
        ///
        /// [계획서 수정] 원 계획의 두 가지가 빠져 있었다:
        /// (1) seed의 Effort("high")가 orchestrator 생성자의 consolidatorEffort("default")와
        /// 달라 ReuseKey가 절대 맞지 않았다 - orchestrator가 실제로 쓰는
        /// _consolidatorEffort와 seed를 맞췄다.
        /// (2) seed.OpenRun은 attempts/run-001/manifest.json만 쓰고 raw/PlanStructure.md는
        /// 쓰지 않는다 - Step 4가 읽는 파일은 그것이므로, 여기서 직접 써 둬야
        /// 재개 판정 코드가 후보를 찾을 수 있다.
        /// </summary>
        [Fact]
        public async Task RunConsolidatedPipeline_WhenResumeIsApproved_OnlyDefectiveStepsAreRegenerated()
        {
            ...
            // 1) 이어서 할 판을 미리 써 둔다 - S01은 건강, S02는 하한 미달.
            //    Effort는 orchestrator가 실제로 쓰는 consolidatorEffort("default")와
            //    맞춘다 - 다르면 ReuseKey가 안 맞아 재개 후보를 영영 못 찾는다.
            ...
            // 재개 판정은 raw/PlanStructure.md에서 목차를 읽는다(설계 §3-1 정정) -
            // seed.OpenRun은 attempts/ 디렉터리만 쓰므로 이 파일은 직접 만들어야 한다.
            ...
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
                {
                    var step = call.Arg<BatchStepPlan>();
            ...
            // S01은 재사용됐으므로 AI가 안 불린다. S02만 다시 만든다.
            Assert.Equal(new[] { "S02" }, regenerated.Distinct().ToArray());

            // 브레인스토밍·목차 생성 AI 호출 둘도 건너뛰어야 한다(§5-2가 재는 것은
            // 단계 호출만이 아니다 - 재개의 값 전체가 이 둘에도 걸려 있다).
            await aiService.DidNotReceive().BrainstormBatchPlanAsync(
        ...
        public async Task RunConsolidatedPipeline_WhenResumeIsDeclined_EverythingIsRegenerated()
        {
            ...
            // 거절은 게이트(§3-4-b)가 묻는 경로에서만 뜻이 있다 - 무인 배치는 묻지
            // 않고 자동 재개하므로 isBatchMode:true로는 「거절」을 재현할 수 없다
            // (계획서의 원 스니펫이 isBatchMode:true를 썼는데, 그러면 거절값이 아예
            // 안 읽혀 regenerated가 통째로 빈 채로 끝난다 - 실측으로 발견). 대화형
            // 경로를 타려면 L3 승인 루프까지 닿으므로 Approve를 함께 스텁한다.
        ...
        /// <summary>
        /// 무인 배치는 묻지 않고 재개한다(설계 §3-4-b). 게이트가 없으면 AnsiConsole.Confirm이
        /// 비대화형에서 예외를 던져 <b>재개 후보가 있는 모든 무인 배치가 하드 실패로 죽는다</b> -
        /// Task 2 재리뷰어가 Spectre 0.55.2로 실측했다. 「승인」시험과 같은 셋업을 쓰되
        /// interaction은 절대 불리면 안 된다 - 배치 모드는 응답을 기다리지 않고 그냥 재개한다.
        /// </summary>
        [Fact]
        public async Task RunConsolidatedPipeline_InBatchMode_ResumesWithoutAsking()
        {
            ...
            // 무인 배치가 대화형 확인을 시도조차 하지 않는다는 것을 세운다 - Confirm 자체가
            // 예외를 던지는 대체물이었다면, 이 substitute는 애초에 불려서는 안 된다.
            var interaction = Substitute.For<IVerificationUserInteraction>();
        ...
        /// 외부 <c>while(true)</c> 루프(<c>attempt</c>)는 attempt=1에서 그대로
        /// break한다. 실제로 이 시험이 재는 두 번째 호출은
        /// <c>GenerateStepSectionWithFloorRetryAsync</c> <b>내부</b>의 하한 재시도
        /// 루프(같은 attempt 안, maxTries=5)다 - 1차 시도 본문이 하한 미달로 판정돼
        /// 2차 내부 시도로 넘어가기 <b>직전</b>에 그 본문이 이미 디스크에 있는지 본다.
        ///
        /// 외부 attempt 루프가 실제로 넘어갈 때 저널이 살아남는지는 별개 시험
        /// (<see cref="RunConsolidatedPipeline_WhenTheOuterAttemptAdvances_ThePriorAttemptsSectionsSurviveOnDisk"/>)
        /// 이 돈다 - 이 시험만으로는 그 사실을 증명하지 못한다.
        /// </summary>
        [Fact]
        public async Task RunConsolidatedPipeline_WhenAStepFailsItsInnerFloorRetry_ThePriorDraftIsOnDiskBeforeTheNextTry()
        {
            var stepsJson = "```json\n{\n  \"Steps\": [\n" +
                "    { \"Code\": \"S01\", \"Name\": \"첫 단계\", \"LegacyProcedures\": [\"USP_Spec1\"], \"TargetTables\": [\"dbo.T1\"], \"ErrorCodes\": [\"-1\"] }\n" +
                "  ]\n}\n```";
        ...
        // [최종 전체 브랜치 리뷰 Minor 4] 위 시험은 하한 재시도 중간본이 디스크에
        // 남는지만 잰다 - 그 중간본에 결함 칸(DefectKind)이 함께 남는지는 다른
        // 사실이다. journal.RecordStepSection(attempt, step.Code, content)로 그
        // 자리의 defect 인자만 지워도 위 시험은 여전히 초록이다(본문은 그대로
        // 남으므로). 이 시험이 그 결함 칸을 잡는다.
        [Fact]
        public async Task RunConsolidatedPipeline_WhenAStepFailsItsInnerFloorRetry_ThePriorDraftsDefectKindIsOnDiskBeforeTheNextTry()
        {
            var stepsJson = "```json\n{\n  \"Steps\": [\n" +
                "    { \"Code\": \"S01\", \"Name\": \"첫 단계\", \"LegacyProcedures\": [\"USP_Spec1\"], \"TargetTables\": [\"dbo.T1\"], \"ErrorCodes\": [\"-1\"] }\n" +
                "  ]\n}\n```";
            ...
            var journalDir = Path.Combine(
                _consolidatedOutputRoot, "Jobs", "Job_Test", "raw", "attempts", "run-001");
            string? s01DefectKindAtSecondCall = null;
            var sectionCalls = 0;
            ...
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
                {
                    ...
                    if (sectionCalls > 1 && s01DefectKindAtSecondCall == null)
                    {
                        var manifestPath = Path.Combine(journalDir, "manifest.json");
                        if (File.Exists(manifestPath))
                        {
                            var s01 = JsonDocument.Parse(File.ReadAllText(manifestPath))
                                .RootElement.GetProperty("Steps").GetProperty("S01");
                            s01DefectKindAtSecondCall = s01.GetProperty("DefectKind").ValueKind == JsonValueKind.Null
                                ? "(null)"
                                : s01.GetProperty("DefectKind").GetString();
                        }
                    }
            ...
            Assert.True(sectionCalls > 1, "재시도가 일어나지 않아 이 시험이 재려는 것을 재지 못했다.");
            // 하한 미달 중간본이 남긴 결함 칸은 null이 아니어야 한다 - 본문이
            // "(본문 없음 - 하한 미달)"뿐이었던 그 순간의 관측이다.
            Assert.NotNull(s01DefectKindAtSecondCall);
            Assert.NotEqual("(null)", s01DefectKindAtSecondCall);
            Assert.Equal("QualityFloor", s01DefectKindAtSecondCall);
        }

        /// <summary>
        /// [FIX ROUND 1 - Critical] 이 시험이 사용자가 실제로 물은 것을 잰다 —
        /// <c>RunConsolidatedPipelineAsync</c>의 외부 <c>while(true)</c> 루프가
        /// <c>attempt</c>를 실제로 2로 올릴 때, 회차 1이 쓴 골격·섹션이 회차 2의
        /// 첫 AI 호출 시점에 이미 디스크에 있는가.
        ///
        /// 외부 루프를 태우는 경로: 회차 1의 명세서가 원본 반환 코드 -9010을 선언하는데
        /// 목차의 어느 단계도 그 코드를 ErrorCodes로 선언하지 않는다(귀속 불가). 회차
        /// 1의 본문에도 "-9010"이 없으므로 <c>MechanicalValidator.FindMissingErrorCodes</c>가
        /// 이를 잡고, <c>ErrorCodeAttribution.Attribute</c>가 귀속 실패(HasUnattributed)로
        /// 판정해 L1이 무효가 된다. 귀속 실패는 L1 자기 예산이 아니라 채점 예산
        /// (attempt)을 쓴다(:2419 이하 "else" 분기) - 이 분기는 <c>lastSkeleton</c>·
        /// <c>lastStepSections</c>를 전부 null로 비운다(§3-2 전량 재생성). 그래서
        /// 회차 2는 골격부터 완전히 다시 부른다 - 회차 2의 첫 AI 호출은 골격 생성이다.
        ///
        /// Critic(L2)은 여기 관여하지 않는다 - L1이 먼저 걸려 continue하므로 회차
        /// 1에서는 <c>ReviewConsolidatedPlanAsync</c>가 아예 호출되지 않는다. 회차
        /// 2는 본문에 "-9010"을 실어 L1을 통과시키고, Critic이 결함 없음을 내
        /// 파이프라인이 종료한다.
        /// </summary>
        [Fact]
        public async Task RunConsolidatedPipeline_WhenTheOuterAttemptAdvances_ThePriorAttemptsSectionsSurviveOnDisk()
        {
            // LegacyProcedures가 명세서 파일명(USP_Spec1)과 일부러 다르다 -
            // PlanStructureEnricher가 legacy 프로시저 매칭으로 specReturnCodes의
            // -9010을 이 단계의 ErrorCodes에 자동으로 채워 넣으면(실제로 그런다),
            // 어느 단계도 그 코드를 "선언하지 않은" 상태를 만들 수 없다 - 귀속 실패
            // 시나리오 자체가 성립하지 않는다.
            var stepsJson = "```json\n{\n  \"Steps\": [\n" +
                "    { \"Code\": \"S01\", \"Name\": \"첫 단계\", \"LegacyProcedures\": [\"USP_Unrelated\"], \"TargetTables\": [\"dbo.T1\"], \"ErrorCodes\": [\"-1\"] }\n" +
                "  ]\n}\n```";
            ...
            var skeletonCalls = 0;
            string? s01AtOuterAttemptTwo = null;
            string? skeletonAtOuterAttemptTwo = null;
            aiService.GenerateBatchPlanSkeletonAsync(Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(call =>
                {
                    skeletonCalls++;
                    if (skeletonCalls == 2)
                    {
                        // 회차 2의 첫 AI 호출 시점 - 회차 1이 쓴 것이 이미 있어야 한다.
                        var stepPath = Path.Combine(journalDir, "steps", "S01.md");
                        var skeletonPath = Path.Combine(journalDir, "skeleton.md");
                        s01AtOuterAttemptTwo = File.Exists(stepPath) ? File.ReadAllText(stepPath) : null;
                        skeletonAtOuterAttemptTwo = File.Exists(skeletonPath) ? File.ReadAllText(skeletonPath) : null;
                    }

                    return new AiResult { Content = SkeletonMarkdown };
                });
            ...
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
                {
                    var step = call.Arg<BatchStepPlan>();
                    stepCalls++;
                    // 회차 1: -9010을 싣지 않는다(귀속 불가 유발). 회차 2: -9010을 실어
                    // L1을 통과시킨다.
                    var body = stepCalls == 1
                        ? HealthyStepSection(step.Code, step.TargetTables[0], step.ErrorCodes[0])
                        : HealthyStepSection(step.Code, step.TargetTables[0], step.ErrorCodes[0]) +
                          "\n\n원본 반환 코드 -9010도 그대로 기록한다.";
                    return new AiResult { Content = body };
                });
            ...
            // 명세서가 원본 반환 코드 -9010을 선언한다(SpecReturnCodeExtractor가
            // 읽는 대입문 형태).
            var specs = new List<(string, string)>
            {
                ("dbo.USP_Spec1", "## 개요\n\n실패 시 SET @po_intRetVal = -9010 후 반환한다.\n")
            ...
            // attempt가 실제로 외부 루프에서 2로 올랐다는 직접 증거 - 회차 1에서
            // NotifyL1Errors가 attempt=1로 호출됐어야 한다. 내부 하한 재시도는
            // 이 알림을 절대 부르지 않는다.
            userInteraction.Received(1).NotifyL1Errors("Job_Test", 1, Arg.Any<int>(), Arg.Any<List<string>>());
            Assert.Equal(2, skeletonCalls);
            Assert.Equal(2, stepCalls);

            // 회차 2의 첫 AI 호출(골격 재생성) 시점에 회차 1의 산출물이 이미 있었다.
            Assert.NotNull(s01AtOuterAttemptTwo);
            Assert.DoesNotContain("-9010", s01AtOuterAttemptTwo!);
        ...
        public async Task RunConsolidatedPipeline_LegacyProcedureSplitAcrossSteps_PassesWithoutRetry()
        {
            ...
            await aiService.Received(2).GenerateBatchStepSectionAsync(
                ...
                Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>());
        }
        ...
        public async Task RunConsolidatedPipeline_WhenAStepExhaustsQuotaAfterAllRetries_ReportsAbortReasonEvenThoughAStubIsProduced()
        {
            ...
            _aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns<AiResult>(_ => throw new AiCallFailedException(
                    "쿼터 소진", new Exception("한도"), 1, AiRetryVerdict.Exhausted));
            _aiService.ReviewConsolidatedPlanAsync(Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
                .Returns(new ReviewResult { HasDefects = false, ScoreAccuracy = 10, ScoreCrud = 10, ScoreInterface = 10, ScoreException = 10, ScoreReadability = 10 });
        ...
        // [FINAL FIX - Critical] 구제 채택이 재설계 이전 목차로 되돌아갈 때 여는 판이
        // 비어 있었다 - RestoreAdoptedGenerationState가 메모리로 복원하는 골격·섹션이
        // 그 판에는 다시 기록되지 않아, 재사용 키가 같은 목차 해시를 가리키는 더 오래된
        // 판(재료가 있다, run-001)과 방금 연 판(재료가 없다, run-003) 사이에서 2단계의
        // "지금 목차 해시와 같은 최신 판" 판 고르기가 재료 0개인 판을 고른다.
        //
        // 위 RescueAdoptsPreRedraftAttempt_...와 같은 점수 시나리오(70->60->60(재설계
        // 트리거)->40)를 쓰되, 이 시험은 분할 경로(StepsJson)를 태워 골격·섹션이
        // 실제로 존재하게 한다 - 단일 호출 경로에는 되살릴 재료 자체가 없다.
        [Fact]
        public async Task RunConsolidatedPipelineAsync_RescueAfterRedraft_ReopenedRunCarriesSkeletonAndSections()
        {
            var stepsJson = "```json\n{\n  \"Steps\": [\n    { \"Code\": \"S01\", \"Name\": \"첫 단계\", \"LegacyProcedures\": [\"USP_Spec1\"], \"TargetTables\": [\"dbo.T1\"], \"ErrorCodes\": [\"-1\"] }\n  ]\n}\n```";
            var firstStructure = "## 목차 A\n" + stepsJson;
            var redraftStructure = "## 목차 B\n" + stepsJson;

            var aiService = Substitute.For<IAiService>();
            aiService.BrainstormBatchPlanAsync(Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = "Brainstorm Result" });
            aiService.DraftBatchPlanStructureAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<IReadOnlyList<string>>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = firstStructure }, new AiResult { Content = redraftStructure });
            aiService.GenerateBatchPlanSkeletonAsync(Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = SkeletonMarkdown });
                ...
                .Returns(call =>
                {
                    ...
                    return new AiResult
                    {
                        Content = HealthyStepSection(step.Code, step.TargetTables[0], step.ErrorCodes[0]) +
                            $"\n\n<!-- attempt-marker:{stepCalls} -->"
                    };
                });

            // 70 -> 60 -> 60(2회 연속 정체 + 구조 결함 지목 -> 3차에서 재설계) -> 40.
            // 1차가 최고점이라 예산 소진 시 1차가 채택된다 - 그 채택이 구제 채택이다.
            aiService.ReviewConsolidatedPlanAsync(Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())
                .Returns(
                    _ => Task.FromResult(new ReviewResult { HasDefects = true, StructureDefective = true, FeedbackComment = "결함", ScoreAccuracy = 7, ScoreCrud = 7, ScoreInterface = 7, ScoreException = 7, ScoreReadability = 7 }),
                    _ => Task.FromResult(new ReviewResult { HasDefects = true, StructureDefective = true, FeedbackComment = "결함", ScoreAccuracy = 6, ScoreCrud = 6, ScoreInterface = 6, ScoreException = 6, ScoreReadability = 6 }),
            ...
            var orchestrator = new VerificationPipelineOrchestrator(
                Substitute.For<IDbMetadataService>(), aiService, new MechanicalValidator(),
                userInteraction, "3", "gpt-4", null, aiService, aiService, "high", "high", "default", 8);

            var specs = new List<(string, string)> { ("dbo.USP_Spec1", "content1") };

            await orchestrator.RunConsolidatedPipelineAsync(
                specs, "C#", "RescueReopenJob", "OpenAI", _consolidatedOutputRoot, isBatchMode: true);

            var attemptsDir = Path.Combine(_consolidatedOutputRoot, "Jobs", "RescueReopenJob", "raw", "attempts");
            // run-001: "목차 A"(run-start). run-002: "목차 B"(structure-redraft, 3->4차
            // 경계). run-003: 회귀 롤백이 "목차 A"로 되돌리며 여는 판 - 이 판이 비어
            // 있었다.
            var run3Dir = Path.Combine(attemptsDir, "run-003");
            Assert.True(Directory.Exists(run3Dir), "구제 채택이 세 번째 판을 열지 않았다.");

            var manifest3 = JsonDocument.Parse(
                await File.ReadAllTextAsync(Path.Combine(run3Dir, "manifest.json"))).RootElement;
            Assert.Equal("rescue-adopt", manifest3.GetProperty("OpenedBy").GetString());
            Assert.Equal(
                PlanAttemptJournal.ComputeSha256(firstStructure),
                manifest3.GetProperty("ReuseKey").GetProperty("PlanStructureSha256").GetString());

            // 재료가 실제로 다시 기록됐는가 - RestoreAdoptedGenerationState가 메모리로
            // 복원하는 그 값이 곧바로 이 판에도 적혀야 한다.
            Assert.True(File.Exists(Path.Combine(run3Dir, "skeleton.md")), "구제로 연 판에 골격이 없다.");
            Assert.True(File.Exists(Path.Combine(run3Dir, "steps", "S01.md")), "구제로 연 판에 단계 섹션이 없다.");

            Assert.True(
                manifest3.GetProperty("Steps").TryGetProperty("S01", out var s01Entry),
                "manifest가 재기록된 섹션을 모른다.");
            // 채택된 본문은 1차 시도가 만든 것이다 - 그 시도 번호가 남아야 한다.
            Assert.Equal(1, s01Entry.GetProperty("Attempt").GetInt32());

            var s01Content = await File.ReadAllTextAsync(Path.Combine(run3Dir, "steps", "S01.md"));
            Assert.Contains("attempt-marker:1", s01Content);
        }
...
" + StepsJson;
            ...
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
                {
                    var step = call.Arg<BatchStepPlan>();
...
```";
            ...
            await aiService.Received().GenerateBatchStepSectionAsync(
                ...
                Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>());
        }
        ...
        // [FINAL FIX - Important 2] 저널 manifest가 단계의 결함 종류를 남기는지
        // 오케스트레이터 배선 수준에서 확인한다(PlanAttemptJournalTests는 저널
        // 자신의 계약만 잰다 - RunStepAsync·GenerateStepSectionWithFloorRetryAsync가
        // 실제로 violation을 journal.RecordStepSection까지 실어 나르는지는 여기서만
        // 잡힌다). S01은 하한 미달로 확정되고 S02는 건강하다 - manifest의 DefectKind가
        // 그 갈림을 그대로 반영해야 한다.
        [Fact]
        public async Task GenerateBySplitAsync_StepFloorViolation_RecordsDefectKindInJournalManifest()
        {
            var aiService = Substitute.For<IAiService>();
            aiService.GenerateBatchPlanSkeletonAsync(Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = SkeletonMarkdown });
            // S01은 코드 블록이 없어 하한 미달로 확정된다. S02는 항상 건강하다.
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
                {
                    var step = call.Arg<BatchStepPlan>();
            ...
            var journal = PlanAttemptJournal.Create(
                _consolidatedOutputRoot, "DefectKindJob", "OpenAI", "gpt-4", null, "C#",
                PlanAttemptJournal.ComputeSha256("specs"));
            journal.OpenRun("목차", "run-start");

            await InvokeGenerateBySplitAsync(
                orchestrator, "목차", steps!, specs, "C#", "DefectKindJob",
                NullProgressScope.Instance, null, null, null, new Dictionary<string, StepDefect>(),
                Array.Empty<string>(), Array.Empty<string>(), CancellationToken.None,
                journal: journal);

            var manifestSteps = JsonDocument.Parse(
                    File.ReadAllText(Path.Combine(journal.CurrentRunDirectory!, "manifest.json")))
                .RootElement.GetProperty("Steps");

            Assert.NotEqual(JsonValueKind.Null, manifestSteps.GetProperty("S01").GetProperty("DefectKind").ValueKind);
            Assert.Equal(JsonValueKind.Null, manifestSteps.GetProperty("S02").GetProperty("DefectKind").ValueKind);
        }
...
```";
            ...
            _aiService.GenerateBatchPlanSkeletonAsync(Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = SkeletonMarkdown });
            _aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
                {
                    var step = call.Arg<BatchStepPlan>();
                    return new AiResult { Content = HealthyStepSection(step.Code, step.TargetTables[0], step.ErrorCodes[0]) };
        ...
        /// (<c>GOTO</c>)을 심는다. 이 자리는 단계 섹션 <b>밖</b>이므로
        /// <see cref="L1ViolationAttribution"/>이 <b>골격</b>으로 귀속한다 -
        /// 골격 지목 재생성 경로(반복 실행 판2·판3이 지목한 결함의 수리)를 타는
        /// 픽스처다. 귀속 자체가 실패하는 픽스처는 아래
        /// <see cref="SkeletonMarkdownWithUnattributableShortcut"/>다.
        /// </summary>
        private const string SkeletonMarkdownWithSkeletonGoto = @"## 통합 배치 아키텍처 개요
개요.
...
```";
        ...
        /// <summary>
        /// 어느 자리로도 귀속되지 않는 위반. 축약어 검사는 금지 토큰을 <b>작은따옴표</b>로
        /// 싣고(`'이하 생략'`) 백틱으로 싣지 않으므로
        /// <see cref="MechanicalValidator.ViolationLexemes"/>가 빈 어휘를 낸다 - 어휘가
        /// 없으면 훑을 것이 없어 단계에도 골격에도 붙지 않는다. 골격 귀속이 생긴 뒤에도
        /// 남아 있는 「자리를 모른다」 부류의 실물이고, 그때의 답은 여전히 전량 재생성이다.
        /// </summary>
        private const string SkeletonMarkdownWithUnattributableShortcut = @"## 통합 배치 아키텍처 개요
개요.
...
```";
        ...
        ///
        /// 골격 자체에 귀속 불가능한 위반(축약어 `이하 생략` - 검사가 백틱 어휘를
        /// 싣지 않아 훑을 것이 없다)을 심어 뒀으므로 L1은 매 회차 같은 이유로 계속
        /// 실패한다 - 그래서 리뷰(L2)에는 도달하지 않고, 오직 "귀속 실패 -> 전량
        /// 재생성"의 섹션 호출 횟수만으로 고정한다. 골격 귀속이 생긴 뒤에도 이 부류는
        /// 남아 있고, 그때의 답이 여전히 전량 재생성이라는 것이 이 테스트의 몫이다.
        ...
        public async Task RunConsolidatedPipeline_L1AttributionFailureInSplitPath_RegeneratesAllSectionsNextRound()
        {
            ...
            aiService.GenerateBatchPlanSkeletonAsync(Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = SkeletonMarkdownWithUnattributableShortcut });
        ...
        /// <summary>
        /// 반복 실행 3판(2026-09-04)이 재현성 저하의 최대 원인으로 지목한 결함의 닫힘
        /// 시험이다. 골격(공통 규약 절)에 있는 L1 위반은 지목 재생성이 열지 못했고
        /// (그 경로는 단계 섹션만 연다), 귀속 실패의 폴백인 전량 재생성조차 골격을
        /// 다시 만들지 않았다 - 호출부가 <c>lastSkeleton</c>을 일부러 남겼기 때문이다.
        /// 판3에서는 깨진 mermaid 하나가 글자 하나 안 바뀐 채 6회차를 전부 태웠고,
        /// 판2에서는 공통 절의 <c>IF @@ROWCOUNT = 0</c>이 같은 일을 했다.
        ///
        /// 고정하는 불변식 넷:
        /// <list type="number">
        /// <item>골격이 <b>다시 만들어진다</b>(호출 2회) - 결함이 실제로 닿는다.</item>
        /// <item>두 번째 호출이 <b>패치</b>다 - 직전 골격을 싣고 그 위에서 고치라고
        /// 한다. 단계 목록 표·오류 코드 표의 정합이 여기서 지켜진다.</item>
        /// <item>단계 섹션은 <b>동결</b>이다(각 1회) - 골격을 고치자고 멀쩡한 단계를
        /// 다시 쓰면 회귀 롤백이 막으려는 회귀를 그대로 들인다.</item>
        /// <item>채점 예산(attempt)을 <b>안 쓴다</b> - maxL2Attempts=0으로 총 1회로
        /// 조여 두고도 통과한다. 이것이 판2 3회 · 판3 5회를 되찾는 자리다.</item>
        /// </list>
        /// </summary>
        [Fact]
        public async Task RunConsolidatedPipeline_L1ViolationInSkeleton_RepairsSkeletonOnlyAndKeepsScoringBudget()
        {
            var aiService = Substitute.For<IAiService>();
            aiService.BrainstormBatchPlanAsync(Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = "Brainstorm" });
            ...
            // 1회차 골격은 공통 규약 절에 규칙 3-1 위반(GOTO)을 담고, 수리 호출은
            // 깨끗한 골격을 낸다 - 그 사이에 무엇이 오갔는지를 아래에서 붙잡는다.
            var skeletonRevisions = new List<SkeletonRevision?>();
            aiService.GenerateBatchPlanSkeletonAsync(Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(call =>
                {
                    var revision = call.ArgAt<SkeletonRevision?>(8);
                    skeletonRevisions.Add(revision);
                    return Task.FromResult(new AiResult
                    {
                        Content = revision == null ? SkeletonMarkdownWithSkeletonGoto : SkeletonMarkdown
                    });
                });
            ...
            var ui = Substitute.For<IVerificationUserInteraction>();
            // maxL2Attempts=0 -> 총 채점 예산 1회. 골격 수리가 채점 예산을 쓴다면
            // 여기서 소진되어 통과할 수 없다.
            var orchestrator = new VerificationPipelineOrchestrator(
                Substitute.For<IDbMetadataService>(), aiService, new MechanicalValidator(),
                ui, "0", "gpt-4", null,
            ...
            // ① 골격이 실제로 다시 만들어졌다. 고치기 전에는 1회였고, 그래서 같은
            //    위반이 예산이 다할 때까지 그대로 남았다.
            Assert.Equal(2, skeletonRevisions.Count);

            // ② 두 번째 호출이 패치다 - 직전 골격을 싣고, 고칠 것을 이름으로 댄다.
            Assert.Null(skeletonRevisions[0]);
            Assert.NotNull(skeletonRevisions[1]);
            Assert.Contains("GOTO", skeletonRevisions[1]!.PreviousSkeleton);
            Assert.Contains("GOTO", skeletonRevisions[1]!.Feedback);

            // ③ 단계 섹션은 동결됐다 - 각 1회씩만 불렸다.
            Assert.Equal(new Dictionary<string, int> { ["S01"] = 1, ["S02"] = 1 }, sectionCallsByStep);

            // ④ 채점 예산을 안 썼다. attempt가 1에 고정된 채로 통과했다.
            ui.Received(1).NotifyL1Errors("Job_Test", 1, Arg.Any<int>(), Arg.Any<List<string>>());
            ui.Received(1).NotifyStatus(Arg.Is<string>(m => m.Contains("골격") && m.Contains("좁혀")));
        }
        ...
        ///
        /// 골격이 매 회차 같은 위반을 그대로 내도록 두고, 수리 요청 둘의 모양을 본다:
        /// 첫 수리는 직전 골격을 싣고(패치), 두 번째는 싣지 않는다(백지).
        /// </summary>
        [Fact]
        public async Task RunConsolidatedPipeline_SkeletonRepairedTwiceForSameDefect_EscalatesToBlankRewrite()
        {
            var aiService = Substitute.For<IAiService>();
            aiService.BrainstormBatchPlanAsync(Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
                .Returns(new AiResult { Content = "Brainstorm" });
            ...
            // 수리해도 같은 위반을 그대로 낸다 - 패치가 근본 결함을 못 고치는 상태다.
            var skeletonRevisions = new List<SkeletonRevision?>();
            aiService.GenerateBatchPlanSkeletonAsync(Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(call =>
                {
                    skeletonRevisions.Add(call.ArgAt<SkeletonRevision?>(8));
                    return Task.FromResult(new AiResult { Content = SkeletonMarkdownWithSkeletonGoto });
                });
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
                {
            ...
            var orchestrator = new VerificationPipelineOrchestrator(
                Substitute.For<IDbMetadataService>(), aiService, new MechanicalValidator(),
                Substitute.For<IVerificationUserInteraction>(), "0", "gpt-4", null,
                aiService, aiService, "high", "high", "default", 8,
                stepConcurrency: 1, maxL1RepairAttempts: 2);
            var specs = new List<(string, string)> { ("dbo.USP_Spec1", "content1") };
            ...
            // 수리 예산 2회를 다 쓰고도 못 고쳤으므로 L1Exhausted다 - 예산이 무한히
            // 늘지 않는다는 것도 함께 고정한다.
            Assert.Equal(VerificationOutcome.L1Exhausted, result.Outcome);
            ...
            Assert.Equal(3, skeletonRevisions.Count);
            Assert.Null(skeletonRevisions[0]);
            Assert.NotNull(skeletonRevisions[1]!.PreviousSkeleton);   // 첫 수리는 패치다.
            Assert.Null(skeletonRevisions[2]!.PreviousSkeleton);      // 두 번째는 백지다.
            Assert.Contains("GOTO", skeletonRevisions[2]!.Feedback);  // 무엇을 고칠지는 여전히 싣는다.
        }

        /// <summary>
        /// 패치가 정합을 깼을 때의 기계 가드. 모델이 「나머지는 바이트 그대로」를 어기고
        /// 오류 코드 표에서 코드를 흘리면, 그 자리를 <b>이름으로 대고</b> 한 번 더 부른다.
        ///
        /// 프롬프트 계약만으로는 안 되는 이유는 <see cref="SkeletonRevisionGuard"/>가
        /// 적는다 - 표에서 사라진 오류 코드는 다음 회차의 L1(오류 코드 누락)으로
        /// 나타나고, 그 귀속은 코드를 선언한 <b>멀쩡한 단계</b>를 연다.
        /// </summary>
        [Fact]
        public async Task GenerateBySplitAsync_SkeletonPatchDropsErrorCode_RetriesOnceWithTheLostAnchor()
        {
            var steps = BatchStepPlanParser.TryParse(StepsJson);
            Assert.NotNull(steps);
            var errorCode = steps![0].ErrorCodes[0];

            // 직전 골격은 그 오류 코드를 담고 있고, 첫 패치 결과는 그것을 흘린다.
            var previousSkeleton = SkeletonMarkdown + $"\n\n| {steps[0].Code} | `{errorCode}` |\n";
            ...
            aiService.GenerateBatchPlanSkeletonAsync(Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(call =>
                {
                    feedbacks.Add(call.ArgAt<SkeletonRevision?>(8)!.Feedback);
                    // 1회차는 표를 흘리고, 2회차(가드 재시도)는 되살린다.
                    return Task.FromResult(new AiResult
                    {
                        Content = feedbacks.Count == 1 ? SkeletonMarkdown : previousSkeleton
                    });
                });
            aiService.GenerateBatchStepSectionAsync(Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(), Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(call =>
            ...
            var result = await InvokeGenerateBySplitAsync(
                orchestrator, "목차", steps, new List<(string FileName, string Content)> { ("dbo.USP_Spec1", "c") },
                "C#", "Job_Test", NullProgressScope.Instance,
                previousSkeleton, new AiResult { Content = previousSkeleton }, previousSections,
                new Dictionary<string, StepDefect>(), Array.Empty<string>(), Array.Empty<string>(),
                CancellationToken.None,
                skeletonRevision: new SkeletonRevision("흐름도를 고치십시오.", previousSkeleton));

            Assert.NotNull(result);

            // 가드가 한 번 더 불렀고, 그 요청이 잃은 앵커를 이름으로 댔다.
            Assert.Equal(2, feedbacks.Count);
            Assert.DoesNotContain(errorCode, feedbacks[0]);
            Assert.Contains(errorCode, feedbacks[1]);

            // 채택된 것은 앵커를 되살린 두 번째 결과다.
            Assert.Equal(previousSkeleton, GetSkeleton(result!));

            // 동결된 섹션은 그대로다 - 가드 재시도가 단계를 건드리지 않는다.
            await aiService.DidNotReceive().GenerateBatchStepSectionAsync(
                Arg.Any<BatchStepPlan>(), Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(),
                Arg.Any<List<(string, string)>>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(),
                Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>());
...
```";
        ...
        public async Task RunConsolidatedPipeline_AfterFeedbackRetry_DoesNotReportFeedbackLogAsUncoveredProcedure()
        {
            ...
            await aiService.Received(1).GenerateBatchStepSectionAsync(
                ...
                Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>());
...
""";
        ...
        private void ArrangeSplitGeneration()
        {
            ...
            _aiService.GenerateBatchPlanSkeletonAsync(
                    ...
                    Arg.Any<List<(string FileName, string Content)>>(), Arg.Any<string>(),
                    Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
            ...
            _aiService.GenerateBatchStepSectionAsync(
                    ...
                    Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(),
                    Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<System.Collections.Generic.IReadOnlyDictionary<string, System.Collections.Generic.IReadOnlyList<string>>>(), Arg.Any<CancellationToken>(), Arg.Any<IReadOnlyList<(string StepCode, string Body)>?>(), Arg.Any<IReadOnlyDictionary<string, IReadOnlyList<string>>?>(), Arg.Any<string?>())
                .Returns(callInfo =>
                {
        ...
        public async Task RunConsolidatedPipelineAsync_Layout_StaysConsistentAfterRedraftThenRescue()
        {
            ...
            aiService.GenerateBatchPlanSkeletonAsync(
                    Arg.Any<IReadOnlyList<BatchStepPlan>>(), Arg.Any<string>(),
                    Arg.Any<List<(string, string)>>(), Arg.Any<string>(), Arg.Any<string>(),
                    Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<IReadOnlyList<StepInterface>>(), Arg.Any<SkeletonRevision?>(), Arg.Any<CancellationToken>())
                .Returns(call =>
                {
                    var steps = call.Arg<IReadOnlyList<BatchStepPlan>>();
                    return new AiResult { Content = SkeletonMarkdownFor(steps.Select(s => s.Code).ToArray()) };
...
```";
        ...
        //
        // FIX ROUND 1 리뷰 지적(정밀화): 이 테스트는 수정 전 코드에서도 그냥
        // 통과한다 - 수정 전 코드는 missingErrorCodes가 빈 회차마다 무조건
        // false로 되돌리므로, "L2에 도달하는 회차엔 값이 항상 거짓"인 원래
        // 버그(회차 어긋남) 자체가 이 테스트가 조립하는 조건(2회 연속 정체 +
        // Critic 침묵)에서도 우연히 같은 결과(재설계 없음)를 낸다. 그 회차
        // 어긋남 버그를 실제로 잡아내는 것은 옆의
        // RunConsolidatedPipeline_MachineFoundStructureDefectAlone_
        // TriggersRedraftAfterStagnation이다(그 테스트는 수정 전에는 실패한다).
        //
        // 이 테스트가 지키는 것은 그와 다른, "해소 후 리셋" 분기 하나다.
        ...
        // 시나리오를 써서, 고친 코드가 그 분기를 제대로 타는지(그리고 리팩터로
        // 그 분기가 지워지면 실패하는지)를 고정한다.
        //
