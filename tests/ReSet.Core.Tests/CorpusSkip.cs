namespace ReSet.Core.Tests
{
    ...
    ///
    /// [2026-09-07 — 재료가 넷에서 **하나**가 됐다]
    /// 사람이 과거 판 코퍼스(`output.bak-*`)를 지우기로 결정해 `output/` 하나만 남았다.
    /// 그래서 「반쯤 설정」이라는 상태 자체가 없어졌고 `CorpusSetupGuardTests`도 함께
    /// 폐기했다 - 그 가드가 막던 것은 재료가 여럿일 때만 성립하는 실수였다.
    /// 무엇을 잃었는지는 docs/audit-reports/2026-09-07-과거판-코퍼스-폐기.md 에 있다.
    /// </summary>
    public static class CorpusSkip
    {
        public const string Reason =
            "output/ 코퍼스가 없어 건너뜀 - 워크트리라면 메인 저장소의 output/을 심링크한다\n" +
            "(.git/info/exclude에 output이 등록됨):\n" +
            "  ln -s <main>/output output\n" +
            "건너뜀 0이어야 코퍼스 단언이 전부 실제로 돈 것이다.";
    }
}
