...
namespace ReSet.Core.Tests
{
    ...
    /// 닫으려면 추출기에 서술형 한 갈래를 더해야 하는데, 그 추출기는
    /// <see cref="PlanStructureEnricher"/>와 <c>FindMissingErrorCodes</c>도 함께 먹으므로
    /// 이 회차의 승인 범위(검사 하나 추가) 밖이다. 실측해 둔 것: 서술형 패턴을
    /// <c>`(-?\d+)`(을|를|이|가|로|으로)?\s*(반환|설정|기록)</c>으로 조이면 명세 28편에서
    /// 더해지는 것이 `-15`·`0`·`-5`~`-8`뿐이고 잡음이 0이다.
    ///
