...
namespace ReSet.Core.Tests
{
    public class CacheManagerTests : IDisposable
    {
        ...
        public void UpdateCache_StampsTheCurrentFormatVersion()
        {
            ...
            //
            // 23: 무결과·NULL 귀착을 반환 계약이 말하게 했다.
            //     **전건 재분석을 의도한 것이 맞다.** `OverviewAndParameters` 갈래는 모든
            //     객체에 돌고 규칙·체크리스트가 늘었으므로 「영향 객체 0」 예외에 안 걸린다.
            //     옛 계약 산출물은 반환 절 이름이 여섯 가지로 갈려 있고 대입 행 보유 16 편
            //     중 14 편이 귀착을 안 적었다 - 다음 감사에 그대로 결함으로 남는다.
            //     CacheManager.cs 의 버전 23 주석 참고.
            //
            //     그 주석의 마지막 문단도 함께 읽어라 - 같은 날 계획서 프롬프트를 바꿨을
            //     때는 **올리지 않았다.** 그 경로가 이 캐시를 안 타기 때문이다.
            //     「프롬프트가 바뀌면 올린다」를 관할 확인 없이 적용하지 마라.
            //
            // 이 리터럴은 일부러 못 박혀 있다. 버전을 올리면 이 테스트가 깨지고, 깨진
            // 자리에서 "정말 전건 재분석을 의도했는가"를 한 번 더 묻게 된다.
            Assert.Equal(23, (int)entry["FormatVersion"]!);
        }
