...
namespace ReSet.Core.Tests
{
    public class CacheManagerTests : IDisposable
    {
        ...
        public void UpdateCache_StampsTheCurrentFormatVersion()
        {
            ...
            //     DML 범위 표의 문장 칸이 `SELECT n`·`IF n`을, 범위 칸이 `하위 질의`를
            //     담는다(CacheManager.cs의 버전 11 주석 참고). 프롬프트 입력이
            //     달라졌으므로 옛 엔트리를 재사용하면 산출물이 옛 재료 그대로 남고,
            //     이 회차가 넓힌 L1도 캐시 히트에서는 발동하지 않는다. 전건 재분석을
            //     의도한 것이 맞다.
            //
