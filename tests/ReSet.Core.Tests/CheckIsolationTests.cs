...
namespace ReSet.Core.Tests
{
    ...
    public class CheckIsolationTests
    {
        ...
        [Fact]
        public void Validate_WithoutExpectations_RunsExactlyTheFiveUnconditionalChecks()
        {
            ...
            //
            // 5: 2026-09-19 에 CheckReturnOutcomeStated 가 더해졌다. 그 검사도
            // expectations를 안 받는다 - 재료가 문서 자신(실행 의미 표)이다.
            var probe = new Probe();

            probe.Validate(MissingRequiredHeaders);

            Assert.Equal(5, probe.SeenCheckExpressions.Count);
        }
        ...
        public void ValidateConsolidated_RunsEveryCheckThroughSafeCheck()
        {
            ...
            // 2026-09-19 CheckValidationSetBlockRole(검증 세트 블록이 게이트냐 기록이냐)를 더해 14 → 15.
            Assert.Equal(15, probe.SeenCheckExpressions.Count);
