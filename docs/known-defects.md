...
| **반려된 판을 아무도 안 남긴다 — 검사가 잡은 것을 사후에 검증할 수 없다** | **2** | 두 파이프라인이 각각 「고쳐진 결과」만 남기고 **고쳐지기 전 판을 파괴한다.** ① `VerificationPipelineOrchestrator` — L1이 반려한 회차의 문서가 산출물 트리에도 로그에도 없다. 실측(2026-09-04): 판3의 mermaid 사건(발화 14건이 6회차를 태웠다)을 실물 픽스처로 옮기려 했으나 `Jobs/POQSettleBatch5R3`의 `docs`·`raw`(prompt-context 포함)·로그 어디에도 깨진 다이어그램이 없다 — 로그가 담는 것은 mmdc stderr의 **발췌 한 줄과 캐럿**뿐이다. ② `PrdDerivationService` — 교정 재호출이 성공하면 첫 초안을 버려, 검사가 **무엇을** 잡았는지의 증거가 그 자리에서 파괴되고 `Log.Information`의 **개수**만 남는다(다른 세션 실측, 원장 `b5f9ee2d`; 발화 규모를 보려고 별도 프로브를 만들어야 했던 이유가 이것이다). **②의 비용에 숫자가 있다(같은 세션 실측)**: 프로브로 첫 판정을 직접 잰 4편에서 발화가 **38건**(8·12·0·18)인데 최종 배너에 남은 것은 8편 통틀어 **6건**이다 — 분모가 달라 차 32를 그대로 「소실」로 읽을 수는 없지만, **첫 판정의 발화 대부분이 최종값에서 사라진다**는 것은 이 수치로 분명하다. 그 사라짐 때문에 스윕 결론이 하마터면 「검사가 실재 문서에서 거의 발화하지 않는다」가 될 뻔했고, 실제로는 그 발화가 **전량 오탐**이었다. 즉 반려본을 안 남기면 **발화 수를 성과로도 침묵으로도 잘못 읽게 된다.** **한 줄로 줄이면: 검사가 잡은 것을 사람이 나중에 검증하려면 반려된 판이 어딘가 남아야 한다.** 두 자리 다 「그 사건을 증언하는 검사」를 원리적으로 못 만들게 만든다 — 형태 픽스처가 최대다. **예외 하나가 실측으로 나왔다(2026-09-04, 판 F1)**: **골격 L1 반려본만은 남는다.** 골격 수리가 패치라서 프롬프트에 직전 골격을 통째로 싣고, 그 프롬프트가 `raw/prompt-context.md`에 기록되기 때문이다 — F1의 그 파일에 깨진 화살표(`DB--->App: 커밋 완료`)와 mmdc 진단이 원문 그대로 있다. **설계 의도가 아니라 패치 계약의 부수 효과이므로 우연이다** — 누가 골격 수리를 백지 재작성으로 바꾸면 이 보존은 조용히 사라진다. 그러므로 이 항목은 여전히 열려 있고(단계 하한 미달본·L2 반려본·PRD 첫 초안은 그대로 사라진다), 정책을 정할 때 이 우연을 **의도로 승격**할지도 함께 본다. **정책 결정이 선행한다**: 반려본을 **어디에** **어떤 이름으로** 남길 것인가(산출물 트리를 오염시키지 않으면서 · 회차 번호와 함께 · 언제 지울 것인가). 두 파이프라인이 같은 규약을 써야 하므로 한쪽만 정하면 갈라진다 |
