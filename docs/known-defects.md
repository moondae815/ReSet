...
| **파서가 파생 테이블의 비한정 컬럼을 물리 테이블에 과잉 귀속한다** | 2026-08-22 재감사의 미귀속 🟡 5건을 2026-08-23 9회차(③(b) 병합·v13 재생성 뒤)가 판정했다 — (나) 3건·(가)-`COMM_UPD` 1건은 **안 났고**, (가)-`EXCEPTION_PROC`의 `TPGProperty` `PLTID, ID`만 **같은 자리에 남아** 단위가 파서 귀속으로 확정했다(`object_definition.sql:60-61`의 비한정 `PLTID`·`ID`는 파생 X = `TSettleMst A`의 컬럼인데 `ReferencedColumnsPerTable`이 `TPGProperty`에 붙였고, 명세서는 계약대로 파서를 따랐다 — `2026-08-23-POQSettlePrco20-axisA.md` 4-1절 (G)). 같은 모양의 `COLLECTYMD` ⚪가 7회차에 있었다. 재생성으로는 닫히지 않는다 — 파서의 파생 테이블 컬럼 귀속 규칙을 고칠 자리다 | 파서를 고치면 `ReferencedColumnsPerTable`이 바뀌어 명세서 SELECT 대상 표가 달라진다(재생성 필요). 안 고치면 표가 계속 파서의 오귀속을 옮긴다 — 🟡 추적성이라 금액 영향은 없다 |
