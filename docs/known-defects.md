...
읽던 자리**가 드러나 아래 첫 건이 섰다. **그 뒤 축 B 재감사(2026-09-05)가 둘을 더 세웠다 — 셋 다 원인이 모델이 아니라 도구에 있어 재생성으로는 안 닫힌다.**
...
| **청크 Few-Shot 이 종료 조건 없는 헬퍼를 사양으로 내보낸다** | 1 | **모델이 아니라 도구다 — mermaid 정화기와 같은 모양의 세 번째 사례다.** `AiService.cs:2359-2373` 의 Few-Shot 이 `chunkRanges(bounds, params, size)` 라는 **저장소 어디에도 정의가 없는** 헬퍼로 청크 루프를 적고, 단계는 `SELECT MIN(<키>), MAX(<키>)` + `<키> >= @p_from AND <키> < @p_to` 만 남긴다. **그 앞 판에는 보장이 있었다** — `dd6041e8`(2026-08-29, 「Few-Shot 을 두 층으로 가른다」) 이전의 T-SQL Few-Shot 은 `WHILE @CurrentID <= @MaxID` 라는 종료 조건을 가졌다(`output.bak-batch1-preregen-20260904/Jobs/POQSettleBatch1/raw/prompt-context.md:84-94`). 그 커밋이 상한 산술의 보장을 잃었다. **실물**: POQSettleBatch1 재생성본의 다섯 단계(S02~S06)가 이 템플릿을 글자 그대로 채웠고 공통 규약 `agent/common/01-step-contract.md:75-103` 이 그것을 Job 규칙으로 못박는다 — **경계 술어의 모양은 모델 재량이 아니다.** **Critic 서술을 그대로 옮기지 마라**: 1차 리뷰가 「최대 키 행이 누락된다」고 적었으나 헬퍼 정의가 없으므로 누락은 **미명세이지 확정이 아니고**, 범위도 「S02~S06 전부」가 아니다(S02 는 5쌍 중 3쌍, S06 은 UPDATE 1·2 제외). **재생성으로는 안 닫힌다** — 고칠 자리는 Few-Shot 이다. 곁가지 둘도 같은 회차 실측이다: 후 S05 는 청크 키가 `CHAR(8)` 날짜인데 `size: 10000` 이고 경계는 `TExtraSettleIn.ReqYMD` 에서 뽑아 `TSettleMst.YMD` 에 걸며, 후 S03 의 경계 조회에는 원본 업무 필터가 없다(같은 규약 `:89` 와 정면으로 어긋난다). 출처: `docs/audit-reports/2026-09-05-POQSettleBatch1-축B-재감사.md` §5 |
