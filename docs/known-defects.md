...
| **축 A 재감사 잔여 🟡 5건의 귀속** | 2026-08-22 재감사 34건 중 **29건이 닫혔고**(①2+②4+④3+⑤5 = 14 · ③(a) 11 · ③(b)(`2026-08-23-axis-a-new-material`) **4건** — 08-23 재생성 실측으로 확인, `audit-defect-catalog.md` 4-2절 끝. 같은 재생성이 닫은 `UF_Get_CLComm4MobileCo:32`는 계획 밖 결함이라 34건에 없다), **나머지 5건은 어느 설계에도 귀속되지 않았다** — 전부 🟡이고 금액·행 집합 영향이 없다. 두 부류다. **(가) 파서 참조 컬럼을 명세서가 빠뜨림** 2건: `COMM_UPD`의 `SeperateAmt`·`SettleCurrency`·`ForeignSettleAmt`(UPDATE 15의 `TSettleMst B` 참조째), `EXCEPTION_PROC`의 `TPGProperty` `PLTID`·`ID`. 둘 다 파서가 파생 테이블의 비한정 컬럼을 물리 테이블에 **과잉 귀속**한 것일 가능성이 크다(같은 모양의 `COLLECTYMD` ⚪가 있다) — 그러면 명세서가 아니라 파서 쪽이다. **(나) 표가 관할하지 않는 산문의 사실 누락** 3건: `COMM4CLIENT4PARTIALCANCEL` 2차 조회의 다중 행 비결정성, `WORKDAY2`의 NULL 파라미터 둘을 한 주장으로 묶음(2026-08-23 C가 CRUD 설명 칸에서 닫은 것과 같은 모양이나 함수 갈래), `COMM_UPD` 파생 테이블 D의 `GROUP BY PLTID`. **③(b)가 병합된 뒤 축 A를 한 번 더 돌려** 이 미귀속 5건이 또 나는지 본다(③(b)가 닫은 4건 + 계획 밖 1건과 다른 5건이다) — 집합 술어·참조 함수 표가 독립 SELECT까지 넓어지면 (나)의 일부가 덮일 수 있고, (가)는 재생성 뒤 같은 자리가 남는지가 파서/명세서 판정의 가장 싼 근거다 | 또 나면 (가)는 파서의 파생 테이블 컬럼 귀속을 고칠 자리이고, (나)는 C와 같은 방식으로 산문이 쓸 자리를 없애거나 표로 올릴 자리다. 안 나면 ③(b)가 닫은 것이므로 이 행을 지운다 |
