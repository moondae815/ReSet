...
| **~~모델이 `sequenceDiagram`에 `--->`를 반복해 쓴다~~ → 원인 귀속이 틀렸다 · 해소(2026-09-04)** | 1 | **모델이 아니라 도구였다.** `MechanicalValidator.CleanseMermaidCode`의 화살표 보정(`-[^-]>` → `-->`)이 flowchart 전용인데 **모든** mermaid 블록에 적용돼, sequenceDiagram의 **유효한** `-->>`(점선+화살촉)를 `--->`로 바꿨다 — 그 토큰은 sequenceDiagram 문법에 없다(flowchart에서는 대시를 늘린 긴 링크라 유효하다). `->>`도 `-->`가 되어 화살촉을 잃었다. 즉 **정화기가 멀쩡한 다이어그램을 부순 뒤 그것을 mmdc에 넘겨** 파스 오류를 받았고, 수리 프롬프트는 모델에게 **모델이 쓰지 않은 줄**을 고치라고 했다. 모델이 다시 옳게 쓰면 정화기가 또 부쉈다 — 판 F2가 그렇게 세 번 연속(패치·패치·백지) 실패해 L1 예산을 소진하고 채점에 한 번도 못 갔고, 판 R3는 6회차를 통째로 태웠다. **배송된 문서도 깨져 있었다** — 파이프라인이 정화본을 산출물로 채택하므로(`consolidatedPlan = l1Result.CleansedMarkdown`) 두 트리에 이미 `--->`를 담은 문서가 6편 있다. **고쳤다**: 정화를 flowchart 계열(`flowchart`·`graph`)로 한정하고, 종류를 모르면 건너뛴다 — 실측에서 **적용은 파괴였고 건너뜀은 무해했다**. 통제군 sequenceDiagram 13편에 옛 정화를 걸면 3편이 새로 깨지고 **깨지지 않은 변경은 하나도 없었다**. `stateDiagram` 27편도 같은 오발을 받았다(노드 셰이프 정규식이 `[*]`에 걸려 공백을 지운다, 11편). **교훈**: 로그의 오류 메시지만 보고 원인을 모델에 귀속했다가 실측으로 뒤집혔다(`measure-dont-reason`). `MechanicalValidator` 주석이 2026-08-29에 이미 `Settle--->Batch`를 관측하고도 「모델이 그렇게 쓴다」로 읽고 지나간 자리다. **남는 함정**: 이 수정 뒤 mermaid L1 발화가 줄 텐데, 그 침묵은 「모델이 잘 쓰게 됐다」가 아니라 「도구가 부수기를 멈췄다」다 |
