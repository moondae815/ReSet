...
- **캐시 17 인상 전 선결 조건 — I2·I3·재매핑 위험을 한 자리에 모은다(2026-08-25,
  ...
  **세 개의 앞선 명시적 판정과 정면으로 부딪힌다** — 감추지 않고 여기
  적는다.
  ...
  **왜 이번 판정이 그 셋을 뒤집는가.** 세 판정 중 소스를 직접 인용한
  것은 Task 22뿐인데, 그 인용 자체가 `CardCost AS (...)`로 **CTE 본문을
  ...
  (`output/Procedures/dbo.UP_UTIL_SETTLE_EXCEPTION_PROC/docs/Spec.md:320-326`)의
