...
| 항목 | 반복 | 요지 |
|---|---|---|
| **골격·문서 전역 L1 위반에 지목 재생성 경로가 없다** | 1 | **반복 실행 3판이 재현성 저하의 최대 원인으로 지목한 자리다.** 지목 재생성은 **단계 섹션만** 연다(`VerificationPipelineOrchestrator`의 `pendingDefectiveSteps` → `GenerateBySplitAsync`). 골격(H2 「Mermaid 기반 통합 흐름도」·공통 규약 절)에 있는 위반은 그 경로로 닿지 않고, 귀속 실패의 폴백인 전량 재생성도 실측에서 골격을 다시 만들지 않았다. **판3 실측**: L1 발화 7회가 전부 골격 mermaid였고 14건이 모두 같은 오류(`Parse error on line 8` · `LOCK--->ORCH`)다. 골격 생성 호출 2회는 **둘 다 첫 실패 이전**(19:35·20:20 대 20:35~22:14)이라, 깨진 다이어그램이 글자 하나 안 바뀐 채 6회차를 전부 태웠다. **판2**도 같은 구조다 — 공통 절의 `SqlSideControlFlow`가 귀속에 실패해 채점 예산을 먹었다. 결과: 채점 못한 회차 판2 3 · 판3 5, 최종 점수 82→80→72. **처방은 `MaxL1RepairAttempts` 인상이 아니다**(예산이 아니라 도달 범위의 문제다). **설계 결정이 하나 앞선다**: 골격만 다시 만들 때 이미 만들어진 단계 섹션과의 정합(단계 목록·오류코드 표)을 어떻게 지킬 것인가. 출처: `2026-08-30-batch-plan-loop-reproducibility-design.md` §7-2 「결과」·§7-5 |
| **`MermaidCliError`는 귀속 어휘를 만들 수 없다** | 1 | `MechanicalValidator.ViolationLexemes`는 메시지의 **백틱 토큰**만 훑는데, 이 유형의 메시지는 mermaid 컴파일 로그라 백틱이 하나도 없다. 그래서 항상 빈 어휘 → 귀속 실패 → 전량 재생성 → **채점 예산 1회**다. 위 항목이 골격 귀속을 만들면 유형 기반 하드 귀속(`ErrorType.BatchRunRowNeverCreated`가 쓰는 갈래)으로 함께 닫히므로, 별건으로 고치기 전에 그쪽 설계를 먼저 정하라. 출처: 같은 문서 §7-2 |
