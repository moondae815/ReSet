...
| **검사 B 의 조인 키 접힘이 무엇을 덮는지 안 쟀다** | 1 | `MechanicalValidator.cs:7811-7829` 는 조인 파트너가 CTE·파생 테이블이면(`HasOpaqueJoinSource`) **조인 키 칸 대조를 통째로 접는다.** 근거(태스크 22)는 유효하나 **비용을 잰 적이 없다.** 축 B 재감사(2026-09-05)가 POQSettleBatch1 재생성본에서 원본 DDL 로 확인한 조인 키 회귀 **둘**이 정확히 그 모양이다 — `INSERT … SELECT … FROM (SELECT … LEFT JOIN …) X` 의 파생 테이블 안 조인이고, ① 요율 조인이 `A.OrgYMD` 에서 `A.YMD` 로(`steps/S05.md:193,195`, 원본 DDL 194-195 · 전 산출물은 옳았다), ② `TClientCMRate` 조인에서 `MallID` 소실(`steps/S06.md:214`, 원본 DDL 166 · 전 산출물은 옳았다)이다. 같은 스윕(`2026-09-05-step-sweep.md`)에서 **Batch1 은 전 검사 0 발화**다(측정 쌍 0 인 Job 은 Proc20·Proc5 뿐이므로 측정됐고 침묵했다). **접힘 때문인지 앵커 미해결 때문인지는 안 쟀다** — `verify-by-removal` 대로 접힘을 없앴을 때 코퍼스에서 무엇이 새로 발화하는지 세는 것이 첫 걸음이고, 그 답이 나오기 전에 새 검사를 만들면 오탐만 는다. 세 번째 회귀(파생 테이블의 중복 투영, `steps/S05.md:170` 대 `:173`)는 **그 계열의 검사가 아예 없다** |
