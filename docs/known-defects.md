...
| **재생성이 만든 회귀 셋이 배송본에 실려 있다 — 손으로 못 고친다** | 1 | 축 B 재감사(2026-09-05)가 **원본 DDL 로 확인**한 셋이다. **전 산출물에서는 셋 다 옳았다** — 재생성이 만들었다. ① **요율 조인 키 `A.OrgYMD` → `A.YMD`**(`output/Jobs/POQSettleBatch1/agent/steps/S05.md:193,195`). 원본 DDL 194-195 는 `ON A.OrgYMD = B.YMD` / `ON A.OrgYMD = C.YMD` 이고 명세서 조인 키 칸도 `OrgYMD` 를 싣는다(`UP_UTIL_SETTLE_INS_EXTRA/docs/Spec.md:167`). 전 `S09.md:228,232` 는 옳았다. 요율을 **원거래일이 아니라 요청일**로 조회하므로 `CLComm`·`PGComm` **금액이 달라진다**. ② **파생 테이블 `X` 의 중복 투영**(`S05.md:170` 대 `:173`) — `ISNULL(B.incVTax,0) AS PGINCVTAX` 와 `B.incVTax AS PGIncVTax` 를 함께 낸다. T-SQL 식별자는 대소문자를 안 가려 **중복 컬럼명**이고, 깨지지 않더라도 `ISNULL` 없는 쪽을 집으면 `NULL` 이 전파돼 `CLVT`/`PGVT` 가 `NULL` 이 된다. 전 `S09.md:183-184` 는 한 쌍뿐이었다. ③ **`TClientCMRate` 조인에서 `MallID` 소실**(`S06.md:214`). 원본 DDL 166 은 세 키를 다 갖고 명세서 조인 키 칸도 `MALLID` 를 싣는다(`UP_UTIL_SETTLE_INS_EXTRA4PLCARD/docs/Spec.md:47`). 전 `S10.md:205-207` 은 옳았다. 같은 `ClientID+PGName` 에 `MallID` 가 둘 이상이면 **행이 증식한다**. **고치는 길은 손편집이 아니라 「검사 → 재생성」이다** — `output/Jobs/` 는 생성물이라 다음 재생성이 덮는다. **[2026-09-05 갱신] ①③ 은 이제 검사 B 가 잡는다** — 접힘을 `ReadsOwnTarget` 으로 좁혀(`83bdf03f`) 두 좌표가 발화하고, 그 발화는 `SuggestedPromptFix` 를 타므로 **다음 Batch1 재생성이 그 두 조인 키를 되살리라는 지시를 받는다.** **② 는 여전히 그 계열의 검사가 아예 없다** — 파생 테이블이 같은 컬럼을 두 번 투영하는 것을 보는 검사가 없고, 그래서 ② 만은 다음 재생성이 우연히 고치지 않는 한 남는다. **나머지 9 건은 여기 옮기지 않았다** — 재감사 §4 에 있고, 단위마다 상한을 건 표본이라 전건으로 올리면 분모를 잘못 만든다. 그 표에는 등급 🟠 다섯(대사 방어 소실 · 정산주기 결합 이동 · 단일 트랜잭션 분할 · 대체 검증의 항상 불일치 · 기정산 가드 약화)이 함께 있다. 출처: `docs/audit-reports/2026-09-05-POQSettleBatch1-축B-재감사.md` §4 |
