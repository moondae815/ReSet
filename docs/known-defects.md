...
| **모델이 `sequenceDiagram`에 `--->`를 반복해 쓴다 — 재생성이 못 고친다** | 1 | `--->`는 flowchart에는 있고 sequenceDiagram에는 **없다**. **4회 관측**: `Settle--->Batch`(2026-08-29 통제군) · `LOCK--->ORCH`(판 R3) · `DB--->App`(판 F1) · `CKPT--->ORCH`·`DB--->ORCH`(판 F2). **판 F2가 결정적 증거다** — 골격 귀속이 세 번 다 정확히 발동해(패치 → 패치 → 백지 에스컬레이션) 골격을 세 번 다시 만들었는데 **세 번 다 같은 실수를 냈다.** 결과는 L1 예산 소진 · 채점 0회 · 구제할 후보 없음 · 배너 붙은 문서 출고다. 즉 이것은 도달 범위 문제가 아니라 **수리 성공률** 문제이고, `MaxL1RepairAttempts` 인상은 같은 실수를 더 태울 뿐이다(§7-5의 논리 그대로). 「돌려 봐야 아는 것」의 「유령 테이블 결함이 재생성으로 고쳐지는지」가 걱정한 부류의 **실물 사례**이며, 답은 「못 고친다」였다. **처방 후보**: `MechanicalValidator.CleanseMermaidCode`가 이미 mermaid를 정화하는데 화살표 **문법**은 안 건드린다(라벨만 정규화한다) — sequenceDiagram 안의 `--->`는 언제나 틀리므로 결정적으로 치환하면 이 부류가 **재생성 0회·예산 0**으로 닫힌다. 부차적으로 수리 피드백이 mmdc stderr를 통째로 실어 **신호 3줄(문제 줄·캐럿·Expecting 목록)이 puppeteer 스택 트레이스 12줄에 묻힌다** — 그것도 함께 좁힌다. **함정**: 정화기가 고치면 L1이 조용해지는데 그 침묵을 「모델이 배웠다」로 읽으면 안 된다. 치환은 문서를 고치는 것이지 습관을 고치는 것이 아니므로, 치환 **건수와 함께 무엇을 치환했는지**를 남겨야 이 결함이 계속 보인다(`check-oracle-provenance`). 개수만 남기면 부족하다는 것도 실측이 있다 — 위 PRD 자리가 「결함 1건」처럼 개수만 적어, 그것이 오탐인지 진짜인지 가르려고 초안을 다시 뽑는 프로브를 따로 만들어야 했다. 지금은 치환 종류가 `--->` 하나라 덜 아프지만 정화기 항목이 늘면 같은 자리에 선다 |
