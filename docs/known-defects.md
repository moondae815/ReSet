...
정책 결정도 실측도 아니고 코드 변경만 남았다. 아래 표의 취소선 둘은 기록이다 — 둘 다
**두 오라클이 서로 반대를 말하는** 부류였고, 그 상태에서 검사나 지표를 넓히면 오탐만
늘어난다는 것을 보여 준 자리다(`check-oracle-provenance`).
...
| **`CorpusSetupGuardTests`가 코퍼스 재료 셋 중 둘만 지킨다** | 1 | 가드는 `output/`과 `CorpusPaths.PriorEdition` 둘만 본다. 셋째 `output.bak-stage4-control-20260828/`은 가드 **밖**이라 빠뜨려도 빨간불이 없고 `ProcedureClosureCorpusTests`·`LegacyErrorCodeInventionCorpusTests`·`BatchRunLogReaderTests`가 **조용히 건너뛴다**. AGENTS.md 워크트리 코퍼스 절의 「「반쯤」 상태는 가드가 막는다」는 앞의 둘에 대해서만 참이다. 2026-09-03에 두 세션이 각각 이 자리에서 틀린 안내를 주고받았다(한쪽은 「재료는 둘」, 다른 쪽은 「셋째가 빠지면 가드가 막는다」 — 둘 다 틀렸다). 가드를 셋으로 넓히면 닫힌다 |
