...
| 항목 | 반복 | 요지 |
|---|---|---|
| **`DmlScopeExtractor`의 문장↔코드 귀속이 DML 배치 형태를 탄다** | 2 | DELETE 4개(`-1`~`-4`)가 먼저 서고 INSERT 4개(`-5`~`-8`)가 뒤에 몰린 원본에서 허용 집합이 `0`·`-1`~`-4`로 확정돼, L1 `CheckLegacyStepErrorCodeInvention`이 **참인 `-5`를 발명으로 고발**한다. DELETE·INSERT가 번갈아 서는 원본에서는 옳게 작동한다. **모델이 L1을 만족시키려 `-5`~`-8`을 `-1`~`-4`로 붕괴시키자 Critic이 그 붕괴를 채택본의 필수 수정 1번으로 지목했다** — 루프가 그 자리에 갇혀 예외 축이 회차 내내 5~7에 머물렀고 채택본이 품질 미달 배너를 달았다. 2026-08-30 Batch5 대조 실행의 단계 재생성 22회 중 **10회(45%)**가 이 한 줄이다. 자리: `DmlScopeExtractor.RecordErrorCode`·`BuildStatementOrdinals`. **재료를 고치기 전에 이 검사를 넓히지 마라.** 출처: `2026-08-30-batch-plan-loop-reproducibility-design.md` §7-8 |
| **`BatchRunLogReader`의 단조성 계수기가 다른 궤적을 센다** | 2 | 설계가 만들려는 것은 **채택본 궤적**의 단조성인데 `CountMonotonicityViolations`는 **Critic 원점수 궤적**의 하락 회차를 센다. 회귀 롤백은 문서 상태를 되돌릴 뿐 다음 회차의 점수를 올려 주지 않으므로, 설계가 완벽히 작동해도 이 값은 0이 되지 않는다 — 그래서 대조 실행의 이 지표가 「판정 불가」로 끝났다. 기준선에서 두 궤적이 같은 값(2)을 낸 것은 그때 **롤백이 없어서** 둘이 같았던 우연이다. 같은 클래스의 `UnscoredAttempts`가 이미 겪은 사고와 동형이고, 그 경위는 클래스 주석에 있다. 출처: 같은 문서 §7-6 |
