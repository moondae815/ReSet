...
| **반려된 판을 아무도 안 남긴다 — 검사가 잡은 것을 사후에 검증할 수 없다** | **2** | 두 파이프라인이 각각 「고쳐진 결과」만 남기고 **고쳐지기 전 판을 파괴한다.** ① `VerificationPipelineOrchestrator` — L1이 반려한 회차의 문서가 산출물 트리에도 로그에도 없다. 실측(2026-09-04): 판3의 mermaid 사건(발화 14건이 6회차를 태웠다)을 실물 픽스처로 옮기려 했으나 `Jobs/POQSettleBatch5R3`의 `docs`·`raw`(prompt-context 포함)·로그 어디에도 깨진 다이어그램이 없다 — 로그가 담는 것은 mmdc stderr의 **발췌 한 줄과 캐럿**뿐이다. ② `PrdDerivationService` — 교정 재호출이 성공하면 첫 초안을 버려, 검사가 **무엇을** 잡았는지의 증거가 그 자리에서 파괴되고 `Log.Information`의 **개수**만 남는다(다른 세션 실측, 원장 `b5f9ee2d`; 발화 규모를 보려고 별도 프로브를 만들어야 했던 이유가 이것이다). **②의 비용은 이렇게 갈린다(같은 세션 실측, 2026-09-04 정정본)**. **메커니즘은 코드로 확정**이다 — 재호출이 성공하면 첫 초안을 버린다. **직접 확인된 소실은 1건**이다(EXCEPTION_PROC, 같은 실행에서 첫 판정 1 → 최종 0). **규모는 미측정이다** — 프로브가 첫 판정을 잰 5편의 **56건**과 배너가 보고한 8편의 **6건**은 **서로 다른 생성**이고 문서 목록도 겹치지 않아, 두 수의 차를 소실량으로 읽을 수 없다(같은 EXCEPTION_PROC이 스윕에서 1, 프로브에서 8을 냈다 — 생성이 다르면 첫 판정 수 자체가 달라진다). 그래도 대가는 실재한다: 그 사라짐 때문에 스윕 결론이 하마터면 「검사가 실재 문서에서 거의 발화하지 않는다」가 될 뻔했고, 실제로 그 발화는 **전량 오탐**이었다. 즉 반려본을 안 남기면 **발화 수를 성과로도 침묵으로도 잘못 읽게 된다.** **한 줄로 줄이면: 검사가 잡은 것을 사람이 나중에 검증하려면 반려된 판이 어딘가 남아야 한다.** 두 자리 다 「그 사건을 증언하는 검사」를 원리적으로 못 만들게 만든다 — 형태 픽스처가 최대다. **예외 하나가 실측으로 나왔다(2026-09-04, 판 F1)**: **골격 L1 반려본만은 남는다.** 골격 수리가 패치라서 프롬프트에 직전 골격을 통째로 싣고, 그 프롬프트가 `raw/prompt-context.md`에 기록되기 때문이다 — F1의 그 파일에 깨진 화살표(`DB--->App: 커밋 완료`)와 mmdc 진단이 원문 그대로 있다. **설계 의도가 아니라 패치 계약의 부수 효과이므로 우연이다** — 누가 골격 수리를 백지 재작성으로 바꾸면 이 보존은 조용히 사라진다. 그러므로 이 항목은 여전히 열려 있고(단계 하한 미달본·L2 반려본·PRD 첫 초안은 그대로 사라진다), 정책을 정할 때 이 우연을 **의도로 승격**할지도 함께 본다. **정책 결정이 선행한다**: 반려본을 **어디에** **어떤 이름으로** 남길 것인가(산출물 트리를 오염시키지 않으면서 · 회차 번호와 함께 · 언제 지울 것인가). 두 파이프라인이 같은 규약을 써야 하므로 한쪽만 정하면 갈라진다 |
| **B4 — 축 B 나머지 10건** | 1 | 레거시 불변식인지 여부를 `Spec.md`로 판정해야 판단이 선다. **대부분의 Job에서 아직 축 B를 돌리면 안 된다** — `agent/` 번들이 재생성 전 명세서로 만든 것이라, 현행 `Spec.md`와 대조하면 세대 차이가 결함으로 잡힌다. **날짜를 2026-09-05에 다시 쟀다**: 현행 `Spec.md`는 08-22가 아니라 **08-28**이고(`output/Procedures` 14편 전건 같은 날), 번들 쪽은 `POQSettlePrco20` 08-19 20:50 · `POQSettleProc19` 08-19 17:58 · `POQSettleProc18` 08-19 15:26 순으로 그대로다 — **세대 차는 기록보다 넓다.** **다만 전제가 한 자리에서 깨졌다**: `output/Jobs/POQSettleBatch1/agent`는 51개 파일이 **2026-09-04 20:06에 재생성**됐다(로드맵 5 단계 번들 하네스 `d947510c`). 그러므로 차단은 **번들이 낡은 Job에만** 남고, Batch1은 이 사유로 막지 않는다 — 적힌 날짜만 보고 「이미 지났다」로도, 「전부 막혔다」로도 읽지 마라. 나머지는 명세서 결함을 고치고 Job 설계 문서를 다시 만든 뒤가 순서다. 7회차가 축 B로 넘긴 입력 셋은 카탈로그 §5에 있다 |

### 고쳐야 하는 것 — 자리가 좁혀졌다

정책 결정도 실측도 아니고 **손을 대는 것만** 남았다. 2026-09-05에 이 절이 한 번 비었다 —
여섯 건이 전부 닫혀 「닫힌 것」으로 내려갔고, 그때 **원인이 닫혔다고 결과까지 닫힌 것으로
읽던 자리**가 드러나 아래 첫 건이 섰다. **그 뒤 축 B 재감사(2026-09-05)가 셋을 더 세웠다** — 그중 청크 Few-Shot 은 같은 날 원인을 닫았으므로(`7feb3c54`) **그중 하나는 같은 날 닫혀 아래로 내려갔고, 남은 셋 중 둘이 「원인은 닫혔고 배송본은 안 닫혔다」는 같은 모양이다.**

| 항목 | 반복 | 요지 |
|---|---|---|
| **정화기가 부순 다이어그램이 배송본에 남아 있다** | 1 | **닫힌 것은 원인이고 배송본은 안 닫혔다.** `CleanseMermaidCode`를 flowchart 계열로 한정해 2026-09-04에 원인을 막았지만, 그 전에 나간 문서는 아무도 안 고친다. **실측(2026-09-05, 저장소의 산출물 트리 넷 전수)**: `sequenceDiagram` 블록 **안**에 `--->`를 담은 문서 **8편**이다. ① `output/Jobs/POQSettleProc10/`의 `docs/BatchMigrationPlan.md`와 `agent/common/00-architecture.md`(각 **14건**) · ② `output.bak-2026-08-22/`의 같은 Job 같은 두 파일(각 14건, ①의 백업본) · ③ 통제군 `output.bak-stage4-control-20260828/Jobs/POQSettleBatch4/`의 같은 두 파일(각 2건) · ④ 같은 트리 `POQSettleBatch5F2/`의 같은 두 파일(각 5건). `output.bak-batch1-preregen-20260904/`은 **0편**이다. **앞선 보고의 「6편」은 트리 둘만 보고 낸 값이었다** — 트리를 세는 범위를 안 적으면 같은 실측이 다른 수를 낸다. **세지 않은 것**: 로그 파일의 `--->`(진단 발췌다)와 `raw/prompt-context.md`의 `--->`(펜스 밖이고 반려본의 기록이라 오히려 남아야 한다 — 위 「반려된 판」 항목의 예외가 그것이다). **고칠 수 있는 것은 ①의 두 편뿐이다.** ②는 백업본이고 ③④는 **재생성하면 안 된다** — 얼어붙은 입력이고(`control-run-harness`), F2는 **정화기가 부수던 시절의 표본 자체**이며 Batch4는 코퍼스 재료다. 고치는 것이 아니라 **그 상태인 이유를 아는 것**이 이 셋의 값이다. 이 구분을 안 적으면 다음 사람이 「깨진 문서를 고친다」며 표본을 태운다. 경위는 「자리가 좁혀졌다가 닫힌 것」의 mermaid 항목에 있다 — 그 항목 머리가 「해소」라 본문의 이 사실이 지나쳐진다 |
| **청크 Few-Shot 의 종료 조건 — 원인은 닫혔고 배송본은 안 닫혔다** | 1 | **모델이 아니라 도구였다 — mermaid 정화기와 같은 모양의 세 번째 사례다.** `AiService.cs` 의 Few-Shot 이 `chunkRanges(bounds, params, size)` 라는 **저장소 어디에도 정의가 없는** 헬퍼로 청크 루프를 적어, 단계에는 `SELECT MIN(<키>), MAX(<키>)` + `<키> >= @p_from AND <키> < @p_to` 만 남았다. **그 앞 판에는 보장이 있었다** — `dd6041e8`(2026-08-29) 이전의 T-SQL Few-Shot 은 `WHILE @CurrentID <= @MaxID` 라는 종료 조건을 가졌다(`output.bak-batch1-preregen-20260904/Jobs/POQSettleBatch1/raw/prompt-context.md:84-94`). 그 커밋이 루프를 앱 층으로 옮기면서 상한 산술의 보장을 함께 잃었다. **[2026-09-05 원인 닫음 `7feb3c54`]** 종료 조건을 **예시 본문에** 되돌렸다(`lo, hi = queryRow(...)` → `WHILE from <= hi:` → `to = from + 10000` → `from = to`). **주석이 아니라 본문인 이유가 요점이다** — 생성된 단계와 공통 규약은 예시를 베끼지 프롬프트 주석을 베끼지 않는다. `dd6041e8` 의 「바깥은 언어 중립 의사코드, 안쪽은 앱이 보내는 문장」 분리는 그대로 두었다. 키 타입 한 줄도 함께 넣었다(업무 키가 정수가 아니면 — `CHAR(8)` 날짜 같은 것 — 식별자 컬럼으로 청크하고 업무 필터는 WHERE 에 남긴다. Batch1 S05 가 `ReqYMD CHAR(8)` 에 `size: 10000` 을 걸었다). 기존 테스트 둘이 `FOR EACH chunk` 토큰에 기대고 있어 **토큰이 아니라 의도를 옮겼다.** **⚠ 남은 것 — 배송본이다.** `output/Jobs/POQSettleBatch1/` 의 **S02~S06 다섯 단계**와 **`agent/common/01-step-contract.md:75-103`** 이 옛 템플릿을 글자 그대로 담은 채 있다. **다음 재생성이 닫는다 — 손으로 고치지 마라**(생성물이라 덮인다). **Critic 서술을 그대로 옮기지 마라**: 1차 리뷰가 「최대 키 행이 누락된다」고 적었으나 헬퍼 정의가 없었으므로 누락은 **미명세이지 확정이 아니고**, 범위도 「S02~S06 전부」가 아니다(S02 는 5쌍 중 3쌍, S06 은 UPDATE 1·2 제외). **함께 보되 섞지 말 것**: 후 S03 의 경계 조회에 원본 업무 필터가 없는 것은 Few-Shot 이 **이미 지시하고 있는데도** 안 지킨 것이라 도구 결함이 아니라 **모델 미준수**다 — 검사·강제 쪽 몫이다. 출처: `docs/audit-reports/2026-09-05-POQSettleBatch1-축B-재감사.md` §5 |
| **재생성이 만든 회귀 셋이 배송본에 실려 있다 — 손으로 못 고친다** | 1 | 축 B 재감사(2026-09-05)가 **원본 DDL 로 확인**한 셋이다. **전 산출물에서는 셋 다 옳았다** — 재생성이 만들었다. ① **요율 조인 키 `A.OrgYMD` → `A.YMD`**(`output/Jobs/POQSettleBatch1/agent/steps/S05.md:193,195`). 원본 DDL 194-195 는 `ON A.OrgYMD = B.YMD` / `ON A.OrgYMD = C.YMD` 이고 명세서 조인 키 칸도 `OrgYMD` 를 싣는다(`UP_UTIL_SETTLE_INS_EXTRA/docs/Spec.md:167`). 전 `S09.md:228,232` 는 옳았다. 요율을 **원거래일이 아니라 요청일**로 조회하므로 `CLComm`·`PGComm` **금액이 달라진다**. ② **파생 테이블 `X` 의 중복 투영**(`S05.md:170` 대 `:173`) — `ISNULL(B.incVTax,0) AS PGINCVTAX` 와 `B.incVTax AS PGIncVTax` 를 함께 낸다. T-SQL 식별자는 대소문자를 안 가려 **중복 컬럼명**이고, 깨지지 않더라도 `ISNULL` 없는 쪽을 집으면 `NULL` 이 전파돼 `CLVT`/`PGVT` 가 `NULL` 이 된다. 전 `S09.md:183-184` 는 한 쌍뿐이었다. ③ **`TClientCMRate` 조인에서 `MallID` 소실**(`S06.md:214`). 원본 DDL 166 은 세 키를 다 갖고 명세서 조인 키 칸도 `MALLID` 를 싣는다(`UP_UTIL_SETTLE_INS_EXTRA4PLCARD/docs/Spec.md:47`). 전 `S10.md:205-207` 은 옳았다. 같은 `ClientID+PGName` 에 `MallID` 가 둘 이상이면 **행이 증식한다**. **고치는 길은 손편집이 아니라 「검사 → 재생성」이다** — `output/Jobs/` 는 생성물이라 다음 재생성이 덮는다. **[2026-09-05 갱신] ①③ 은 이제 검사 B 가 잡는다** — 접힘을 `ReadsOwnTarget` 으로 좁혀(`83bdf03f`) 두 좌표가 발화하고, 그 발화는 `SuggestedPromptFix` 를 타므로 **다음 Batch1 재생성이 그 두 조인 키를 되살리라는 지시를 받는다.** **[2026-09-05 갱신 2] ② 도 이제 검사가 잡는다** — `CheckDuplicateProjectionNames`(`1f7ea018`)가 이름 붙은 테이블 식의 중복 출력 컬럼을 본다. **오라클이 없는 검사다**(명세서도 원본 DDL 도 안 본다 — T-SQL 이 파생 테이블·CTE 에 중복 컬럼명을 금지하므로 SQL 자체가 서지 않는 것을 본다). 코퍼스 전수에서 발화는 정확히 2 건이고 둘 다 `POQSettleBatch1/S05` 의 파생 테이블 X(`PGINCVTAX`·`CLINCVTAX`)다. **그래서 회귀 셋이 전부 검사에 걸린다 — 남은 것은 산출물이고 다음 재생성이 닫는다.** **함정 하나를 실측으로 남겼다**: 처음에 `CleanedSqlFences` 를 썼더니 단위 테스트는 통과하는데 코퍼스 발화가 0 이었다. 그 헬퍼는 주석·문자열 리터럴을 지운 정화본을 내는데 **정화본은 파서에 먹일 수 없다**(실물로 재니 원문 파스 오류 0 대 문자열 지운 판 11). **구문을 보는 검사는 원문을 파싱하라.** **나머지 9 건은 여기 옮기지 않았다** — 재감사 §4 에 있고, 단위마다 상한을 건 표본이라 전건으로 올리면 분모를 잘못 만든다. **[2026-09-05] 그 9 중 셋(N8·N9·N11)은 `docs/audit-reports/2026-09-05-축B-잔여결함-분류.md` §5 가 닫았다** — N8·N9 는 **결함이 아니고**(N8 은 전제가 거짓이었다: `TSettleMst.PLTID` 가 `IsNullable=False` 라 `IS NOT NULL` 이 `EXISTS` 와 동치다), N11 은 **상류 계약이 그렇게 정했다**. **뒤의 부류는 그 상류가 바뀌면 다시 열어야 한다 — 결함이 없어서 닫은 것이 아니다.** 그 문서가 잔여 22 건 전체의 방침도 갖는다(K¹ 4 · K² 6 · P 7 · N 5 · U 0). 그 표에는 등급 🟠 다섯(대사 방어 소실 · 정산주기 결합 이동 · 단일 트랜잭션 분할 · 대체 검증의 항상 불일치 · 기정산 가드 약화)이 함께 있다. 출처: `docs/audit-reports/2026-09-05-POQSettleBatch1-축B-재감사.md` §4 |
...
| **재시도 중 축 간 역행이 다시 커지는지** | 2026-08-23 재측정에서 한 축을 고치면 다른 축이 2~4점 떨어지는 역행이 실재했다(`UP_UTIL_SETTLE_INS` 8/21: 시도 2에서 CRUD 7로 고치자 시도 3에서 정합 9→7). Actor가 백지에서 다시 쓰기 때문이다. **2026-08-30 Batch5 대조 실행이 첫 실측을 줬다** — Critic 원점수는 여전히 진동하나(78→76→74→90→88→84, 폭 16점), 단계 동결(`StepFreezeState`)과 회귀 롤백이 **채택 궤적을 단조로** 만들었다(78→78→78→78→90→90→90, 하락 0). 역행 자체는 남아 있고 흡수될 뿐이므로 관측을 계속한다 | 폭이 다시 20점대로 오르거나 역행이 수렴하지 않으면 「시도 간 진동 억제」를 되살린다 — 그때는 `IAiService`에 이전 명세서를 넘기는 방향이 다시 정당하다. 채택 궤적이 아니라 **원점수** 궤적으로 판정할 것 — 근거는 「닫힌 것 → 자리가 좁혀졌다가 닫힌 것」의 `BatchRunLogReader` 항목에 있다(옛 단조성 계수기가 센 것이 원점수 궤적이었다) |
| **`AdoptedIsBest`가 게이트를 통과해 끝난 판에서 거짓이 되는지** | 통과 경로에는 채택 줄이 없고 **마지막 회차**가 채택된다 — 4차 90 뒤 5차 85가 통과하면 85가 배송된다. 2026-08-30 대조 실행은 통과로 끝나지 않아 이 갈래를 밟지 않았고, 실재 여부는 다음 판이 답한다(`2026-08-30-batch-plan-loop-reproducibility-design.md` §7-6) | 실재하면 고칠 자리는 판독기가 아니라 **채택 규칙**이다 — 조기 종료가 궤적의 최고점을 버리고 나중 회차를 배송한다. 지표가 거짓을 내는 것이 아니라 파이프라인이 진짜로 최고점을 버리는 것이므로, `AdoptedIsBest`의 거짓을 판독기 결함으로 읽으면 안 된다 |
...
- **캐시 17 인상 전 선결 조건 — I2·I3·재매핑 위험을 한 자리에 모은다(2026-08-25,
  ...
  **(5-3-7) 검사 D 가 통째로 꺼졌다 — 강제 없는 재료에 기댄 검사 (2026-08-28).**
  ...
  > (`output.bak-cache17-20260827`)을 근거로 썼고, 그 판의
  > `SUMMARY_EXTRA/docs/Spec.md:76`에 전용 헤딩 없는 「내부 변수 명칭」 표가
  > 실제로 있다(리뷰어가 직접 열어 확인 — 더 옛 판 `output.bak-2026-08-22`
  > 에서는 같은 자리에 `### 내부 변수 목록`이라는 헤딩이 있었다).
  ...
  >
  > **낡은 주석 기록(고치지 않음).** `SpecStatementFactsExtractor.cs`의
  > 「알려진 한계 6번」 주석이 `output/Procedures/dbo.UP_UTIL_SETTLE_SUMMARY_EXTRA
  > /docs/Spec.md:75`를 가리키는데, 현재 판의 그 파일 그 줄에는 그 표가
  > 없다 — 주석이 현재 판에 대해 낡았다. 이 주석은 이 태스크의 쓰기 범위
  > 밖이라 고치지 않고 여기 기록만 한다.
  >
  ...
  > 대조할 "명세서 쪽"이 애초에 없다.
  >
  > **(1)번은 자기 번호를 받았다 — (5-3-9) 참고 (2026-08-29, Fix Round 2 최종 리뷰
  > Important 6).** 여기 각주로만 있으면 결함 목록을 훑는 다음 사람이 못 본다는 지적이었다.
  > 이 문단(실측 배너)은 그대로 두고, 급박도 판정("급하지 않다")과 급박도의 근거를 갈라
  > 적은 서술은 (5-3-9)에 있다 — `SpecMaterials.cs`의 SetTargets 항목 주석도 같은 판정으로
  > 맞췄다.
  >
  ...
  > `RunCodeObjectPipelineAsync` 하나이고 그것을 부르는 곳은 **둘이다**
  > (2026-08-29 리뷰 재현, grep 실측) — `src/ReSet.Cli/Program.cs:1833`
  > (참조분석 OFF, SP 단일 객체 경로)와
  > `src/ReSet.Core/Services/DependencyAnalysisOrchestrator.cs:18`(참조분석 ON,
  > `Program.cs:1853`의 `AnalyzeAsync`에서 재귀 탐색 중 각 노드마다 들어온다).
  > 결론은 바뀌지 않는다 — 둘 다 SP 분석 경로이고 **배치 Job 흐름은 그 자리를 안
  > 지난다.** → **계획서·단계 생성에서는 이 검사가 한 번도 안 돈다.** 다만 검사가
  > 실제로 도는 자리는 이 절의 옛 서술보다 넓다(참조분석 ON일 때 재귀 깊이만큼
  > 반복 호출된다).
  >
  ...
  >
  > **(아) ★ 잔여 위험 — 경쟁 헤딩이 앞서면 검사 D 가 계속 침묵한다
  > (2026-08-29 리뷰, 문서만 — 코드는 이번 회차에서 안 고친다).**
  > `SpecStatementFactsExtractor.ReadTable` 은 `LocalVariableHeadingPrefixes`
  > (`"### 지역 변수"`·`"### 내부 변수"`) 둘 중 **문서에서 먼저 나오는 헤딩
  > 하나만** 잡는다. 이번 회차가 세운 기계 표(`### 지역 변수 (기계 확정)`)가
  > 정상으로 실려도, 그 **앞에** 모델이 자기 재량으로 쓴 `### 내부 변수`
  > 표(헤더 칸에 `명칭` 이 없는 모양)가 먼저 오면 `FindColumn(table.Value.Header,
  > "명칭")` 가 -1 을 내 `if (iName < 0) return variables;` 로 조기 반환하고,
  > 리더는 지역 변수 0 개를 본다. **L1 은 초록이다**(기계 표 자체는 완전
  > 전사됐으니 `CheckLocalVariableDeclarationTable` 이 만족한다) **— 검사 D
  > 만 침묵한다.** 정확히 (5-3-7) 원 결함과 같은 모양(재료 0, 검사 조용히
  > 꺼짐)이 다른 경로로 되살아난다.
  >
  > **왜 이 회차의 어느 층도 이것을 막지 못하는가.** 강제 세 층 — 카탈로그
  > (`MachineConfirmedTables`) · Critic 면제 블록 · L1(`CheckLocalVariableDeclarationTable`)
  > — 은 셋 다 "기계 표가 요구된 자리에 있는가"만 본다. 모델이 그 표
  > **말고 추가로** `### 내부 변수` 같은 표를 더 쓰는 것은 어느 층도
  > 금지하지 않는다 — 카탈로그는 필수 표의 존재만 검사하고, Critic 면제는
  > 기계 표 내용을 건드리지 말라는 규칙이지 다른 표의 존재를 막는 규칙이
  > 아니고, L1 은 위에서 본 대로 기계 표 자체의 전사만 대조한다(Important 1
  > 수정 뒤에도 마찬가지 — 그 수정은 대조 창을 좁혔을 뿐, "경쟁 헤딩이 검사
  > D 리더보다 먼저 온다"는 이 갈래와는 무관하다).
  >
  > **실물 사례(직접 확인).** `output/Functions/dbo.UF_GET_OUTYMD4REFUND
  > /docs/Spec.md:62` 에 정확히 이 모양의 `### 내부 변수` 표가 **지금
  > 실재한다**(헤더: `변수 · 데이터 타입 · 초기값 · 설명` — `명칭` 칸 없음).
  > 이 객체는 `## 파라미터 목록` 절 안에 파라미터 표 다음, 그 표보다 앞자리에
  > 이 `### 내부 변수` 표를 이미 갖고 있고, DECLARE 사실 1건(`@po_strOutYMD`)이
  > 있어 다음 재생성에서 `### 지역 변수 (기계 확정)` 표를 함께 받는다 — 그
  > 표가 기존 `### 내부 변수` 표 뒤에 놓이면(모델이 기존 절 순서를 유지하는
  > 한 그럴 가능성이 높다) 이 갈래가 그대로 발화한다.
  >
  > **이음매 테스트가 이 갈래를 구조적으로 못 본다.** `LocalVariableTableSeamTests`
  > 는 기계 표 **하나만** 있는 문서를 리더에 먹인다 — 같은 절에 경쟁 헤딩이
  > 함께 있는 문서를 만든 적이 없으므로, 이 갈래가 생겨도 그 스윗은 계속
  > 초록이다.
  >
  > **→ 첫 재생성의 카나리아 항목으로 지정한다.** 이 표를 강제 대상으로 올려
  > 첫 재생성을 돌린 뒤, `dbo.UF_GET_OUTYMD4REFUND` 의 검사 D
  > (`CheckSpecLocalVariablesDeclared`) 리더 출력이 0 이 아닌지를 가장 먼저
  > 본다 — 0 이면 이 잔여 위험이 실제로 발화한 것이고, 그때는
  > `SpecStatementFactsExtractor` 의 리더를 넓히는 별도 계획이 필요하다(이
  > 계획의 범위 밖 — 리더를 넓히면 옛 세대 판정 범위가 함께 바뀐다).
...
- **배치 설계의 진입점 목록이 인코딩 없는 경로로 파일계에 묻는다 (2026-09-04)** —
  ...
  **날짜를 오독하지 않도록 적어 둔다 — 이 결함은 그날까지 참조분석 ON에서만 났다.**
  통일 전 OFF는 저장도 조회도 **같은 미인코딩 손조립**을 써서 짝이 맞았고(옛
  `Program.SaveDocumentsAsync`가 `Path.Combine(outputDir, "Procedures", $"{schema}.{name}", "docs")`,
  조회가 같은 모양), 그래서 **우연히 면역이었다.** 분석 경로 통일로 저장이 해석기를 거치게
  되면서 **기본 설정(OFF)까지 이 결함에 들어왔다.** 즉 통일이 만든 결함이 아니라
  ON에 있던 결함의 **적용 범위가 넓어진 것**이며, 고칠 자리는 통일 이전과 같다 —
  조회 쪽을 `OutputPathResolver`로 보내면 두 모드가 함께 닫힌다.
...
`AGENTS.md` 체크리스트의 「심볼을 지웠으면 `grep -rn "<지운 이름>" docs/`」가 **왜 그것만으로는
부족한지**의 실측 근거다. 그 항목이 이 절을 가리킨다.

- **이름 없이 산문으로만 서술한 자리는 그 grep이 원리적으로 못 잡는다 (2026-09-04)** —
  `ExportRawMetadataAsync`를 지운 회차에서 `docs/architecture.md`의 `MetadataExporter` 행이
  그 메서드의 동작("JSON 덤프, Raw 프롬프트 마크다운, 개별 DDL 및 테이블 스키마 내보내기")을
  **심볼 이름 없이** 서술하고 있었고, 이어서 「**재귀 분석에서는** 정본 DDL·매니페스트를
  내보내며」라고 적었다. 통일로 그 조건이 사라졌으므로 둘 다 거짓이 됐는데, 같은 회차가
  `architecture.md` §4.1.1을 정반대로 고쳤으므로 **문서가 자기 자신과 모순한 채** 병합될
  뻔했다. 지운 이름 일곱을 전부 훑는 스윕이 통과했다 — 그 자리에 이름이 없었기 때문이다.
  **처방**: 이름 스윕 뒤에 **지운 심볼이 하던 일의 명사구**로 2차 스윕을 돈다(이 건에서는
  「덤프」·「내보내기」·「재귀 분석에서는」·「켰을 때만」). 사람 대조를 대신하지는 못한다.
- **오탐률이 높아 전문 대조를 기계화할 수 없다 (선행 실측)** —
  `IsCandidateForAnchoredStatementCheck` 제거가 문서 네 자리를 남겼고 그중 하나는 캐시 인상
  판단의 방향을 **반대로** 말했다. 문서 전문 대조는 오탐 88%다(레거시 SQL·CTE·BCL 이름이
  같은 백틱을 쓴다).
...
- **섀도우 복구가 캡처보다 넓게 지운다 — 롤백이 데이터를 파괴한다 (2026-09-06 실측, 🔴)** —
  `output/Jobs/POQSettleBatch1/agent/steps/S05.md`. 캡처(`:113`, 동적 SQL 문자열 안)는
  `… AND TxAmt = 0 AND ExtraSettleFlag = 1` 인데 `SQL_RESTORE_DELETE`(`:285`)에
  **`TxAmt = 0` 이 없다.** 복구 `DELETE` 가 섀도우에 없는 행까지 지우고
  `SQL_RESTORE_INSERT` 는 그것을 되살리지 못한다 — **영구 손실**이다. 그 자리의 주석이
  「rule 4b, 삭제했던 범위와 정확히 동일하게 복구」라고 적혀 있는데 본문이 그것을 어겼다.
  축 B 분류의 **N12** 이고, **검사는 만들지 않기로 했다**(사람 결정 2026-09-06) — 짝 7 중
  진짜가 1 이라 크기가 안 맞는다. 근거·재개 조건·다시 집을 때의 설계 재료(결합 키는 대상
  테이블 · ScriptDom 불가 · 정규화 둘)는 `docs/audit-reports/2026-09-05-축B-잔여결함-분류.md`
  **§16**. **이 관용구는 재생성된 Job 에만 있으므로 전건 재생성 뒤에 재료가 수십으로 는다.**
...
아래 표는 **무엇이 왜 닫혔는지만** 남긴다. **예외는 뒤의 「자리가 좁혀졌다가 닫힌 것」이다** —
설계 문서가 아니라 이 문서가 실측의 원본인 건들이라 본문을 줄이지 않는다.
...
| **계획서의 SQL 배치 위치** (정책 표에서 3회 미룸) | **기준을 정하고 기계로 못박아 닫음.** 2026-08-29 강제 수단 전수 조사가 규칙 21개를 세어 온전 6·부분 7·전무 11을 냈는데, 규칙 3-1(SQL 거처)과 10(NOLOCK)은 기계 강제가 **0건**이었다 — 프롬프트와 Critic 두 층은 둘 다 모델 재량이라 채점자가 바뀌면 함께 꺼진다(실측: 2차 통제군의 Critic이 API 지정 11건을 추론 로그에 적고도 통과시켰다). `MechanicalValidator.CheckNoLockHints`·`CheckPrescribedFrameworkType`·`CheckSqlSideControlFlow`가 그 셋을 못박는다. 판정 스코프가 요점이다 — 셋 다 문서 전수가 아니라 **코드 펜스만** 보고 주석·문자열·mermaid는 뺀다(산문의 `NOLOCK` 약 300건은 전부 「원본의 힌트는 제거한다」는 이행 서술이다). 2026-08-30에 `IF @@ROWCOUNT`가 제어 흐름 판정식에 더해졌고, `SET @v = @@ROWCOUNT`(정상 이행)는 넣지 않았다. `architecture.md §4.4.1` |

### 자리가 좁혀졌다가 닫힌 것 — 2026-09-02 ~ 09-05

**여기만은 요약하지 않는다.** 일곱 건 다 실측 수치가 근거이고, 그 수치를 지우면
「왜 그렇게 고쳤는가」가 재구성되지 않는다. 그래서 본문을 줄이지 않고 옮겼다.
뒤의 둘은 **두 오라클이 서로 반대를 말하는** 부류였고, 그 상태에서 검사나 지표를 넓히면
오탐만 늘어난다는 것을 보여 준 자리다(`check-oracle-provenance`). 앞의 넷은 2026-09-04에
닫혔고, 첫 항목의 요지는 하나다 — **예산이 모자란 것이 아니라 재생성이 그 위반에
닿지 못했다.** 마지막 항목이 남긴 미확인 하나(`AdoptedIsBest`)는 닫히지 않았으므로
위 「돌려 봐야 아는 것」으로 올렸다 — 닫힌 행 안에 두면 묻힌다.

| 항목 | 반복 | 요지 |
|---|---|---|
| **~~골격·문서 전역 L1 위반에 지목 재생성 경로가 없다~~ → 해소(2026-09-04)** | 1 | **반복 실행 3판이 재현성 저하의 최대 원인으로 지목했던 자리다.** 지목 재생성이 단계 섹션만 열었고, 귀속 실패의 폴백인 전량 재생성도 `lastSkeleton`을 일부러 남겨 골격에 닿지 못했다. 실측: 판3의 L1 발화 7회가 전부 골격 mermaid(`Parse error on line 8` · `LOCK--->ORCH`)였고 골격 생성 호출 2회가 둘 다 첫 실패 이전이라 깨진 다이어그램이 6회차를 태웠다(채점 못한 회차 판2 3 · 판3 5, 점수 82→80→72). **고친 방법**: `L1ViolationAttribution`이 구역을 셋(`Step`/`Skeleton`/`Unknown`)으로 갈라 단계 섹션 밖의 위반을 **골격**에 귀속한다 — 골격의 정의는 조립 구조가 준다(단계 상세 H2 밖 전부 + 그 H2 안에서 첫 단계 섹션 앞까지). `HeaderMissing`은 하드 귀속, `MermaidCliError`는 블록 본문을 문서에서 되찾아 위치로 판정한다. 골격 수리는 `MaxL1RepairAttempts`(L1 예산)를 쓰고 채점 예산을 건드리지 않으며, 섹션은 동결된 채 남는다. **정합(단계 목록 표·오류 코드 표)은 패치로 지킨다** — `SkeletonRevision`이 직전 골격을 싣고 「지적된 자리만 고치고 나머지는 바이트 그대로」를 요구하며, 지켜졌는지는 프롬프트가 아니라 `SkeletonRevisionGuard.FindLostAnchors`가 기계로 본다(잃은 앵커를 이름으로 대고 1회 재시도). 같은 자리가 연속 2회 지목되면 백지로 에스컬레이션한다(§3-8과 같은 규칙). **함께 고친 것**: 귀속 실패의 전량 재생성이 이제 골격도 버린다 — 「전량」이 골격을 남기던 것이 이 결함의 나머지 절반이었다. `MaxL1RepairAttempts`는 올리지 않았다(예산이 아니라 도달 범위의 문제였다). 출처: `2026-08-30-batch-plan-loop-reproducibility-design.md` §7-2 「결과」·§7-5 |
| **~~모델이 `sequenceDiagram`에 `--->`를 반복해 쓴다~~ → 원인 귀속이 틀렸다 · 해소(2026-09-04)** | 1 | **모델이 아니라 도구였다.** `MechanicalValidator.CleanseMermaidCode`의 화살표 보정(`-[^-]>` → `-->`)이 flowchart 전용인데 **모든** mermaid 블록에 적용돼, sequenceDiagram의 **유효한** `-->>`(점선+화살촉)를 `--->`로 바꿨다 — 그 토큰은 sequenceDiagram 문법에 없다(flowchart에서는 대시를 늘린 긴 링크라 유효하다). `->>`도 `-->`가 되어 화살촉을 잃었다. 즉 **정화기가 멀쩡한 다이어그램을 부순 뒤 그것을 mmdc에 넘겨** 파스 오류를 받았고, 수리 프롬프트는 모델에게 **모델이 쓰지 않은 줄**을 고치라고 했다. 모델이 다시 옳게 쓰면 정화기가 또 부쉈다 — 판 F2가 그렇게 세 번 연속(패치·패치·백지) 실패해 L1 예산을 소진하고 채점에 한 번도 못 갔고, 판 R3는 6회차를 통째로 태웠다. **배송된 문서도 깨져 있었다** — 파이프라인이 정화본을 산출물로 채택하므로(`consolidatedPlan = l1Result.CleansedMarkdown`) 두 트리에 이미 `--->`를 담은 문서가 6편 있다. **고쳤다**: 정화를 flowchart 계열(`flowchart`·`graph`)로 한정하고, 종류를 모르면 건너뛴다 — 실측에서 **적용은 파괴였고 건너뜀은 무해했다**. 통제군 sequenceDiagram 13편에 옛 정화를 걸면 3편이 새로 깨지고 **깨지지 않은 변경은 하나도 없었다**. `stateDiagram` 27편도 같은 오발을 받았다(노드 셰이프 정규식이 `[*]`에 걸려 공백을 지운다, 11편). **교훈**: 로그의 오류 메시지만 보고 원인을 모델에 귀속했다가 실측으로 뒤집혔다(`measure-dont-reason`). `MechanicalValidator` 주석이 2026-08-29에 이미 `Settle--->Batch`를 관측하고도 「모델이 그렇게 쓴다」로 읽고 지나간 자리다. **남는 함정**: 이 수정 뒤 mermaid L1 발화가 줄 텐데, 그 침묵은 「모델이 잘 쓰게 됐다」가 아니라 「도구가 부수기를 멈췄다」다. **같은 모양이 다른 자리에도 있었다** — PRD 파서가 표 파이프 분해에 `MarkdownTableCellCodec`을 안 써서 인용된 Spec 표 행이 터지고, 그 터진 행을 근거로 「확신도가 CHAR(8)이다」라고 고발한 건이다(다른 세션, 2026-09-04 해소). 둘 다 검사가 아니라 **검사 앞단이 입력을 부수고 그 부순 결과를 근거로 모델에게 지시**했다. 세 번째를 만나면 검사부터 의심하지 말고 **그 앞단이 입력에 무엇을 했는지 먼저 재라** |
| **~~`MermaidCliError`는 귀속 어휘를 만들 수 없다~~ → 해소(2026-09-04)** | 1 | `MechanicalValidator.ViolationLexemes`는 메시지의 백틱 토큰만 훑는데 이 유형의 메시지는 mermaid 컴파일 로그라 백틱이 하나도 없어 항상 빈 어휘였다. 위 항목이 만든 골격 귀속과 함께 닫혔다 — 어휘 대신 `L1ViolationAttribution.AttributeBlock`이 `DetailedError.RawContext`(블록 본문)를 문서에서 줄 단위로 되찾아 구역을 판정한다. **유형만 보고 골격으로 단정하지 않는 이유**: 코퍼스 22편의 mermaid 블록 50개가 전부 골격에 있었지만, 하드 귀속이면 단계 안 mermaid가 깨졌을 때 그 단계가 영영 얼어붙는다. 대조 문서는 `ValidationResult.CleansedMarkdown`이다 — `PostProcessMarkdown`이 mermaid 블록 안을 손보므로 원본으로 찾으면 못 찾는다 |
| **~~`CorpusSetupGuardTests`가 코퍼스 재료 셋 중 둘만 지킨다~~ → 해소(2026-09-04)** | 1 | 가드가 `output/`과 `CorpusPaths.PriorEdition` 둘만 봐서, 셋째 `output.bak-stage4-control-20260828/`은 빠뜨려도 빨간불이 없었다. `CorpusPaths.ControlEdition`·`ControlEditionExists`를 두고 `CorpusSetup_WhenOutputIsPresent_ControlEditionMustAlsoBePresent`가 앞의 둘과 같은 규약(있으면 단언, 없으면서 연결된 워크트리도 아닐 때만 건너뜀)으로 지킨다. 존재 판정은 디렉터리가 아니라 **소비자가 실제로 읽는 파일** (`Jobs/POQSettleBatch4/raw/prompt-context.md`)로 한다 — 링크만 걸고 안이 비면 가드는 초록인데 테스트는 건너뛴다. **함께 정정**: 이 항목이 적었던 「`BatchRunLogReaderTests`가 조용히 건너뛴다」는 낡은 사실이었다 — 그 테스트는 이미 커밋된 발췌(`Fixtures/POQSettleBatch4RunExcerpt.log`)로 옮겨 셋째 재료를 안 쓴다. 실제 의존은 `ProcedureClosureCorpusTests` 하나이고, `LegacyErrorCodeInventionCorpusTests`는 `RESET_SWEEP_ROOT` 창구뿐이다 |
| **~~`DmlScopeExtractor`의 문장↔코드 귀속이 DML 배치 형태를 탄다~~ → 해소(2026-09-02)** | 2 | **원인 귀속이 틀렸었다.** `DmlScopeExtractor`는 두 DML 배치 형태 모두에서 온전하다 — 실측으로 기각. 실제 원인은 재료를 만드는 `SpecReturnCodeExtractor`가 Spec.md **산문**을 `@po_intRetVal = <수>` 패턴으로 훑는데, 그 명세서가 뒤의 넷만 "「-5」를 반환합니다" 꼴로 적어 패턴에 안 걸린 것이다(반환 코드 표는 여덟 개를 다 갖고 있었다). 즉 오라클이 **모델이 쓴 산출물**에 걸려 있었다. 코퍼스 120쌍 대조: DDL⊄산문 2쌍/1편(5트리 중 2트리 — **비결정적**) · 산문⊄DDL 29쌍/6편(`-9`·`4000`·`-3`·`-15`). 그래서 교체가 아니라 **합집합**이다 — 교체하면 29쌍이 새 오탐이 된다. `ErrorCodeMaterial.Merge`가 산문 ∪ 원본 DDL AST를 만들고 루프와 배너가 **같은 값**을 쓴다. 출처: `2026-08-30-batch-plan-loop-reproducibility-design.md` §7-8 |
| **~~`BatchRunLogReader`의 단조성 계수기가 다른 궤적을 센다~~ → 해소(2026-09-02)** | 2 | 설계가 만들려는 것은 **채택본 궤적**의 단조성인데 `CountMonotonicityViolations`는 **Critic 원점수 궤적**의 하락 회차를 센다 — 롤백은 문서 상태를 되돌릴 뿐 다음 회차의 점수를 올려 주지 않으므로, 설계가 완벽히 작동해도 이 값은 0이 되지 않는다. 기준선에서 두 궤적이 같은 값(2)을 낸 것은 그때 **롤백이 없어서** 둘이 같았던 우연이다. **대체 지표를 넣었다**: `UnrolledBackRegressions`(점수가 직전 최고점 아래로 갔는데 되돌림 로그가 없는 회차 수)와 `AdoptedIsBest`(최종 채택본이 궤적의 최고점인가). 대조 실행 재판독 결과 **0건 · 참**으로 「판정 불가」가 닫혔다. 옛 값은 참고값으로 남긴다. **뒤에 남은 것**: `AdoptedIsBest`는 게이트를 **통과해** 끝난 판에서 거짓이 될 수 있다 — 그 경로엔 채택 줄이 없고 마지막 회차가 채택되므로, 4차 90 뒤 5차 85가 통과하면 85가 배송된다. 이 판은 그 갈래를 안 밟아 실재 여부가 미확인이다. 출처: 같은 문서 §7-6 |
| **~~검사 B 의 조인 키 접힘이 진짜 결함 둘을 덮고 있다~~ → 좁혀서 닫음(2026-09-05 `83bdf03f`)** | 1 | `MechanicalValidator.cs:7811-7829` 는 조인 파트너가 CTE·파생 테이블이면(`HasOpaqueJoinSource`) **조인 키 칸 대조를 통째로 접는다.** Task 22 의 근거(원본 단일 UPDATE 를 `대상 AS Y INNER JOIN <계산용 CTE>` 로 재구성하면 진짜 필터가 그 서브쿼리 안에 있어 최상위 `JoinColumns` 로는 안 보인다)는 **일부만 맞는다 — 이번에 처음 쟀다.** **실측 방법**: `23eb16d5` 를 못박은 격리 워크트리에 코퍼스 심링크 셋을 걸고 기준선과 「접힘 OFF」로 `--sweep` 을 각각 돌려 좌표 다중집합을 차분했다(던지는 변경이고 커밋하지 않았다). **결과**: 검사 B 발화 **25 → 40(+15)**, **사라진 발화 0**, 다른 검사 전부 불변(A 18 · C 20 · D 0 · E 56 · 미분류 1095). **폭발하지 않은 이유가 요점이다** — `relocated`(하위 스코프 컬럼 단위 필터)가 이미 `ReportMissing` 안에서 **두 라벨 모두에** 걸려 있어, 「CTE·파생 테이블로 옮겨간 컬럼」은 접힘 없이도 걸러진다. 접힘은 그 위에 덧댄 **통째 차단**이다. **새 발화 15 는 좌표 15 가 아니라 세 부류다.** ① `OrgYMD`(Batch1/S05)·`MALLID`(Batch1/S06) **2 건 — 축 B 재감사가 원본 DDL 로 확인한 진짜 회귀이고, 접힘이 정확히 이 둘을 덮고 있었다.** ② `ID` **2 건**(`POQSettleProc11/S06`·`POQSettleProc17/S07` 의 UPDATE 13, 명세서 조인 키 `PLTID, ID, ClientID, CardCPID` 중 `ID` 소실) — 조인 파트너가 **전부 물리 테이블**이라 Task 22 의 근거가 **안 맞는다**. 원본 DDL 대조는 안 했으나 **오탐이라 볼 근거도 없다.** ③ `PLTID` **11 건**(6 개 Job) — Task 22 관용구 그대로다(`POQSettleProc1/S04` 실물: 최상위가 `INNER JOIN Calc AS C ON C.ID = S.ID` 뿐이고 `PGName` 은 CTE 안 `ON` 에 있어 relocated 가 덮으며 `PLTID` 만 투영에 남는다. 남은 키가 행 식별자 `ID` 라 행 집합이 보존될 공산이 크다). **즉 접힘이 사 주는 값은 좌표 11 개가 아니라 관용구 하나이고, 그 대가가 진짜 결함 최소 둘이다 — 통째 차단을 유지할 근거는 약해졌다.** **함께 쟀다 — 침묵의 주인이 바뀌었다.** 같은 두 INSERT 가 **전 산출물에서는 앵커가 아예 없어**(`Anchor=null`, `CodeAnchor` 만 `-2`) 검사가 도달조차 못 했는데, 재생성이 앵커를 제대로 달자(`Anchor=1`) 그 순간 접힘이 이어받아 계속 침묵했다(리더를 직접 돌려 확인, 못 읽은 문장 0). **좋아진 것이 방어를 켜지 못하고 다른 가드에 삼켜지는 모양이다.** **아직 못 한 것 — 가르는 기준.** `PLTID`(안전)와 `ID`·`OrgYMD`(위험)를 나누는 것은 「남은 조인 키가 행을 유일하게 고르는가」인데 로컬 DB 가 빈 스키마라 기계로 알 수 없다. **길은 하나 열려 있다** — 기준값을 명세서의 컬럼 **이름 목록** 대신 원본 DDL(`raw/metadata.json` 의 `DdlText`)의 **실제 조인 짝**으로 바꾸는 것. 「접힘을 좁힌다」와 크기가 전혀 다르므로 착수 전에 따로 분류하라. 출처: `docs/audit-reports/2026-09-05-POQSettleBatch1-축B-재감사.md` §6·§7 **[2026-09-05 시도와 기각 — 「유일 원천」 판별자]** 접힘을 Task 22 의 자기 근거만큼 좁히려 했다. 그 근거는 `대상 AS Y INNER JOIN <계산용 CTE>` 라는 **파트너** 모양을 말하므로, 서브쿼리가 원천 **전부**인 `INSERT … SELECT … FROM (…) X` 는 접지 말자는 것이었다. 원본 DDL 쪽 근거도 정확히 갈렸다 — 안전 13 은 잃은 조인 키가 **최상위**에 있었고(EXCEPTION_PROC U2 `ON AA.PLTID = BB.PLTID` 93행 · U17 511행 · U13 `X.ID = Y.ID` 379행 · COMM_UPD U7 `X.PLTID = K.PLTID` 257행, 넷 다 상대가 **대상의 재계산**), 위험 2 는 **파생 테이블 안**의 물리 테이블끼리였다(INS_EXTRA 194행 · PLCARD 166행, 상대가 **외부 요율 조회표**). **그러나 단계 실물에서 기각됐다.** 구현해 스윕을 돌리니 검사 B 가 **25 → 25 로 불변**이었다(예측은 +2). 원인: 두 위험 INSERT 도 최상위가 `) X LEFT OUTER JOIN SETTLE_POQ_DB.dbo.TPGProperty Y ON Y.PGName = X.PGNAME` 이라 **파생 테이블이 실물 테이블과 나란한 파트너**다 — 「유일 원천」이 아니다. 단위 테스트는 통과했는데 **합성 픽스처가 실물보다 단순했던 것**이 원인이다(`measure-dont-reason` 의 그 자리 — 사전 선언이 「넷 중 하나라도 어긋나면 되돌린다」고 못박아 두지 않았으면 조건을 손봐 숫자를 맞췄을 것이다). 변경은 **전량 되돌렸고 커밋하지 않았다.** **그러므로 판별자는 단계 문장 안에 없다** — 「원본에서 그 짝이 최상위였나 파생 안이었나」는 DDL 쪽 지식이고, 그것을 검사에 들이는 두 길(명세서에 싣기 = 전건 재생성 / 검사 때 재파생 = 둘째 오라클)이 그대로 남는다. **다음 후보 하나는 아직 안 쟀다**: 리더가 이미 계산하는 `ReadsOwnTarget`·`RowSourceTables` — 불투명 파트너가 **대상의 재계산**인가(안전 13) **외부 원천**인가(위험 2)를 가를지도 모른다. 추측이고 재지 않았다 **[닫은 방법]** 접힘 조건에 `ReadsOwnTarget` 을 더했다 — 접는 것은 불투명한 상대가 **대상 자신의 재계산**일 때뿐이다. 근거가 그리는 그림이 정확히 그것이고, 상대가 대상을 아예 읽지 않으면 「진짜 필터가 그 안에 있다」는 전제 자체가 없다. **착수 전에 합격 조건 넷을 못박고 쟀다** — 검사 B **25 → 27**, 새 발화는 `POQSettleBatch1/S05/INSERT 1/OrgYMD` 와 `S06/INSERT 1/MALLID` **둘뿐**, 사라진 발화 **0**, 다른 검사 **전부 불변**(A 18 · C 20 · D 0 · E 56 · 미분류 1095). 넷 다 맞았다. 게이트 실패 0 · 건너뜀 0 · 경고 0(통과 3,533). **이 발화는 `SuggestedPromptFix` 를 타므로 다음 Batch1 재생성이 그 두 조인 키를 되살리라는 지시를 받는다.** **남은 것 하나** — 회귀 ②(파생 테이블의 중복 투영)는 여전히 그 계열의 검사가 없다(아래 회귀 항목 참고). |
