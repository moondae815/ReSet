...
* **상태 문구의 이스케이프 계약(1차 방어)**: `NotifyStatus`는 인자를 Spectre 마크업으로 파싱합니다 — 호출부가 `[yellow]…[/]`를 직접 싣는 설계라 구현부가 인자를 통째로 이스케이프할 수 없습니다(색이 죽습니다). 그래서 **우리가 쓰지 않은 글자**(AI 본문, 검사 오류문, DB 객체 이름, 경로, 예외 메시지)를 싣는 보간은 호출부가 `EscapeMarkup`으로 감쌉니다. 면제는 [markup-escape-policy-baseline.txt](../../tests/ReSet.Core.Tests/markup-escape-policy-baseline.txt) 한 곳에만 두고 `MarkupEscapePolicyTests`가 양방향으로 잠급니다(새 자리도, 고친 뒤 안 지운 자리도 실패). 지금 남은 면제는 숫자·설정 문자열·도구가 자기 리터럴로 조립한 문장 셋뿐입니다. `NotifyError`·`NotifyL1Errors`·`NotifyL2Defects`는 구현부가 전량 이스케이프하므로 이 계약의 대상이 아닙니다.
* **콘솔이 파이프라인을 죽이지 않는다(2차 방어)**: [ConsoleUserInteraction.SafeMarkupText](../../src/ReSet.Cli/ConsoleUserInteraction.cs)가 출력 직전 `Markup` 생성자로 파싱을 시도하고, 읽히지 않으면 원문을 이스케이프해 내보내며 경고를 남깁니다(태그가 글자로 보일 뿐 문구는 보존됩니다). 1차 방어를 대신하지 않습니다 — 이스케이프를 빠뜨린 호출부는 여전히 결함입니다. 이 층이 없던 2026-09-19에 POQSettleBatch24가 죽었습니다: 하한 검사 발화가 인용한 AI 본문에 `SELECT S.[value] FROM STRING_SPLIT(…)`가 있었고, Spectre가 `[value]`를 스타일로 읽어 던진 예외가 병렬 단계 태스크를 죽였습니다. 예외는 나머지 19단계가 끝난 뒤 `Task.WhenAll`에서 올라와 회차 전체를 실패로 만들었고, 앞선 시도가 L1을 못 넘겨 구제할 점수가 없었으므로 판이 산출물 없이 버려졌습니다.
