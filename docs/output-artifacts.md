...
- `output/` 전체는 Git 추적에서 제외된다. 여기 있는 것은 **딱 하나만 빼고** 전부
  **재생성 가능한 파생물**이며, 사람이 직접 편집해 보존할 자리가 아니다. **그 하나 —
  `settlement-process.md`(§1 ✍️ 사람, §8) — 는 지우면 안 된다.** 재생성은 자리표시자
  초안만 다시 쓸 뿐(`SettlementProcessRosterDraft.Build`), 사람이 채운 단계 이름·순서·
  제외 목록은 돌아오지 않는다 — 지우기 전에 이 예외를 반드시 읽어라.
...
| ✍️ **사람** | 사람이 직접 작성·편집 | 텍스트 에디터 | 도구는 파일이 없을 때 자리표시자 초안 하나만 쓰고, 그 뒤로는 절대 덮어쓰지 않는다. `output/`에서 **재생성으로 못 돌아오는** 유일한 부류다(§8 `settlement-process.md`) |
...
있는데, 이행 스크립트를 두지 않았기 때문이다. `output/`은 (`settlement-process.md` 하나만
빼고, §1) 재생성 가능한 파생물이다.
...
| `Policy/SettlementPolicy.md` | 🤖 AI + ⚙️ 결정적 조립 | 계층 ①이 있고(`Spec.md` 1편 이상) 명부(`settlement-process.md`)가 유효할 때만, `--policy` 배치 실행 또는 TUI 메뉴 `4` | 계층 ①의 명세서 코퍼스와 정산 프로세스 명부에서 뽑은 **정산 정책서.** 계층 ②~④와는 무관하지만 **①은 필수 선행**이다 — 대상 SP가 하나도 없으면 "개별 SP 분석을 먼저 수행하십시오"로 중단한다. SP 단위 계획서처럼 **L1도 L2도 거치지 않아** 헤더에 "검증 없음"이 박힌다 |
...
| `settlement-process.md` | ✍️ 사람 (초안만 ⚙️ 도구) | 명부가 없을 때 최초 실행에서 초안 생성, 그 뒤로는 사람이 직접 편집 | **정산 업무 순서를 정하는 명부.** `output/`의 다른 파일과 달리 재생성 가능한 파생물이 아니라 **사람이 소유하는 입력**이다 — 도구는 파일이 이미 있으면 절대 덮어쓰지 않는다(§10 참고 — 지우면 재생성은 자리표시자 초안만 돌려주고 사람이 채운 내용은 못 돌아온다) |
...
이 문서 첫머리는 `output/`이 (한 예외를 빼고) 재생성 가능한 파생물이라고 적는다.
**재생성으로 같은 것을 다시 만들 수 있다는 뜻이지, 지금 있는 것을 잃어도 된다는
뜻이 아니다.** 재생성은 되돌릴 수 없다.

### 예외 — `settlement-process.md`는 스냅샷으로도 못 지킨다

아래 절차(`cp -a output ...`)는 "재생성이 같은 것을 다시 만든다"는 전제 위에 서
있다 — 스냅샷은 **비교 기준**이지 유일한 보존 수단이 아니다. 이 파일 하나에는 그
전제가 성립하지 않는다. 지우면 재생성은 자리표시자 초안만 새로 쓸 뿐이고
(`SettlementProcessRosterDraft.Build`가 만들고 `SettlementPolicyService.cs`가
호출한다), 사람이 채운 **단계 이름·순서·`## 제외` 목록은 돌아오지 않는다.** 이
파일은 스냅샷이 아니라 **버전 관리**(직접 커밋하거나 별도 저장소에 두는 것)로
지켜야 한다 — 지운 뒤에 스냅샷 관례를 떠올려도 늦다.
