...
`DependencyAnalysisOrchestrator` 한 경로를 타므로 아래 규칙은 켜고 끄고와 무관하게 같다.
끄면 달라지는 것은 **그래프가 루트 한 노드로 끝난다**는 것뿐이다(자식을 분석하지 않으므로).
...
따라서 어떤 산출물이 안 보이면 그것은 설정의 결과가 아니라 **그 노드가 실패했다는 뜻**이다.
