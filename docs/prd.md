...
*   **통합 정산 정책서 (Settlement Policy)**: 레거시를 모르는 업무 담당자가 읽는 **정산 업무 인수인계 문서**를 도출해야 합니다. 서술은 SQL이 아니라 업무 언어여야 합니다.
    *   분석이 완료된 명세서(`Spec.md`)를 **사용자 지정 없이 자동으로** 대상 삼아야 합니다. 사람의 선택 누락이 문서의 누락이 되어서는 안 됩니다.
    *   **업무 수행 순서는 사람이 소유하는 명부(`settlement-process.md`)가 정해야 합니다.** 코드에서 도출되는 것은 부분 순서(선행 적재·호출 무리)뿐이므로, 도구는 그것으로 **초안만** 만들고 파일이 이미 있으면 **덮어써서는 안 됩니다.** 근거는 `architecture.md` §4.10 에 있습니다.
    *   명부가 미완이거나 실제 명세서 목록과 어긋나면 경고로 낮추지 말고 **생성을 중단**해야 합니다 — 빠진 SP 의 업무 규칙은 흔적 없이 사라지기 때문입니다. 의도한 제외는 명부의 `## 제외` 절로만 통과시켜야 합니다.
    *   모든 업무 규칙에 근거가 붙어야 하며, **AI 서술의 근거는 `Spec.md` 로 한정**하고 그 인용은 원본에 축자로 실재해야 합니다. 명부에 실린 SP 는 **최소 한 번은 근거로 인용**돼야 합니다.
    *   **코드값 사전**: 좌변(코드 상수)은 DDL 사본에서 구문 트리로 추출하되 **명세서에 등장하는 값만 채택**하고 탈락분도 사전 파일에 남겨야 합니다. 우변(업무 의미)은 DB 의 코드 테이블에서 채우되, **DB 없이도 「의미 미상」으로 완주**해야 합니다.
    *   이 문서는 L1/L2/L3 를 타지 않으므로 **검증 없음**과 함께, **잔여 결함**(귀속 미통과·미인용 SP·코드값 번역 커버리지)과 **근거가 된 명세서들의 검증 상태**를 문서 머리에 실어야 합니다.
