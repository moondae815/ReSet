...
| **비즈니스 분석가 / PM** | 레거시 시스템에 숨겨진 정산/회계 비즈니스 규칙과 도메인 정책을 한눈에 파악하고 문서화해야 함. | 스키마 설명 누락 추론, 코드-주석 불일치 감지, 그리고 분석된 명세서 전량을 가로지른 통합 정산 정책서 도출(업무 수행 순서는 사람이 명부로 정하고 도구가 덮지 않음). |
...
### 3.2. 메타데이터 정화 및 주석 보완 (Cleansing & Annotation)
...
### 3.3. 3단계 신뢰성 검증 파이프라인 (Verification Pipeline)
...
### 3.4. 통합 배치 현대화 계획 및 정산 정책서 (Modernization & Policy Extraction)
*   **Multi-SP 배치 전환 계획서**:
    *   분석이 완료된 개별 명세서(`Spec.md`) 목록 중, 사용자가 순서대로 지정 선택하여 단계를 구성하고 통합 배치 계획서(`BatchMigrationPlan.md`)를 작성해야 합니다. (순서 보장형 TUI 수집 루프 가동)
*   **통합 정산 정책서 (Settlement Policy)**: 업무 담당자가 읽는 **정산 업무 인수인계 문서**를 도출합니다. 독자는 레거시 시스템을 모르는 사람이므로, 산출물은 SQL이 아니라 업무 언어로 쓰여야 합니다.
    *   분석이 완료된 명세서(`Spec.md`) **전량을 자동으로 대상**으로 삼아야 합니다. 사용자가 SP를 하나씩 지정하게 해서는 안 됩니다 — 정산 정책은 SP 하나가 아니라 코퍼스를 가로질러 성립하므로, 사람의 선택 누락이 곧 문서의 누락이 되어서는 안 되기 때문입니다.
    *   **업무 수행 순서는 사람이 소유하는 명부(`settlement-process.md`)가 정해야 합니다.** 순서는 코드에서 도출되지 않습니다 — 호출 그래프의 EXEC 간선이 희박하고, 쓰기→읽기 그래프는 허브 테이블 때문에 거의 완전 그래프가 되어 순서를 주지 못합니다. 도구는 명부가 없을 때 **초안만** 만들어야 하며, 파일이 이미 있으면 **덮어써서는 안 됩니다.** 이 파일은 `output/`에서 유일하게 **사람이 소유하는 입력**입니다.
    *   명부의 단계 제목이 미완이거나 명부와 실제 명세서 목록이 어긋나면, 결함을 경고로 낮추지 말고 **생성을 중단하고 사람에게 돌려주어야 합니다.** 명부에서 빠진 SP의 업무 규칙은 문서에서 아무 흔적 없이 사라지기 때문입니다. 의도한 제외는 명부의 `## 제외` 절로만 통과시켜야 합니다.
    *   문서의 모든 업무 규칙에는 **근거가 붙어야 하고, 그 근거는 원본 명세서에 축자로 실재해야** 합니다. 근거의 출처는 `Spec.md`로 한정하며, 이 단계에서 DDL을 다시 읽거나 DB를 재분석해서는 안 됩니다.
    *   **코드값 사전**: 좌변(코드 상수)은 SP DDL의 조건식에서 **구문 트리(ScriptDom)로** 추출해야 합니다 — 정규식은 동적 SQL 조립 조각(`' + @v_strClientID + '`)을 상수로 오인합니다. 다만 사전에 **실릴지는 명세서가 정해야** 하며(근거가 명세서뿐이라는 계약이 서야 하므로), 걸러진 값도 버리지 않고 「명세서 미수록」으로 남겨야 합니다. 우변(업무 의미)은 데이터베이스의 마스터/공통 코드 테이블에서 채우되, **DB 연결이 없어도 「의미 미상」으로 완주**해야 합니다.
    *   이 문서는 L1/L2/L3 검증 파이프라인을 타지 않으므로, **검증되지 않았다는 사실을 문서 머리에 명시**해야 합니다. 검증된 명세서와 나란히 놓였다고 같은 신뢰도로 읽혀서는 안 됩니다.
...
*   **TDD 선제 배포 및 자가 수정 피드백 루프**:
    *   외부 코딩 에이전트 기동 직전에 `ValidatorAiService`를 가동하여 타겟 프레임워크 언어에 맞는 단위 테스트 및 ArchUnit 아키텍처 검증 코드(C# xUnit / Java JUnit)를 자동으로 설계 및 tests 디렉토리에 선제 공급해야 합니다.
    *   외부 코딩 에이전트가 로컬에서 작성한 소스코드가 빌드 및 단위 테스트를 통과(L0 성공 - ExitCode 0)했는지 감지 및 제어해야 합니다.
    *   L0 성공 시 L1 정적 검사(구문/컴파일 에러 숏컷) 및 L2 AI 의미론적 일치성(Gap) 검증을 순차적으로 수행하고, 검증 실패 시 오류/불일치 내역을 지시서 하단 피드백 영역(`<!-- FEEDBACK_START -->`, `<!-- FEEDBACK_END -->`)에 갱신 및 축적하여 `MaxL2Attempts` 시도 한도 내에서 외부 코딩 에이전트의 재기동 및 자가 코드 수정을 자동화하고 조율해야 합니다.
