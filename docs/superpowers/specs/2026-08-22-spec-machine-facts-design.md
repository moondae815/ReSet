...
| `커서 수명` | `DECLARE CURSOR`마다 ① `OPEN`~`CLOSE` 사이 `RETURN` 존재 → "그 경로로 종료되면 `CLOSE`/`DEALLOCATE`에 도달하지 않는다"(관측과 직접 귀결만 — 도달 가능성이나 오류 경로 여부는 단정하지 않는다) ② `LOCAL`도 `GLOBAL`도 미지정 → "범위가 **데이터베이스**의 `default_to_local_cursor` 설정에 달려 있다"(`GLOBAL`이 명시되면 그 설정과 무관하게 범위가 전역으로 확정되므로 이 문장은 내지 않는다 — 침묵. I1, Task 15, 2026-08-22 최종 브랜치 리뷰) | 🟠 `UP_UTIL_SETTLE_SUMMARY_ETC` (+ `UP_UTIL_SETTLE_PROC_ETC` ⚪) |
