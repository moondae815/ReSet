...
| `커서 수명` | `DECLARE CURSOR`마다 ① `OPEN`~`CLOSE` 사이 `RETURN` 존재 → "그 경로로 종료되면 `CLOSE`/`DEALLOCATE`에 도달하지 않는다"(관측과 직접 귀결만 — 도달 가능성이나 오류 경로 여부는 단정하지 않는다) ② `LOCAL` 미지정 → "범위가 **데이터베이스**의 `default_to_local_cursor` 설정에 달려 있다" | 🟠 `UP_UTIL_SETTLE_SUMMARY_ETC` (+ `UP_UTIL_SETTLE_PROC_ETC` ⚪) |
