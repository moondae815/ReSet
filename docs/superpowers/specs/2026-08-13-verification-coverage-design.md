...
실행에서 확인한다 — 헤더에 비율이, §0에 해당하는 사유만 실려야 하고, 분할이
실패한 회차에서는 §0이 ✅ 대신 ⚠️를 달아야 한다.

**확인 결과 (POQSettleProc9, 2026-08-13 22:11)** — 네 표면이 같은 사실을 가리켰다.

```
헤더:   단계 검증: 17/18      (종합 신뢰도 바로 아래, §2.1 배치 그대로)
배너:   [검증 불가] S01       (하나)
§0:     ## ⚠️ 0. …  "다만 대조할 재료가 목차에 없어 검증되지 못한 단계가 있습니다."
목차:   TargetTables가 빈 단계 = S01 (하나)
```

18 − 1 = 17이 그대로 맞고, §0은 네 사유 중 **해당하는 하나만** 실었다 — 오류코드
누락도, 커버리지 미확인도, 분할 미실행도 언급하지 않았다. 원본 오류코드는 76/76
보존됐고 12개 프로시저가 모두 본문에 있다. 빈 `LegacyProcedures` 6개는 전부
레거시 출신이 없는 오케스트레이션 단계(검증·잠금·게시·해제)로, 규칙이 명시한
예외 범주 그대로다.
...
이름으로 지목한다. (2026-08-13 후속에서 `GenerationFailed`가 갈라져 나갔다 —
본문이 아예 없는 단계는 하한 검사가 돈 적이 없으므로 이제 함께 빠진다.
`QualityFloor`만 남긴다는 원칙은 그대로다.)

---

## 결정 기록 — Critic 채점에 완전성 축을 넣지 않는다 (2026-08-13)

이 설계는 "점수 자체를 완전성에 연동할지"를 범위 밖으로 미루고, 판단 재료가
모이면 다시 보기로 했다. 네 회차가 모였으므로 여기서 닫는다.

| | 단계 검증 | 오류코드 | 정합성 | 종합 |
|---|---|---|---|---|
| POQSettleProc6 | 1 / 33 | 76/76 | 9 | 88 |
| POQSettleProc7 | 미실행 | **56/76** | 9 | **92** |
| POQSettelProc8 | 17 / 19 | 76/76 | 9 | 88 |
| POQSettleProc9 | 17 / 18 | 76/76 | **8** | 88 |

**가설은 네 점 모두에서 확인됐다.** 오류코드 20개가 사라진 회차가 정합성 9점을
받았고, 76개를 모두 지킨 Proc9가 8점을 받았다. 커버리지가 가장 나빴던 회차와
가장 좋았던 회차가 같은 88점이다. 점수는 완전성과 무관하게 움직인다.

그럼에도 채점을 고치지 않는다. 세 선택지 중 나머지 둘이 더 나쁘다.

- **Critic 프롬프트에 완전성 축을 추가** — 원본 오류코드·프로시저의 잔존 여부는
  이미 기계가 정확히 판정한다(`MechanicalValidator.FindMissingErrorCodes`,
  목차 커버리지 검사). 같은 사실을 모델에게 다시 시키면 두 판정이 갈라지고,
  갈라지면 어느 쪽을 믿을지 다시 문제가 된다. 이 저장소는 그 실패를 반복해서
  겪었다(`TryLocateStepsBlock`, `SpecReturnCodeExtractor`의 주석이 각각 같은
  이유를 적어 두고 있다).
- **기계 판정을 종합 신뢰도에 결합** — 커버리지가 낮으면 점수를 깎는 방식.
  `종합 신뢰도`는 "L2 리뷰가 매긴 점수"라는 정의를 가진 필드이고,
  `SpecHeaderReader`를 비롯한 기존 소비자가 그 의미로 읽는다. 값의 의미를
  바꾸면 문서를 읽는 쪽이 아니라 필드를 읽는 쪽이 조용히 틀린다.

**문제는 "점수가 틀렸다"가 아니라 "점수만 보였다"였다.** Critic은 읽어서 품질을
판정하는 자기 일을 하고 있었고, 없는 것을 보지 못하는 것은 결함이 아니라 그
도구의 성질이다. 완전성은 기계가 이미 정확히 재고 있었고, 빠져 있던 것은 그
측정을 사람 눈앞에 놓는 일이었다 — 이 설계가 한 일이 그것이다.

두 축을 나란히 두는 것이 최종 형태다. 이 결정을 뒤집으려면 위 두 선택지 중
하나가 왜 더 나은지를 새로 논증해야 한다.

## 남은 후속

없음. `GenerationFailed`의 실전 발화(`[생성 실패]` 배너)는 네 회차 모두에서
나오지 않아 단위 테스트와 변이 검사로만 확인됐다 — 드문 경로이므로 일부러
만들지 않고, 실제로 나는 날 확인한다.
