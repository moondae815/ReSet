...
#### 후기 — 네 번째 조건 (전체 브랜치 리뷰에서 발견)

위 셋을 구현하고 전체 브랜치를 리뷰하는 과정에서, 같은 뿌리의 결함이 하나 더
남아 있는 것이 드러났다. `AttachPipelineBanners`는 문서 본문에 두 배너를 이미
붙이고 있었는데, 그중 어느 것도 §0의 판정에 들어가지 않았다.

- `CoverageUnverifiable` — 목차의 모든 단계가 `LegacyProcedures`를 비워 커버리지
  대조 자체가 실행되지 못했을 때.
- `UncoveredProcedures` — 대조는 실행됐지만 원본 프로시저 중 일부가 어느
  단계에도 없을 때. `docs/architecture.md:82`가 이미 이 배너를 "문서가 통과를
  주장하지만 조용히 잘못된" 신호로 이름 붙이고 있었다.

두 배너는 `AttachPipelineBanners`의 `if (noOriginsAtAll) { ... } else if
(uncoveredProcedures.Count > 0) { ... }`로 이미 상호 배타적이다 — 대조를 아예 못
돌린 상태와 돌렸는데 일부가 빠진 상태는 같은 순간에 참일 수 없다. 그래서
`VerificationCoverage`에 필드 하나(`HasUncoveredProcedures`)만 추가해 두 배너를
함께 담는다.

**상호 배타성은 필드를 하나로 합쳐도 되는 이유이지, 문장을 아무렇게나 써도
되는 이유는 아니다.** 첫 시도는 이 둘을 혼동했다 — §0 문구를 "원본 프로시저
일부가 목차 어디에도 나타나지 않았고"로 썼는데, 이는 `UncoveredProcedures`
쪽에서는 참이지만 `CoverageUnverifiable` 쪽에서는 거짓이다. 부재를 단정하는
문장이기 때문이다. `CoverageUnverifiable` 배너 자체가 이 오류를 명시적으로
경고한다 — "원본 명세서 N개가 어느 단계에 대응하는지 확인되지 않았다는
뜻이며, **문서가 그 프로시저들을 다루지 않았다는 뜻은 아닙니다.**" 두 조건이
동시에 참일 수 없다는 사실은 어느 문장이 발화되는지를 결정할 뿐, 그 문장의
내용이 두 원인 모두에서 참인지는 별도로 검증해야 한다. 이 저장소가
`CoverageUnverifiable` 배너를 `UncoveredProcedures`와 갈라 만든 바로 그 이유
(§4.4.5)를 §0에서 한 단계 위로 재현한 것이었다. 최종 문구는 "원본 프로시저
커버리지가 확인되지 않았고"로 고쳤다 — 대조가 아예 안 돈 경우("확인할 방법이
없었다")와 대조는 돌았지만 특정 프로시저가 빠진 경우("확인해 봤지만 못
찾았다") 양쪽에서 그대로 참이다.

이 조건이 빠졌을 때의 결과는 이 설계 문서 자체가 경고하는 모순의 재현이었다.
§0이 `## ✅ 0. … 모두 통과한 계획입니다.`를 찍는 바로 위에, 원본 소스 프로시저
아홉 개가 문서 어디에도 나타나지 않는다고 말하는 `[커버리지 누락]` 배너가 붙을
수 있었다 — 세 사유를 모두 통과했어도 이 넷째 사유 하나만으로 이미 거짓
"모두 통과"였다.
...
| 미검증 단계 존재 | `StepsVerified < StepsTotal` |
| 문서 전체 오류코드 누락 | `HasDocumentCodeGap` |
| 목차 프로시저 커버리지 누락 | `HasUncoveredProcedures` |

넷 중 하나라도 참이면 ⚠️와 단서를 붙인다. 나머지 규칙(참인 조건만 나열, 쉼표로
연결, `Passed`가 아닌 경로는 불변)은 그대로다.
...
원래는 세 조건이었다. `HasUncoveredProcedures`가 후기(위 "후기 — 네 번째 조건"
참고)로 추가되며 아래 표는 네 조건 기준으로 갱신했다.

| 확인할 것 |
|---|
| 네 조건 각각이 단독으로 ⚠️와 단서를 유발한다 |
| 넷 다 거짓이면 ✅이고 단서가 없다 |
| 문구가 참인 조건만 나열한다 |
| `HasUncoveredProcedures`의 문구가 두 원인(대조 미실행/대조 후 누락) 모두에서
  참이다 — 부재를 단정하지 않는다 |
| `Passed`가 아닌 경로의 출력이 바뀌지 않는다 |

두 번째가 부재 확인 테스트다. 조건이 뒤집히면 정상 산출물마다 거짓 경고가 붙는데,
그것을 잡는 테스트는 이것뿐이다. 네 번째를 빠뜨리면 문구가 한쪽 원인에서만
참인 채로 통과할 수 있다 — 실제로 첫 구현이 이 케이스에서 걸렸다.
