...
- ~~신뢰도 점수와 검증 커버리지의 분리 표기 (Proc7의 92점 문제)~~ **해소됨(2026-08-13).**
  [검증 커버리지](2026-08-13-verification-coverage-design.md)가 `VerificationCoverage`
  레코드를 도입하고 `VerificationDocumentFormatter`가 `단계 검증: {검사한 단계}/{전체 단계}`를
  **종합 신뢰도 바로 아래, 축별 점수보다 위에** 찍도록 닫았다 — 독자가 종합 신뢰도를 저울질할
  때 곁에 둘 수치이지 축별 점수 뒤에 딸린 부록이 아니기 때문이다. `StepsTotal`의 `null`(분할
  미실행)과 `0`을 구별하므로 분모 없는 상태가 비율처럼 보이지 않는다.
