...
**새 검사를 켰을 때 이 셋이 깨지면 검사가 틀린 것이다** — 단, 아래 단서가 붙는다.

### 그 규칙이 적용되지 않는 경우 (2026-08-17 추가)

위 문장은 조건 없이 쓰였는데 그것이 틀렸다. **검사가 요구하는 재료가 그 명세서를 생성할
당시에 존재하지 않았다면, 골든 케이스가 실패하는 것은 검사의 결함이 아니다.**

이 계획의 모든 재료가 구조상 그렇다. 새 프롬프트 재료와 그 짝인 L1을 함께 넣으면, 저장된
26개 명세서는 전부 그 재료 없이 생성됐으므로 첫 재생성에서 한 번은 걸린다. 그것이 §설계 0의
계약이 의도한 동작이지 오탐이 아니다.

실측 사례 — `CheckDmlScopeTable`(설계 3.1)을 켜면 프로시저 14개 중 **13개**가 실패하고 그중
골든 케이스 둘(`INS_EXTRA4PLCARD`, `Util_Settle_Summary_AcqManual`)이 포함된다. `CANCEL_INS`만
통과하는데, 그것도 검사가 관대해서가 아니라 그 SP 본문에 `UPDATE`/`DELETE`가 하나도 없어
대조할 행 자체가 없기 때문이다.
...
| 상황 | 골든 실패가 뜻하는 것 |
|---|---|
| 검사가 요구하는 재료가 생성 당시 프롬프트에 있었다 | **검사가 틀렸다** — 원래 규칙 그대로 |
| 재료가 그 뒤에 신설됐다 | 재생성 전 1회 실패는 정상. 재생성 후에도 실패하면 그때 검사를 의심한다 |

그래서 Task 12의 골든 회귀 테스트는 **추출기만** 돌리고 저장된 `Spec.md`에 L1을 걸지 않는다.
그 선택은 `output/`이 `.gitignore` 대상이라 CI에서 비결정적이 된다는 이유와, 여기 적은
"재생성 전에는 실패가 정상"이라는 이유 **둘 다** 때문이다.

**남은 공백**: 위 §테스트 표는 "L1 검사 — 결함 Spec은 잡고 골든 Spec은 통과"라는 양방향
층을 적었으나, `DmlScopeFacts`/`CheckDmlScopeTable`에 대해 그 층을 실제로 구현한 태스크는
없다. 재생성된 명세서가 생기기 전에는 만들 수 없으므로 이 계획의 범위 밖이다. 나중에
누군가 그 층이 이미 있다고 가정하지 않도록 여기 적어 둔다.
...
## `docs/known-defects.md`(당시 `docs/todo.md`)의 유예를 깬다
