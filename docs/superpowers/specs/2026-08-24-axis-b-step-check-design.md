...
> **정정 포인터 (설계서 본문은 얼린다 — 여기는 정정만 가리킨다).** 위 3단의 전제
> ("앵커는 코퍼스에서 드물게만 빠지고, 그때 1건 요구로 충분하다")는 2026-08-24 코퍼스
> 스윕으로 무너졌다 — 앵커는 326개 단계 파일 **전수에서 0개** 잡혔고, 이 3단의 "1건 요구"는
> 사실이 아닌 문구("SQL이 명세서 갱신 번호를 주석으로 달지 않았습니다")를 냈다. 정정은
> `docs/known-defects.md`의 2026-08-24 항목 "검사 B·C의 앵커 방식 자체가 이 코퍼스에서
> 성립하지 않는다"에 있다 — 그 폴백은 이후 요구를 내지 않고 침묵하도록 바뀌었다.
