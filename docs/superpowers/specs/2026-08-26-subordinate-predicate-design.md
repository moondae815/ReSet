...
설계 초안에 없었다 — **`INNER JOIN`에서는 동작 자체가 방어 가능하다**: 그
하위질의가 대상 행을 실제로 거르므로(조인 파트너 행이 `WHERE Z.Hidden = 1`을
만족하지 않으면 조인 자체가 성립하지 않는다), 갱신 대상 행을 거르는 술어로
세는 것이 의도와 어긋나지 않는다.

**그런데 코드는 조인 종류를 구분하지 않는다.** 위 SQL의 `INNER JOIN`을
`LEFT JOIN`으로만 바꿔 직접 돌려 확인했다:

```sql
UPDATE Y SET Y.X = 1 FROM dbo.TSettleMst AS Y
LEFT JOIN dbo.TCost AS C ON C.PLTID = Y.PLTID
  AND C.ID IN (SELECT Z.ID FROM dbo.TZ AS Z WHERE Z.Hidden = 1)
WHERE Y.YMD = @p;
```

결과는 같다(`Sub=[Hidden]`) — `SubordinatePredicateCollector`가 `ON` 절
하위질의를 모을 때 그 조인이 `INNER`인지 `LEFT`인지 보지 않는다. `LEFT JOIN`은
조인 파트너 행이 없거나 `ON` 조건을 만족하지 못해도 왼쪽(대상) 행을 그대로
남긴다 — 즉 `ON` 절 하위질의가 대상 행을 거르지 않는다. 그래서 위 방어 논리는
**`INNER JOIN`에서만 성립하고 `LEFT JOIN`에서는 서지 않는다**: 이행이 진짜로
최상위 필터를 잃었어도, `LEFT JOIN`의 `ON` 절 안에 있는 동명 컬럼이 "이전됐다"로
잘못 침묵시킬 수 있다.

이것은 §6이 다루는 **동명 컬럼 위험과 같은 뿌리**다 — 수집이 조인 종류도
테이블 한정도 보지 않고 이름만 모으기 때문에 생긴다. 닫으려면 조인 종류를 보고
`INNER JOIN`의 `ON` 절만 수집해야 한다 — 다음 회차 개선 항목으로 남긴다.
문서화가 뒤늦게 이 자리를 인정하는 것뿐이다.
...
근거로 `SET` 절을 배제했다. **여기 남은 네 자리(CTE 본문·파생 테이블·JOIN
ON 절 하위질의·최상위 WHERE 하위질의)에도 같은 위험이 원리상 남아 있다** —
테이블 한정이 없다는 것 자체가 원인이라, `SET` 절을 뺀 것으로는 막히지 않는다.
...
같은 뿌리에서 나온 사촌이 §3의 **`LEFT JOIN` `ON` 절 하위질의**다 — 거기서는
"이름만 모은다"가 아니라 "조인 종류를 안 본다"가 원인이지만, 결과는 똑같이
"대상 행을 거르지 않는 자리의 컬럼이 진짜 소실을 침묵시킬 수 있다"는 거짓침묵
위험이다. 두 위험 모두 닫는 방법이 "지금 있는 신호(이름·조인 종류)보다 더
좁혀서 수집하기"라는 점도 같다.
