...
>미룬 ③(b), 그리고 그 브랜치가 실행 중에 발견한 계획 밖 결함 1건. 합 5건 —
> **③(b) 4건(🔴 2 · 🟡 2) + 계획 밖 1건(🟡)**. 두 출처를 함께 세지 않는다: 앞의 넷만 재감사 34건에
> 들고 계획 밖 1건은 34건 바깥이다. (초판은 "🔴 1 · 🟡 4"로 적었다 — 다섯을 한 출처로 묶고 등급을
> 잘못 배분한 오기이고, 그 줄이 계획서 「완료 기준」까지 번졌다. 근거:
> `2026-08-22-out-of-table-scope-design.md:251`, 8회차 보고서 :82·:80·:96·:100, 아래 §2.2·§2.3.)
...
5. **새 표를 만들지 않는다.** 실행 의미 표에 종류 둘을 더한다 —
   ...
   **새 종류는 목록 끝에 붙인다** — 기존 다섯의 순서를 건드리지 않으면 접두사 캐시의
   앞부분이 보존된다. 어차피 캐시 버전 13이 전건을 미스시키므로 실害는 없으나,
   순서를 흔들 이유가 없다.
6. **캐시 버전을 13으로 올린다.** 프롬프트에 실리는 표가 바뀐다. (설계 시점에는 12였다 — `main`이 2026-08-23 ④ 진단으로 12를 먼저 써서 병합 때 13으로 밀렸다.)
...
| `PROC_ETC:72` | `비집계 대입` 행 — 79행 `집계 대입`과 나란히 (초판의 "71"은 오기 — 71은 `--TSettleMiss 기등록여부 확인` 주석이고 `SELECT @v_intID = ID`가 72행이다) |
