...
> **이행 후 남은 것 (2026-08-30, 최종 리뷰가 실측으로 남긴 관찰).**
>
> 1. **타 DB 프로시저는 폐포 순서에 안 들어간다.** 배치 모드의 경로 복원이
>    `Procedures/{식별자}/docs/Spec.md` 만 만드는데(`Program.cs:1030`·`:1035`),
>    `ReadProcedureReferences` 는 `External/<DB>/Procedures/…` 형태도 돌려줄 수 있다.
>    그런 항목은 미매치가 되어 재정렬에서 **끝에 남는다**. 현재 코퍼스의
>    `output/External/SETTLE_CARD_DB` 아래에는 `Functions`·`Objects` 뿐이라
>    **도달 불가**이고 실측으로 재현되지 않는다. 외부 DB 프로시저가 진입점에 들어오는
>    날 닫아야 한다.
> 2. **`Program.cs` 호출부 자체를 잠그는 테스트는 없다.** `ReorderByClosure` 호출 두 줄을
>    지워도 전체 스위트가 통과한다. 폐포 계산과 재정렬은 단위 테스트로 잠겼지만
>    **배선은 안 잠겼다** — `Program.cs` 의 그 자리가 테스트 가능한 형태로 추출돼 있지
>    않은 구조적 한계다.
> 3. **순서는 부모 단위로만 정확하다.** `MetadataExporter` 가 `Nodes` 를 정규 이름
>    알파벳순으로 쓰고 구현이 `Calls` 간선을 안 본다. 형제 참조가 여럿이면 그 사이 순서는
>    호출 순서가 아니라 알파벳순이다. 오늘의 2건에는 무해하다.
