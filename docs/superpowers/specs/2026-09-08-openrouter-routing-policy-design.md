...
그 셋은 전부 라이브 엔드포인트와 설정의 어긋남인데, 옛 검사는 설정을 하드코딩된 표와
대조할 뿐 API를 부르지 않으므로 원리적으로 못 잡는다.

**다만 "잃는 것은 없다"고 적었던 것은 틀렸다(2026-09-08, T4 리뷰가 반증).**
실제로 하나를 잃는다: **두 `appsettings.json`이 서로 어긋나는 것**이다. 폐기된
`AppSettings_PerModelRouting_DeclaresNothingBeyondPinnedTable`의 주석이 그 일을 한다고
명시했었다 — 양쪽이 같은 표의 부분집합이면서 상위집합이면 집합이 같아지기 때문이다.
새 검사 셋은 구조(네임스페이스·비어있지 않음)와 `z-ai/glm-5.3`의 값만 보므로,
한쪽에만 구조적으로 유효한 새 항목이 붙으면 아무것도 잡지 못한다.

**그래서 검사를 하나 더한다**: `AppSettings_BothCliConfigs_DeclareIdenticalOpenRouterRouting` —
두 파일의 `Routing` 구획이 서로 같은지 본다. 계획서 Task 4 Step 2의 손 대조를 테스트로
옮긴 것이다. 이것으로 6절의 검사는 셋이 아니라 넷이 된다.
