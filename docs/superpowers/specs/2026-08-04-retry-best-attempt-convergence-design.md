...
| L1 재시도 소진, 채점된 시도가 없는 경우(`1009-1011`행) | 범위 밖. 첫 시도부터 L1이 계속 실패하면 어떤 시도도 Critic 리뷰를 받지 못해 점수가 없으므로 최고점 개념이 성립하지 않는다 |
| L1 재시도 소진, 이전 시도가 이미 채점된 경우(`994-1015`행) | **알려진 한계, 범위 밖.** 예: 2차 시도가 90점을 받고 3차 시도가 L1에서 깨지면, `bestAttempt`에 90점 후보가 남아 있어도 소비되지 않고 L1이 깨진 3차 시도가 그대로 채택된다. `BestAttempt.HasCandidate`가 이 판단을 위해 존재하지만 `src/` 어디에서도 참조되지 않는다 |
| L2 리뷰 예외로 미검증 채택(`1128-1136`행, 배치 쌍둥이 `1793-1801`행) | **알려진 한계, 범위 밖.** 예: 2차 시도가 90점을 받고 3차 시도의 Critic 호출이 예외를 던지면 `reviewSuccess`가 false가 되어 `ReviewNotRun` 분기로 빠지고, 미검토 상태인 3차 시도 본문이 채택되며 기록된 90점 후보는 버려진다. 위 L1 경우와 같은 이유(`BestAttempt.HasCandidate` 미참조)로 이번 스코프에서는 고치지 않는다 |
