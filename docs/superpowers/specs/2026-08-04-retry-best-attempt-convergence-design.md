...
## 실증 결과 (2026-08-04 재실행)

`dbo.UP_Util_PG_Client_CMRate_Ins`를 수정 전후로 각 1회 실행했다. 두 실행이 같은 로그 파일 `output/logs/reset-20260804.log`에 남아 직접 비교할 수 있었다(수정 전 15:32–15:42, 수정 후 18:21–18:31). 설정은 동일하다 — `claude-cli` / Critic `codex-cli` / 3시도 / 기준 8점.

**시도별 점수 추이** (정합성·CRUD·인터페이스·예외·가독성 → 정규화 점수)

| 시도 | 수정 전 | 수정 후 |
|---|---|---|
| 1 | 8·8·7·7·5 → **70** | 9·10·7·7·10 → **86** |
| 2 | 10·9·9·7·10 → **90** | 6·5·7·7·7 → **64** |
| 3 | 8·9·6·9·7 → **78** | 9·7·10·10·10 → **92** |
| 채택 | **78** (마지막) | **92** (최고) |

수정 전 실행은 90점 후보를 만들어 놓고 78점을 내보냈다(`7083`행: "마지막 리뷰 반영 버전을 사용합니다"). 수정 후 실행은 `14266`행에 "가장 높은 점수를 받은 3차 시도(92/100)를 채택합니다"를 남겼다.

**항목별 판정**

- **A(최고점 채택) — 실행됐으나 판별력은 미실증.** 이번 실행은 최고점이 마지막 시도와 일치해(둘 다 3차) 옛 코드와 결과가 같다. 채택 로그가 최고점을 *계산해서* 보고한다는 점만 확인됐고, 앞선 시도를 되살리는 동작은 단위 테스트로만 잠겨 있다. 다만 2차 시도의 86→64 붕괴는 이 개입이 막으려는 위험이 실재함을 보여준다 — 그 순서가 뒤집혔다면 옛 코드는 64점을 내보냈을 것이다.
- **B(피드백 누적) — 실증됨.** 2차 프롬프트에 `[L2 AI 리뷰 누적 피드백 (최근 1개 라운드)]`(`10461`행), 3차에 `(최근 2개 라운드)`와 `### [시도 1 피드백]` / `### [시도 2 피드백]`(`12856`행)이 들어갔다. 각 항목에 `- 이 시도의 점수: 정합성 9, CRUD 10, ...` 줄이 동봉됐다. 수정 전 실행은 같은 자리에 `[L2 AI 리뷰 최신 피드백 (Stateful Checklist)]`로 직전 라운드만 있었다.
- **C(Mermaid `@`) — 실증됨.** `at at ERROR`는 로그 전체에서 `5932`~`7064`행에만 나타나며 전부 수정 전 실행 구간이다. 수정 후 실행은 0건이고, 최종 `Spec.md`의 다이어그램은 `{"@@ERROR <> 0 ?"}`처럼 원본 표기를 따옴표 안에 그대로 유지한다. 수정 전 3차 Critic이 이 왜곡을 지적해 가독성 7점을 준 반면, 수정 후 3차는 가독성 10점을 받았다.
- **부수 확인.** 최종 문서는 CRUD 7점으로 여전히 품질 미달이지만, 배너가 `[품질 불합격] CRUD 기준 미달`로 실제 미달 항목을 지목한다. 하드코딩 문구였다면 정합성 9·가독성 10인 이 문서를 "정합성/가독성 기준 미달"로 잘못 표기했을 것이다.

**남는 사실.** 1회씩의 실행이므로 B와 C의 품질 효과는 증명도 반증도 되지 않았다. 확인된 것은 세 개입이 모두 의도한 자리에서 의도한 동작을 했다는 것뿐이다. 점수가 70/90/78에서 86/64/92로 바뀐 것도 개입의 효과가 아니라 시도 간 분산의 재현이다 — 두 실행 모두 시도 사이에 20점 이상 출렁였고, 이 진동 자체는 이번 스코프에서 다루지 않았다.

### 2회차 재실행 (2026-08-05) — A의 판별력 실증

1회차에서 미실증으로 남았던 **A의 판별력이 이 실행에서 확인됐다.** 최고점이 마지막 시도가 아니었고, 파이프라인이 마지막을 버리고 최고점을 채택했다.

| 시도 | 결과 | 정규화 점수 |
|---|---|---|
| 1 | L1 실패 — 표에 `위와 동일` 축약 감지(`1337`행) | 채점 없음 |
| 2 | 9·10·9·7·9 | **88** |
| 3 | 8·10·7·8·10 | 86 |
| 채택 | 2차 시도 (`6664`행) | **88** |

옛 코드였다면 86점 문서가 나갔다. 이 실행이 A가 실제로 순위를 매겨 고른다는 유일한 운영 증거다.

같은 실행에서 부수적으로 확인된 것:

- **후보 없는 L1 실패 경계가 정상 동작했다.** 1차 시도는 채점 전에 L1에서 걸렸고, 그 시점에 `bestAttempt`는 비어 있었다. 루프는 후보 없이 다음 시도로 넘어갔고 예외 없이 진행됐다.
- **피드백 누적이 라운드 수를 정확히 셌다.** 3차 프롬프트에만 `[누적 피드백 (최근 1개 라운드)]`가 붙었다(`4958`행). 1차는 L1 실패라 L2 피드백이 존재하지 않았으므로 누적 대상이 2차 하나뿐인 것이 맞다.
- **Mermaid `@` 왜곡 0건.** 로그 전체에 `at at`이 없고, 최종 문서는 `{"@@ERROR <> 0 ?"}` 표기를 유지한다.
- **배너가 세 번째로 다른 항목을 지목했다.** 이번엔 `예외 기준 미달`(예외 7점). 세 실행에서 지목된 항목이 각각 달랐다는 것이 계산식이 실제로 동작한다는 증거다.

**해석상 주의.** 이 실행은 모델이 `claude-sonnet-5`에서 `claude-opus-5`로 바뀌었다. 따라서 88점이라는 절대값을 1회차의 92점과 비교하는 것은 의미가 없다. 이 실행이 증명하는 것은 점수의 높낮이가 아니라 **선택 메커니즘**이며, 그것은 모델과 무관하다.

**여전히 남은 것.** 재시도 카운터가 L1 실패와 L2 실패를 함께 센다는 점이 이 실행에서 드러났다 — 1차 L1 실패가 3회 예산 중 하나를 소모해 채점된 시도는 2회뿐이었다. 기존 동작이며 이번 스코프 밖이지만, 재시도 예산의 실효 크기가 설정값보다 작아질 수 있다는 사실은 기록해 둔다.
