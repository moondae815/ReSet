...
4. 어떤 단계의 `LegacyProcedures`가 다른 단계에도 실린 프로시저를 반복하는지 확인. 같은
   프로시저가 두 단계에 걸리면 두 단계가 같은 쓰기 집합을 물려받고 각 본문은 절반씩만
   기술할 수 있어, 재생성으로 해소되지 않는 `QualityFloor` 오류가 난다. 목차 커버리지
   검사는 명세서당 1개 이상만 요구하므로 이 형태를 막지 않는다. 실측 두 회차는 1:1이라
   이 경우가 없었다.

## 구현 후 기록 (2026-08-12)

8개 태스크를 5개 파도로, `/parallel-sdd`로 2웨이 병렬 실행했다. 11커밋, 테스트 1,318 →
**1,342**, 빌드 경고는 8개 기준선을 전 구간 유지했다. 체리픽 충돌은 다섯 파도 전부에서
**0건**이었다 — 계획을 쓸 때 한 독립성 분석이 실측으로도 맞았다.

### 계획·설계 자신이 틀렸던 것 셋

**Task 2의 grep이 대상을 놓쳤다.** `"new BatchStepPlan(\|new(code, name"`으로 생성자 호출부를
전수 확인하라고 적었으나, 실제로는 target-typed `new(...)` 호출부 세 곳을 놓쳤다. 정규식
리터럴이 아니라 **컴파일러의 CS7036**(필수 인자 누락)이 그 세 곳을 잡아냈다. 교훈: 위치
레코드에 필드를 추가할 때 권위는 컴파일러이지 grep이 아니다 — grep은 사람이 쓴 호출부 표기의
다양성을 전제로 하는데, 그 다양성 자체가 놓치는 이유가 된다.

**Task 5/Task 6의 경계가 컴파일이 안 되는 지점에 그어져 있었다.** `Program.cs`가
`CancellationToken`을 위치 인자로 넘기고 있어, Task 5가 새 매개변수를 그 앞에 끼우면 CS1503으로
전체 솔루션이 깨졌다. `ReSet.Core.Tests`가 `ReSet.Cli`를 참조하므로 스위트 전체가 막혔다.
Task 5가 스스로 세운 완료 기준("1,339건 통과")은 Task 6 없이는 애초에 도달 불가능했다. 교훈:
태스크 경계는 반드시 빌드 가능한 지점에 그어야 한다 — "다음 태스크가 마저 고친다"는 전제로
경계를 나누면, 그 사이의 트리는 누구도 검증할 수 없는 상태로 남는다.

**Task 7이 스스로 제안한 테스트가 주장한 하중을 지지 못했다.** 계획이 제시한 예제의
빈-`Name` 원소가 빈 `SpStaticAnalysisResult`를 썼는데, 그러면 "쓰기·읽기 둘 다 빔" 스킵
조건이 이름 가드보다 먼저 걸려 통과했다 — 이름 가드를 지워도 테스트가 그대로 초록이었다.
Task 7이 닫으려던 결함(가드 없이도 통과하는 테스트)이 Task 7 자신의 테스트 안에 있었다.
그 원소에 실제 테이블을 채워 닫았다 — 이름 가드를 지우면 `result[""]`가 만들어지고
`result["실명"]`이 없어져 테스트가 실제로 깨진다.

### 설계가 맞았던 것

**교체-비-합집합 규칙은 측정으로 확인됐다.** S11 회귀 픽스처(`S11PlanStructureExcerpt.md`)가
실측 그대로의 사례를 고정한다 — 목차 선언 4개 중 3개(`TSettleByTX`·`TPartialCancelByTX`·
`TSettleByIN`)가 원본 DDL에 한 번도 등장하지 않았다. 뮤테이션으로 버려진 선언 경고 블록을
지워 보면 이 픽스처가 실제로 깨진다 — 설계가 §3에서 든 근거("합집합하면 허위가 검증 요건으로
승격된다")가 지어낸 예제가 아니라 실측 사례로 뒷받침된다.

### 최종 리뷰로 미룬 것 (여기서 고치지 않음)

- **약한 부정 테스트**: `Pipeline_ShouldNotEnrichTablesWhenDefinitionsAreOmitted`는
  `definitions`를 아예 넘기지 않아 `Extract(null)`이 빈 사전을 낸다. "보강이 스킵됐다"와
  "보강이 돌았는데 결과가 비었다"를 구별하지 못한다.
- **`FixedErrorCodeSection` ↔ `dbo.T1`의 문서화되지 않은 결합**: 오류코드 검증 브랜치가 남긴
  테스트 헬퍼로, 고정 본문 문자열에 `dbo.T1`을 하드코딩하고 그것이 같은 파일의 JSON 픽스처
  `TargetTables: ["dbo.T1"]`과 우연히 일치해야 하한 검사를 통과한다. 어느 쪽도 그 결합을
  주석으로 밝히지 않는다. (최종 브랜치 리뷰 D2에서 `FixedErrorCodeSection`에 결합을 밝히는
  주석 한 줄을 추가해 닫았다 — 문자열 자체는 바꾸지 않았다.)
- **테스트 이름 `WriteAsync_ShouldScopeStepDependenciesToItsOwnTargetTables`가 이제 오해를
  유발한다** — 스코프 원천이 `TargetTables`에서 `SchemaTables`로 바뀌었는데 이름은 옛 이름
  그대로다. (최종 브랜치 리뷰 D3에서 `WriteAsync_ShouldScopeStepDependenciesToItsOwnSchemaTables`로
  이름만 바꿔 닫았다 — 픽스처·단언은 그대로다.)

### 사람이 직접 확인해야 하는 것 (재확인)

위 §사람이 직접 확인해야 하는 것 항목(최종 브랜치 리뷰가 추가한 4항목 포함)은 그대로
유효하다. 이번 구현의 테스트는 전부 단위 수준이고, 그중 어느 것도 단위 테스트로 닫히지
않는다 — 모든 항목이 실제 Job이나 실제 회차 실행을 봐야 한다.
