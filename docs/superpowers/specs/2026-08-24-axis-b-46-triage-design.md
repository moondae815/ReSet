...
**실측이 들어왔다.** Task 10(재생성 실측)은 **돌지 않기로 결정됐고**(2026-08-24, 사용자 결정),
대신 진행 회차가 단계 파일과 코퍼스로 직접 확인한 결과를 아래 `실측` 칸에 싣는다. 재생성 없이
잰 것이므로 "재생성 후에도 그런가"는 여전히 미확인이다.

**#3·#5는 이 표에서 빠졌다** — 검사 C가 둘 중 어느 것도 닫지 못한다는 것이 `be14080`에서
실측으로 확정돼 L층으로 옮겼다(3-2절 끝).

> **예측이 두 번 틀렸다.** #4·#7을 "검사 B가 이미 병합됐으니 닫힘 유력"으로 적었는데 둘 다
> **미달**이다. 검사 B가 병합돼 있었다는 사실은 맞았으나, 그 검사가 **작동하지 않았다** —
> 코퍼스 326개 단계에서 앵커가 **0개** 잡혔다(아래 3-1-1절). 검사가 코드에 있다는 것과
> 그 검사가 재료를 실제로 읽는다는 것은 다르다. 남은 예측도 같은 종류의 오차를 안고 있다.

| # | 등급 | 단계 | 겨냥한 수단 | 예측 | **실측** |
| :-- | :-- | :-- | :--- | :--- | :--- |
| 1 | 🔴 | S07 | 검사 A `CheckStatementCountAgainstSpec` | 닫힘 유력 | **닫힘** — 실물 확인 |
| 2 | 🔴 | S14 | 검사 D | 닫힘 유력 | **닫힘** — 실물 확인. 금액 3종 `MONEY`가 시정 메시지에 실렸다 |
| 4 | 🟠 | S07 | 검사 B `CheckAnchoredStatementFacts` | 닫힘 유력 — 이미 병합됨 | **미달** — 검사가 작동하지 않았다(앵커 0개, 3-1-1절) |
| 6 | 🟠 | S09 | 프롬프트 규약 2조항 | 불확실 | **미측정** — 규약은 유도뿐이고 실측을 하지 않았다 |
| 7 | 🟠 | S11 | 검사 B | 닫힘 유력 — 이미 병합됨 | **미달, 원인도 달랐다** — `S11.md`는 서수 주석을 아예 쓰지 않는다(3-1-1절) |
| 8 | 🟠 | S13 | 검사 E | 닫힘 유력 | **닫힘** — 실물 확인, 코퍼스 129건 검출 |
| 9 | 🟠 | S14 | 프롬프트 규약 2조항 | 불확실 | **미측정** — #6과 같다 |
| 12 | 🟡 | S05 | 검사 E | 닫힘 유력 | **미확인** — 회차 보고가 S05를 따로 들지 않았다 |
| 31 | ⚪ | S13 | 검사 E | 불확실 | **미확인** — 같음 |

**닫힘 3 · 미달 2 · 미측정 2 · 미확인 2.** 회차 목표였던 🔴 2 · 🟠 7 중 실제로 닫힌 것은
🔴 둘과 🟠 하나다.

> **#31은 여전히 열려 있다.** 검사 E의 판정식이 "초기값 ∈ `ErrorCodes`"이면 `NULL` 초기화는
> 통과한다. 진행 회차에 전했고, "선언 기본값과 다른가"로 넓히려면 `SpecExpectations`에
> 파라미터 기본값 재료가 없어 확장이 선행해야 한다는 답을 받았다.

#### 3-1-1. 앵커 방식이 이 코퍼스에서 성립하지 않는다 (진행 회차 실측)

검사 B·C의 재료인 **서수 앵커가 코퍼스 326개 단계에서 0개** 잡힌다. 실물은
`/* U1: … */` → `SET @v_currentStepId = -101;` → `UPDATE …` 순서인데, 그 `SET`은
`AiService`의 오류 추적 규칙이 요구하는 필수 패턴이라 주석과 문장 사이에 항상 끼어든다.
그래서 검사 B의 검출 195건은 **전부 "앵커를 달아라"는 폴백**이었고(이미 달려 있는데),
검사 C는 0건이었다.

**`SET`을 건너뛰게 고치는 것은 더 나빴다.** 앵커는 살아났지만(S07 0/8 → 8/8) 틀린 문장에
붙었다 — 산출물에 **미구현 UPDATE의 서술 주석이 DML 없이** 남아 있어 뒤따르는 무관한 실제
DML이 그 앵커를 훔친다. 실물 3건으로 재현됐다(`S07.md:244`가 `U15` 주석을, `S08.md:51`이
`UPDATE 2` 플레이스홀더를, `S08.md:148`이 `UPDATE 10` 주석을 가져갔다). 지금은 검사 B의
폴백을 침묵으로 바꿔 두었다(195 → 0) — 거짓 시정 지시가 `floorFeedback`으로 나가는 것을
막는 것이 먼저였다.

**다음 수단의 단서.** 오귀속된 문장들이 전부 **다른 spec 행과 술어 컬럼 집합이 정확히
일치**했다. 주석 위치가 아니라 **술어·조인 컬럼 집합으로 문장↔spec 행을 맞추는 것**이
가능해 보인다. 아래 단위 **S**로 세운다.

**#7(S11)은 이 뿌리가 아니다.** `S11.md`는 `SET`이 끼는 문제가 아니라 **서수 주석을 아예
쓰지 않는다** — 원본 오류코드를 라벨로 쓴다(`-- -13: 원천카드 수동매입 지급일 및 매입요청일`).
앵커 방식을 고쳐도 그 단계는 안 잡힌다. 단위 **T**로 따로 세운다.

#### 3-1-2. 파서가 펜스를 통째로 버린다 (진행 회차 실측)

`EXEC @ret = sp @param = CONCAT(...)`처럼 **실행 인자에 함수 호출식**이 오면 ScriptDom 문법
제약에 걸려 **그 펜스 전체가 버려진다.** `TSql100Parser`~`TSql180Parser` 전 버전에서
재현되어 버전 상향으로는 닫히지 않는다(설치된 180.37.3의 9개 파서 클래스 전부 확인).

코퍼스 326개 중 **65개 파일이 0문장**이고, 검사 A의 검출 234건 중 **92건(39%)이 이 부작용**이다.
검사 A의 검출량 절반 가까이가 실제 결함이 아니라 파싱 실패다. 남은 길은 문장 단위 분리
파싱이다. 단위 **U**로 세운다.
...
| 42 | 🟡 | S16 | (나) 같은 자리 | **L** | ✓ 같음 |
| 3 | 🟠 | S07 | 명세서에 없는 `HAVING SUM(TxAmt)=0` 집계를 원본 로직으로 서술 | **R** 원본 집계 유무 대조 | ✗ **DML 범위 표의 GROUP BY 칸으로는 안 된다**(아래 상자) — 원본 DDL의 집계 유무를 새 재료로 뽑아야 한다 |
...
> 둘 다 **기준값이 없는** 쪽이다. #5는 명세서 표에 IF 종류 행이 없고, #3은 아래 이유로
> GROUP BY 칸을 기준값으로 쓸 수 없다. 축 A 산출물이나 추출기를 늘려야 하는 무거운 단위들이다.
>
> **#3에 DML 범위 표의 GROUP BY 칸을 쓰면 안 되는 이유.** 한 번 그렇게 적었다가 회수했다.
> T-SQL UPDATE는 최상위 `GROUP BY`를 가질 수 없으므로 **UPDATE 행의 GROUP BY 칸은 언제나 `—`다.**
> 그러면 "칸이 `—`인데 단계 문장의 `HasGrouping`이 참"은 두 가지를 같은 모양으로 만든다 —
> 단계가 없던 집계를 새로 붙인 경우(#3)와, **원본부터 파생 테이블에서 집계하는 정상 문장**이다.
> 후자는 실재한다: `UP_UTIL_SETTLE_COMM_UPD`(S08의 레거시) UPDATE 7의 파생 테이블 `D`·`K`가
> `PLTID`별로 그룹을 만든다(`COMM_UPD/docs/Spec.md:480`). 그 문장들이 전부 거짓 오류를 받고,
> 그 문자열이 재생성 프롬프트에 시정 지시로 실린다.
>
> **다만 S07 자체는 깨끗하다.** 원본 `UP_UTIL_SETTLE_EXCEPTION_PROC`에는 `GROUP BY`도 `HAVING`도
> **0건**이다(DDL 직접 확인). 그래서 살릴 길이 하나 있다 — 비교 대상을 명세서 칸이 아니라
> **원본 DDL의 집계 유무**로 바꾸면 구별이 성립한다. 그 사실은 `SqlStaticParser`가 이미 보는
> 자리에 있고 명세서 산문에도 파생 테이블 서술로 남아 있다(`COMM_UPD/docs/Spec.md:480`이 그 예다).
> 재료로 뽑는 일이 단위 **R**이다.
...
**46건은 실행 단위 21개다** — 끝난 6개(검사 A·B·C·D·E와 프롬프트 규약 2조항)와 남은 **15개**다.

끝난 6개가 닫은 것은 **3건**이다(#1 #2 #8). 검사 B·C는 재료가 코퍼스에 없어 작동하지
못했고(3-1-1절), 프롬프트 규약 2조항은 실측되지 않았다. **A층 9건 중 6건이 열린 채 남아**
아래 표의 단위 **S·T·U**가 그 뒤를 잇는다.
...
| **I** DML 범위 표 ORDER BY 칸 대조(명세서에 값이 있는데 단계에 없음) | #22 (1건) | 있음 | 작다 — 검사 B와 같은 재료 |
...
| **R** 원본 집계 유무 재료(원본 DDL의 `GROUP BY`·`HAVING`을 문장 단위 사실로) | #3 (1건, 🟠) | **신설** | 중간 — 명세서 칸으로는 원본 보존과 신설이 구별되지 않는다(3-2절 상자) |
| **Q** IF 가드 문장 재료(`DmlCollector`의 `IfStatement` 방문 + 명세서 DML 범위 표의 IF 종류 행) | #5 (1건, 🟠) | **없음** | 크다 — **축 A 산출물의 표를 늘려야 한다**. 명세서 31개 재생성이 딸려 온다 |
| **S** 문장↔spec 행을 **술어·조인 컬럼 집합**으로 맞춘다(주석 위치 대신) | #4 (1건, 🟠) · 검사 B·C의 재료 전체 | 있음 | 크다 — 앵커 방식 자체의 대체. 오귀속 실측이 이 방향을 가리켰다(3-1-1절) |
| **T** 서수 주석을 쓰지 않는 단계의 문장 식별 | #7 (1건, 🟠) | **없음** | 중간 — `S11.md`는 원본 오류코드를 라벨로 쓴다. 단위 S로도 안 닫힌다 |
| **U** ScriptDom 파싱 실패(실행 인자의 함수 호출식)에 대한 문장 단위 분리 파싱 | 직접 닫는 결함은 없으나 **검사 A 검출의 39%가 이 부작용**이다 | — | 크다 — 파서 버전 상향으로는 안 닫힌다(전 버전 재현) |

**권고 순서.** 작고 재료가 있는 것부터다 — **M → L → F → I → J → K**가 한 묶음이다. 여섯 단위로
12건이 닫히고, 전부 기존 재료를 쓰거나 재료가 필요 없다. 그다음이 재료를 늘리는 **G → H → R**(3건)이고,
**C**(계약)와 **Q**(IF 가드)는 각각 설계와 축 A 산출물 변경이 선행해야 하므로 별도 회차다.
**P** 3건은 아무 때나 얹을 수 있다.

**🟠 둘(#3 #5)은 첫 묶음에 없다.** 둘 다 재료를 새로 뽑아야 하고, #3은 한 번 "기존 재료로 된다"고
적었다가 회수한 자리다(3-2절 상자). 등급이 높다고 먼저 닫히지 않는다 — 이 회차에서 등급과 비용이
어긋난 대표적인 자리이므로, 진행 중인 회차가 이 둘을 `known-defects`에 열린 항목으로 남기는 것이
맞다.

**~~주의 하나.~~ 해소됨.** 이 순서를 Task 10(재생성 실측) 이후로 미뤄 두었으나,
**Task 10을 돌리지 않기로 결정됐다**(2026-08-24, 사용자 결정). 재생성을 하지 않으므로
검사·계약 변경이 측정에 잡음을 섞을 대상 자체가 없다. 착수 시점 제약은 사라졌다.

**대신 새 주의가 생겼다 — 단위 U를 먼저 볼 값이 있다.** 검사 A 검출의 39%가 파싱 실패의
부작용이므로, 그것을 닫기 전에는 어떤 검사의 검출량도 신뢰하기 어렵다. 코퍼스 326개 중
65개 파일이 0문장으로 읽히는 상태에서 잰 수는 그 표본 위에서만 참이다.
