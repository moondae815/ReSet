...
**범위 밖**
- **클라이언트 계층의** 429 백오프·재시도 정책. `OpenAiClient`는 현재 비성공 응답을 즉시 예외로 던지며 재시도가 없다(`OpenAiClient.cs:95`). 이를 바꾸는 것은 이 클래스 전반의 오류 정책을 건드리는 별개 과제다.

  **단, 오케스트레이터 계층의 단계 재시도에는 지터 지연을 넣는다** (구현 중 최종 리뷰에서 추가된 판정). 이 설계가 원래 놓쳤던 것은 악화 요인이 동시성 단독이 아니라 **동시성 + 무지연 재시도**의 조합이라는 점이다. 순차일 때는 자연스러운 요청 간격이 rate-limit 창을 넘겼으나, 동시 4에서는 429 하나가 진행 중인 4개 단계를 동시에 때리고 각자 2회 시도를 같은 창 안에서 소진해 최대 4개 섹션이 한 번에 강등된다. `GenerateStepSectionWithFloorRetryAsync`가 **예외로 실패한 경우에만** 재시도 전 0.5~1.5초 무작위 지연을 두어 상관된 폭풍을 흩트러진 재시도로 바꾼다. 하한 미달 재시도는 지연하지 않는다 — 그것은 rate limit이 아니라 모델 출력 품질의 문제다. 13분짜리 구간에서 1초는 무시할 만하고, `OpenAiClient`의 오류 정책은 그대로 둔다
- 골격·브레인스토밍·목차 호출의 병렬화. 각각 1회씩이라 병렬화할 대상이 없다
- L3 피드백 재생성 경로에 대한 **별도 작업**. 이 설계가 작성된 시점에 그 경로는 단일 호출이었으나, 이후 `feat/l3-step-targeting`(병합 커밋 `be51db5`)이 같은 `GenerateBySplitAsync`를 거치도록 배선했다. 따라서 병렬화는 그 경로에도 자동으로 적용된다 — 이 설계가 따로 손댈 것은 없다
