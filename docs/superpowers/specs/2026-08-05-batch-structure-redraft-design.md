...
피드백 본문과 그 성격은 함께 움직이는 값이므로 결과는 이미 피드백을 나르는 DTO에 싣는다. `IVerificationUserInteraction`에 **메서드**를 추가하지 않는다.

다만 `RequestHumanReviewAsync`에는 선택적 매개변수 하나를 더한다.

```csharp
Task<HumanReviewResult> RequestHumanReviewAsync(
    string selectedOption, string specificationMarkdown, VerificationOutcome outcome,
    bool structureRedraftSupported = false);
```

이 메서드는 **두 파이프라인이 함께 쓴다** — 단일 SP 명세서 경로와 통합 배치 계획 경로. 다시 세울 목차가 있는 쪽은 후자뿐이다. `ConsoleUserInteraction`은 피드백 입력 직후 "이 피드백이 문서 구조(목차)까지 바꾸나요?"를 확인하는데, 이 질문을 무조건 던지면 명세서 경로에서는 사용자가 답해도 그 답을 쓸 곳이 없다 — 이 설계가 없애려는 "답했는데 아무 일도 일어나지 않는" 상태를 다른 자리에 새로 만드는 셈이다. 그래서 확인은 이 값이 true일 때만 묻고, 통합 배치 호출부만 true를 넘긴다.

기본값 `false`를 두었기 때문에 기존 구현체와 테스트 fake는 그대로 컴파일된다. "인터페이스를 늘리면 모든 구현체가 함께 바뀐다"는 부담은 매개변수 추가에는 해당하지 않는다.

L3 자체는 `isBatchMode`가 false일 때만 돌지만, 그것만으로는 이 확인의 조건이 되지 못한다. TUI 안에서도 두 파이프라인이 모두 L3에 닿기 때문이다. 판단 기준은 실행 모드가 아니라 **지금 검토 중인 문서가 목차를 가진 문서인가**이며, 그것을 아는 것은 호출부뿐이다.
...
불변식은 이렇다 — **파이프라인이 종료하거나 문서를 사용자에게 건네는 모든 지점에서, `raw/PlanStructure.md`는 그 문서를 실제로 만든 목차를 담고 있어야 한다.** 교체되는 목차는 `raw/PlanStructure.superseded-{n}.md`로 남기고, 번호는 이어 붙인다. 기존 테스트와 감사 흐름이 `PlanStructure.md`를 최종본으로 읽고 있어 그 계약을 깨지 않으면서 구조 변경 이력을 추적할 수 있다. `Brainstorming.md`는 변경하지 않는다.

"재수립할 때 파일을 쓴다"로는 이 불변식이 지켜지지 않는다. 재수립한 목차가 실제로 채택될 문서를 만든다는 보장이 없기 때문이다. 두 경로에서 어긋난다.

- **L2 + `RetryRescue`.** 1회차가 최고점을 잡고, 2회차가 갱신에 실패해 재수립이 발동하고, 3회차가 새 목차로 더 낮은 점수를 낸다. 예산이 소진되면 `RetryRescue`는 1회차를 채택한다 — 재수립 **이전** 목차에서 나온 문서다. 재수립은 애초에 최고점이 안 깨졌을 때만 발동하므로 이것은 드문 경로가 아니라 흔한 경로다.
- **L3.** 재수립은 성공했는데 뒤따르는 본문 재생성이 예외를 내거나 빈 응답을 내면, 화면에는 여전히 이전 문서가 떠 있는데 디스크에는 새 목차가 앉아 있다. 사용자가 그대로 승인하면 저장된 계획서와 기록이 어긋난다.

그래서 파일 쓰기는 **목차를 만드는 일과 분리한다**. 재수립은 새 목차를 돌려주기만 하고 아무것도 쓰지 않는다. 쓰기는 그 목차가 실제로 쓰인다는 것이 확정된 뒤에만 일어난다 — L2는 다음 회차가 그 목차로 생성될 것이 확정된 시점에, L3는 재생성된 본문이 비어 있지 않음을 확인한 뒤에.

같은 쓰기 경로를 구제 채택도 쓴다. `RetryRescue`가 재수립 이전 회차를 채택하면 그 회차를 만든 목차를 다시 현행으로 되돌리고, 버려지는 재수립 목차를 `superseded`로 남긴다. 어떤 목차가 시도됐고 왜 채택되지 않았는지가 `raw/` 디렉터리만 보고 재구성되어야 하기 때문이다. 따라서 `superseded-{n}`은 재수립 기록 전용이 아니다 — 재수립으로 밀려난 목차와 구제 채택으로 되돌려진 목차를 모두 담는다. 되돌린 직후에는 `superseded-{n}`과 `PlanStructure.md`의 내용이 같아질 수 있다. 이 파일들은 이력이지 서로 다름을 보장하는 집합이 아니다.

이를 위해 최고 후보의 목차를 후보와 한 몸으로 들고 다닌다. `BestAttempt.TryRecord`가 갱신을 알린 그 자리에서 현행 목차를 함께 붙잡아 두면, 후보와 그 목차가 어긋날 자리가 없다.

### 5. 실패 처리

재수립은 개선 시도이지 필수 단계가 아니다. 네 가지를 모두 흡수한다.
...
- **파일 기록 실패** → 재수립을 없었던 일로 되돌리고 기존 목차를 유지한다. `superseded` 파일을 먼저 쓰는 순서이므로 어느 쓰기가 실패하든 `PlanStructure.md`는 아직 이전 목차를 가리키고 있다. 되돌리기만 하면 파일과 실제 쓰이는 목차가 다시 일치한다. 여기서도 파이프라인을 죽이지 않는다 — 감사 기록이 남지 않았다고 이미 만들어 둔 계획서까지 잃을 이유는 없다.
...
- `dotnet test`가 기존 648건 + 신규분 전부 통과 (착수 시점 실측값. AGENTS.md가 적고 있던 568은 갱신이 밀린 값이었다)
- 위 문서 3종 동기화 완료
