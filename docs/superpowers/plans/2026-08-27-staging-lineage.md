...
> **[2026-08-27 정정 - 최종 리뷰 I4]** 이 계획서는 사전 문서라 실측 전 추정을
> 그대로 남긴다. 실측 후 정정된 내용(재게시 관용구는 명세서 대상 제외가 아니라
> 자기참조 가드가 막는다는 것, 원천 최다가 `tsettlemst` 118건이 아니라
> 스키마 충돌 1건이라는 것)은
> `docs/superpowers/specs/2026-08-27-staging-lineage-design.md` §2-1과
> `MechanicalValidator.cs`의 `StagingSources`·`ReadsOnlyStaging` 문서를 보라.
> 아래 "기대: 59 → 44"를 포함해 이 문서의 사전 수치는 갱신하지 않는다.
