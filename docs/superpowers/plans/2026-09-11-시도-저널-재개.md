...
`RunConsolidatedPipelineAsync` 안, `PlanStructure.md` 를 쓴 **직후이자 `attemptJournal.OpenRun(currentPlanStructure, "run-start")` 앞**에:

```csharp
                            // 이어서 할 수 있는 판을 찾아 사람에게 묻는다(설계 §3).
                            // 자동으로 쓰지 않는다 - 조용한 재사용은 산출물이 왜 그
                            // 모양인지 설명할 수 없게 만든다.
                            var resumeCandidate = attemptJournal.TryResume(currentPlanStructure);
                            string? resumedFrom = null;
                            if (resumeCandidate != null &&
                                await _userInteraction.ConfirmResumeAsync(jobName, resumeCandidate))
                            {
                                resumedFrom = System.IO.Path.GetFileName(resumeCandidate.RunDirectory);
                                lastSkeleton = string.IsNullOrEmpty(resumeCandidate.Skeleton)
                                    ? null : resumeCandidate.Skeleton;
                                lastStepSections = new Dictionary<string, string>(resumeCandidate.ReusableSections);
                                pendingDefectiveSteps.Clear();
                                pendingDefectiveSteps.AddRange(resumeCandidate.DefectiveStepCodes);

                                // 피드백을 이어받지 않으면 첫 회차가 백지에서 시작해
                                // 이미 정리된 결함이 되살아난다(설계 §3-3).
                                foreach (var (priorAttempt, priorReview) in resumeCandidate.PriorReviews.Reverse())
                                {
                                ...
                                _userInteraction.NotifyStatus(
                                    $"[yellow]{jobName}[/] - run-{resumeCandidate.Run:D3} 에서 이어서 합니다 " +
                                    $"(재사용 {resumeCandidate.ReusableSections.Count}/{resumeCandidate.TotalStepsInManifest}).");
                            }
...
- [ ] **Step 3: `POQSettleBatch7` 로 실물 재개를 확인한다**

그 Job 의 판이 실물로 있다(`run-001`, 단계 17개 중 결함 넷: S03·S07·S08 `QualityFloor`, S10 `Unverifiable`).

같은 Job 이름으로 배치 계획 생성을 다시 돌려서 확인하라:

1. **재개 후보가 제안되는가** — 「13/17 재사용」이 화면에 뜨는가
2. 승인했을 때 **다시 만드는 단계가 넷인가**
3. 새 판(`run-002`)의 manifest 에 **`ResumedFrom: "run-001"`** 과 재사용 섹션의 **원래 `Attempt`** 가 남는가
4. 새 판의 파일에 **BOM 이 없는가**(`head -c3 <파일> | xxd -p` 가 `efbbbf` 가 아니어야 한다)
