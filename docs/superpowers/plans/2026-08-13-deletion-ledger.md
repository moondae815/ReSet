...
마지막 상태) 시점의 `AGENTS.md` L16–L145를 `./scripts/doc-audit.sh 16 145`로 찍은
스냅샷이다. 그 커밋 이후 이 구간은 라우팅 표로 교체되었으므로, 현재 트리에 대고
같은 명령을 다시 돌리면 대부분 빈 결과(또는 완전히 다른 결과)가 나온다 — 이는
회귀가 아니라 삭제가 실제로 반영됐다는 뜻이며, 이 표의 값어치는 삭제 판정이 무엇을
근거로 이뤄졌는지를 재현 가능하게 남기는 데 있지, 현재 상태를 서술하는 데 있지 않다.
