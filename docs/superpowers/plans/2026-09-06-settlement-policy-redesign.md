...
기대: 선행 적재 단계에 `dbo.UP_Util_PG_Client_CMRate_Ins` 1개, 호출 무리 단계에
`dbo.UP_Util_Settle_Summary`와 그것이 EXEC 하는 둘 = 3개, 나머지 **10개**가 순서 미상
단계에 모인다(14 − 1 − 3 = 10).

**「11」이 아니다.** 설계서 §3.1 의 「나머지 11개의 상호 순서는 모른다」는 **호출 그래프에
안 걸리는 SP 의 수**(14 − 3)이고, 여기 순서 미상 단계는 거기서 선행 적재 1개를 더 빼낸
수다. **분모가 다르다** — 이 두 수를 섞지 마라. (2026-09-06 T4 실측으로 정정.
초판 계획서가 11 이라고 적었고 그것이 틀렸다.) 확인 후 **이 임시 테스트는 지우고** 커밋한다(`Skip` 테스트를 남기면
게이트의 「건너뜀 0」이 깨진다).
