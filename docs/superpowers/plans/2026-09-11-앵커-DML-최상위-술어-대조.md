...
- Modify: `src/ReSet.Core/Services/SpecMaterials.cs` — 재료 셋(`IsL1Exhausted`·`DmlRows`·`ErrorCodeToOrdinal`)의 `ConsumingChecks` 에 `EvaluateAnchoredPredicateTerms` 를 더한다(Step 4b)
...
- [ ] **Step 4b: 재료 카탈로그의 소비 검사를 채운다**

`SpecMaterials.All` 의 `ConsumingChecks` 는 「이 재료가 0 이면 죽는 검사 전부」다. `SpecMaterialsTests.EveryNamedCheck_ExistsOnMechanicalValidator` 가 이름의 실재를 대조한다. 이 판정이 명세서 재료 셋을 새로 읽으므로 셋 다에 `"EvaluateAnchoredPredicateTerms"` 를 더한다.

- `IsL1Exhausted`: Task 4 수정 라운드가 `Array.Empty<string>()` 으로 두고 주석에 「소비 검사 — 아직 0」이라 적었다. 그 칸을 `new[] { "EvaluateAnchoredPredicateTerms" }` 로 바꾸고, 주석을 「소비 검사 — 앵커 DML 최상위 술어 대조(S5)」로 고친다.
- `DmlRows`: 기존 배열 끝에 더한다. E2 후보 행과 스테이징 대상(`BuildSpecTargets`)의 재료다. 비면 **두 면제가 반대로 움직인다** — 그 사실을 주석으로 적는다.
  - 커서 면제는 사라진다 → 더 엄격해진다.
  - 스테이징 면제는 넓어진다 → 더 조용해진다. `BuildSpecTargets` 가 빈 집합이 되면 `ReadsOnlyStaging` 의 `All(...)` 이 계보를 가진 모든 문장에서 참이 되기 때문이다(Task 5 리뷰가 잡았다 — 이 계획의 초판은 「더 엄격해진다」 하나로 적었다).
- `ErrorCodeToOrdinal`: 기존 배열 끝에 더한다. 코드 앵커를 서수로 푸는 재료(`MergeErrorCodeMaps`)라 비면 도달이 준다.

- [ ] **Step 5: 시험이 통과하는지 본다 — 전체 스위트**

워크트리에 코퍼스를 건다: `ln -s /Users/payletter/git-root/ReSet/output output`. gitignore 대상이다.

Run: `dotnet test tests/ReSet.Core.Tests`
Expected: 실패는 `AnchorKindOrdinalPairTests` 하나, 건너뜀 0. N5 네 시험도 통과해야 한다(도우미로 옮긴 키 규약이 같다는 증거). 이 태스크는 `ValidateBatchStep` 에 발화를 더하므로, **코퍼스 위에서 발화 수를 못박은 기존 시험**이 빨개질 수 있다. 그런 시험이 나오면 고치지 말고 시험 이름과 메시지를 CONCERNS 에 적는다 — 새 발화가 참인지는 Task 7 이 판정한다.

(1물결 교훈: 걸러 돌린 시험은 카탈로그 가드를 놓쳤다. 전체 스위트가 기본이다.)
...
되돌림 확인: <Step 6 표의 다섯 줄 결과>" -- \
  src/ReSet.Core/Services/MechanicalValidator.cs src/ReSet.Core/Services/SpecMaterials.cs \
...
- [ ] **Step 1: 코퍼스에 `-resume` 복사본이 없을 때만 돈다**

**심링크 조립본을 만들지 마라**(2026-09-11 reset-77 경고). Job 마다 심링크를 건 조립본은 안쪽 항목이 디렉터리 심링크라, 재귀 열거(`EnumerateFiles(..., AllDirectories)`)가 그 안으로 들어가지 않을 수 있다. 그러면 코퍼스 시험과 스윕이 파일 0 개를 읽고 **발화 0 으로 조용히 통과**한다. 루트 `output` 심링크(준비 단계의 것)는 경로 해석으로 들어가므로 이미 검증된 길이다.

```bash
cd /Users/payletter/git-root/ReSet/.worktrees/anchored-predicate
ls -l output                       # 루트 심링크 하나여야 한다
ls output/Jobs/                    # 시작 목록 — §7 에 그대로 적는다
```

- 목록에 `*-resume` 이 **없으면** Step 2 로 간다.
- **있으면** 멈춘다. 그 복사본은 다른 세션(reset-77)의 측정 창이다. 치웠다는 알림을 기다린다. 기다릴 수 없으면 사람에게 묻는다. Job 디렉터리 실복사(합쳐 약 20MB)가 대안이다.
- Step 6(게이트)이 끝나면 `ls output/Jobs/` 를 **다시** 찍어 §7 에 적는다. 두 목록이 같아야 측정 창 안에 복사본이 끼지 않았다는 증거가 된다.
...
## 2차 — 설계 §8 (2026-09-12 사람 결정)

Task 7 이 설계 §4 되돌림 조건 1(오탐 1 — Batch4/S11 UPDATE 1)을 걸었다. 사람이 설계 §8 로 정했다. 아래 두 태스크는 순차다.

### Task 8: R7 — 원본 리터럴 ↔ 이행 매개변수 (설계 §8-1)

**Files:**
- Modify: `src/ReSet.Core/Services/DmlScopeExtractor.PredicateTerms.cs` — `PredicateTerm` 에 init 속성 `LiteralAsParameter`, `PredicateTermsOf` 가 채운다, 도우미 `LiteralAsParameterKey`·`IsLiteralOperand`
- Modify: `src/ReSet.Core/Services/MechanicalValidator.cs` — `EvaluateAnchoredPredicateTerms` 의 `originalKeys` 에 한 줄
- Test: `tests/ReSet.Core.Tests/PredicateTermNormalizerTests.cs`, `tests/ReSet.Core.Tests/AnchoredPredicateTermCheckTests.cs`

**Interfaces:**
- Produces: `public string? PredicateTerm.LiteralAsParameter { get; init; }` — 원본 쪽 대조 키. 항이 `컬럼식 = 리터럴` 이면 `"<정규화한 컬럼식> = @V"`, 아니면 null.

- [ ] **Step 1: 실패하는 시험을 쓴다**

`PredicateTermNormalizerTests.cs` 끝(`R6_TermsFromSeveralWhereClausesAreUnioned` 뒤)에:
...
`AnchoredPredicateTermCheckTests.cs` 끝(클래스 닫는 `}` 앞)에:
...
Run: `dotnet test tests/ReSet.Core.Tests --filter "FullyQualifiedName~PredicateTermNormalizerTests|FullyQualifiedName~AnchoredPredicateTermCheckTests"`
Expected: 컴파일 실패 — `LiteralAsParameter` 가 없다.

- [ ] **Step 3: 정규화기에 R7 키를 더한다**

`DmlScopeExtractor.PredicateTerms.cs` 의 `PredicateTerm` 선언(`… bool IsJoinEquality);`)을 본문 있는 레코드로 바꾼다. 위의 문서 주석과 `<param>` 들은 그대로 둔다:
...
`PredicateTermsOf` 의 `terms.Add(new PredicateTerm(…));` 를 초기화자와 함께 쓴다:
...
`IsJoinEqualityTerm` 메서드의 닫는 줄 뒤(다음 멤버의 `/// <summary>` 앞)에:
...
`MechanicalValidator.EvaluateAnchoredPredicateTerms` 의 `var originalKeys = new HashSet<string>(…);` 바로 다음 줄에:
...
코퍼스를 워크트리에 건다: `ln -s /Users/payletter/git-root/ReSet/output output`. `dotnet build tests/ReSet.Core.Tests -warnaserror` → 경고 0. `dotnet test tests/ReSet.Core.Tests` → 실패는 `AnchorKindOrdinalPairTests` 하나, 건너뜀 0.

- [ ] **Step 6: 없앴을 때**

`originalKeys.UnionWith(…)` 줄을 지우면 `R7_StaysSilent_WhenTheStepBindsAParameterWhereTheOriginalHasALiteral` 이 빨개져야 한다. 확인하고 되돌린다(커밋에 남기지 않는다).

- [ ] **Step 7: 커밋**

```bash
git commit -m "feat: R7 - 원본 리터럴 등식은 이행의 매개변수 등식과도 맞는다(한 방향)" -- \
  src/ReSet.Core/Services/DmlScopeExtractor.PredicateTerms.cs src/ReSet.Core/Services/MechanicalValidator.cs \
  tests/ReSet.Core.Tests/PredicateTermNormalizerTests.cs tests/ReSet.Core.Tests/AnchoredPredicateTermCheckTests.cs
```

---

### Task 9: 재측정 — 설계서 §9 에 싣는다 (설계 §8-4·§8-5)

Task 7 과 같은 절차다(Step 1~7 — 코퍼스 조건, BASE·AFTER 스윕, 전 좌표 판정, 없앴을 때, 게이트, 문서). 다른 점만 적는다.

- **세 판을 맞댄다**: BASE `58da1f1a` · AFTER `6e5365c0`(보고서는 이미 커밋돼 있다 — `docs/audit-reports/sweeps/2026-09-11-step-sweep.md`) · AFTER2(Task 8 통합 끝). BASE 스윕은 다시 돌려 A–E·미분류가 §7 과 같은지 확인한다.
- **합격선은 설계 §8-4 다.** 특히 ④ — AFTER 대비 사라진 P 좌표가 정확히 Batch4/S11 UPDATE 1 하나, 새로 생긴 좌표 0, 「대조까지 간 문장 수」 383 불변.
- **없앴을 때에 하나를 더한다**: ④ R7 줄(`originalKeys.UnionWith(…)`)을 지우면 Batch4/S11 UPDATE 1 이 P 에 다시 나와야 한다. ① 배선 끄기도 다시 한다. ②·③(R2·R5)은 R7 이 건드리지 않으므로 반복하지 않는다 — 대신 COMM_UPD 다섯 곳이 조용한지(합격선 ⑥)만 본다.
- **되돌림 조건은 설계 §4 의 넷 + §8-5 의 셋이다.** 걸리면 §9 에 증거를 적고 멈춘다. 면제·코드·시험은 바꾸지 않는다.
- 산출물: 설계서 끝에 `## 9. 재측정 (<날짜>, BASE 58da1f1a · AFTER 6e5365c0 · AFTER2 <해시>)` 과 AFTER2 스윕 보고서.
