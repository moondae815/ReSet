...
- **⚠ 오탐을 재는 표본은 검사의 정의역과 같아야 한다** (2026-09-05 실측으로 배웠다):
  - ~~**A-3**은 계획서 한 편에 도는 배너 스캐너라 위 두 판이 그 정의역을 덮는다.~~ **← 이 문장은 틀렸다(2026-09-05 최종 리뷰).** `VerificationPipelineOrchestrator.cs:3300` 이 **모든 Job 의 계획서**에 `Scan` 을 돌린다. 그 오독 때문에 A-3 의 오탐을 한 편에서만 쟀고, 20 편에 돌려 보니 `POQSettleProc19/docs/BatchMigrationPlan.md:7248` 에 **새 오탐**이 있었다. **A-3 도 정의역 전체(20 편)에서 재라.**
...
2. Step 2 출력 표 전문
3. **채택한 후보와 규칙, 그리고 기각한 것마다 기각 사유** — 아래 **채택 축 셋** 중 어느 것에서 떨어졌는지로 갈라 적는다
...
| :-- | :--- | :--- |
| 1 | 현행판 오탐 == 0 | 즉시 기각 |
| 2 | 결함판 발화 > `base (현행) × any`의 값 | 즉시 기각 |
| 3 | 앞의 둘을 통과한 것이 여럿이면 **토큰0 잔량이 가장 작은 것** | — |

**⚠ 축이 하나 빠져 있었다 — 「사라진 발화」다(2026-09-05 최종 리뷰).**

| 축 | 조건 | 못 넘으면 |
| :-- | :--- | :--- |
| **−1** | **옛 규칙이 잡던 발화 중 새 규칙이 놓치는 것 == 0** | **즉시 기각** |

축 0~3 은 전부 **오탐만** 재고 **탐지 손실**을 안 잰다. 실제로 그 구멍으로 회귀가 통과했다 —
**분모: 옛 생산 규칙 `base×any×리터럴`(발화 6)** 대비 `별칭.컬럼×majority`(신) 18 건인데
**겹치는 것이 5** 라, 옛 규칙이 잡던 `POQSettleProc9/S06 갱신10`(감사 🔴 자리)이 새 규칙에서
**침묵**했다. **사라진 것은 1 이다**(6 − 5).

> **⚠ 「사라진 발화」에는 분모가 둘이고 섞으면 안 된다.** 위의 **1** 은 **옛 생산 규칙** 대비이고
> 이것이 축 −1 의 합격선이다. 이와 별개로 **라운드 1 중간 규칙**(별칭 민감, 배송된 적 없음)
> 대비로는 84 → 18 로 **66** 이 사라졌는데, 그것은 별칭 오탐 제거가 **의도한 것**이라 합격선이
> 아니다(다만 그 66 에 위 🔴 자리가 들어 있었으므로 「전부 의도한 제거」라는 추정은 최소 1 건에서
> 이미 틀렸다 — 판독 문서 §7 의 열린 항목). **표마다 「무엇 대비」를 칸 제목에 적어라.**

**규칙 변경은 양방향으로 재라** — 「새로 잡는 것」과 「놓치게 되는 것」을 같은 표에 함께 적어라.
이것이 이 저장소의 「없앴을 때로 판정한다」를 규칙 변경에 적용한 형태다.

**셋째 축을 적는 이유**: 앞의 둘만으로는 동점이 안 갈린다(실측에서 셋이 동시에 통과했다).
그리고 A-2가 존재하는 이유가 설계서 §2 ②의 「진짜 사각지대」인 토큰0 27건을 닫는 것이므로,
**토큰0을 27 그대로 두는 후보는 형식 게이트를 통과해도 이 태스크의 목적을 달성하지 못한다.**
셋째 축으로도 안 갈리면 **고르지 말고 조정자에게 올려라.**
...
>
> **⚠ 재생성이 무효화하는 시험 셋 — 착수 전에 이 목록을 확인하고, 재생성 뒤 반드시 재측정한다.**
> 계획서는 고정 오라클을 **둘**(결함 판·현행 판, 둘 다 Jobs 트리)로 못박았으나, 실제로는
> `output/Procedures/*/docs/Spec.md`(= 이 태스크가 다시 쓰는 파일)를 읽는 시험이 생겼다:
>
> | 시험 | 읽는 것 | 재생성 영향 |
> | :--- | :--- | :--- |
> | `SetExpressionCheck_FiresOnDefectiveBundle_AndIsSilentOnCurrentPlan` (`Assert.Equal(12, …)`) | 결함 판 번들 + **명세서** | **받는다** |
> | `SetExpressionCheck_FiresWhenHardFactTokensAreMissing_POQSettleProc9S06Ordinal10` | `output/Jobs/POQSettleProc9/…/S06.md` + **명세서** | **받는다** |
> | `OmissionScanner_FiresExactlyOnAuditedDefectCount_S07` (`Assert.Equal(10, …)`) | 얼어붙은 결함 판만 | 안 받는다 |
>
> 앞의 둘은 **붉어지거나(좋다) 조용히 다른 것을 재게 된다(나쁘다).** 재생성 뒤 값을 다시 재고,
> 달라졌으면 **왜 달라졌는지를 먼저 밝힌 뒤에** 상수를 고쳐라 — 값을 맞추려 고치지 마라.
