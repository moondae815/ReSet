...
**같은 모양이 이 저장소에서 두 번 나왔다.** 통제군 F 시리즈가 mermaid L1 실패로 판을
태운 원인도 모델이 아니라 정화기였다 — `CleanseMermaidCode` 의 화살표 보정이 flowchart
전용인데 모든 블록에 걸려 `DB-->>ORCH` 를 `DB--->ORCH` 로 부쉈고, 그 부순 줄을 근거로
수리 프롬프트가 모델에게 「모델이 쓰지 않은 줄」을 고치라고 했다(피어 `reset-b8`,
main `5a3d4946`). 둘 다 **검사가 틀린 것이 아니라 검사 앞단이 입력을 부수고, 그 부순
결과를 근거로 모델에게 실행 불가능한 지시를 보낸 것**이다. `docs/known-defects.md` 의
mermaid 항목 끝에 한 줄로 모여 있다 — 세 번째를 만나면 검사부터 의심하지 말고 **앞단이
입력에 무엇을 했는지 먼저 재라**.
...
## 2차 스윕 — 미도출 6건 (2026-09-04, main `5206ceed`)

수정 뒤 나머지 6건을 도출해 `output/Procedures` 14건을 전부 덮었다. 이번 스윕의 목적은
커버리지가 아니라 **수정의 사후 검증**이다.

| 대상 | 첫 판정 | 최종 |
| --- | --- | --- |
| `EXPECT_PROC` · `INS_EXTRA` · `Settle_Summary` · `SUMMARY_EXTRA` | **0** | 0 |
| `INS_EXTRA4PLCARD` | 2 | 0 |
| `SUMMARY_ETC` | 1 | 0 |

**파이프 오탐은 재발하지 않았다.** 수정 전 프로브에서 48건을 냈던 넷 중 셋
(`EXPECT_PROC` 12 · `Settle_Summary` 18 · `SUMMARY_EXTRA` 18)이 첫 판정부터 0이다.
생성이 다르므로 수를 직접 비교하지는 않는다 — 비교한 것은 **결함의 종류**이고,
`ConfidenceVocabulary`+`EvidenceMissing` 짝은 한 건도 나오지 않았다.

### 발화 3건 판정 — 진짜 결함이었다 (오탐 아님)

`INS_EXTRA4PLCARD` 를 프로브로 재현하니 종류가 달랐다 — `EvidenceQuoteNotFound` 셋이고,
전부 `## 예외 및 비기능 요구사항`이 `## 로직 흐름 요약`을 근거로 댄 행이었다.

인용문 셋은 `Spec.md` 377·379·380행에 **글자 그대로 실재한다.** 다만 그 자리는
`## 로직 흐름 요약`이 아니라 **`## 비즈니스 흐름 시각화`** 아래
`**다이어그램 범례 및 에러 코드 매핑:**` 이다. 즉 모델은 실재하는 문장을 인용하고
**출처 헤딩만 잘못 붙였고, 검사가 그것을 정확히 잡았다.**

요구 내용이 사라진 것도 아니다. 같은 실행의 저장본은 같은 사실을 허용된 절에서
정당하게 인용했다 — `## 파라미터 목록 > "실패 시 음수 코드(-9, -1, -2)를 설정합니다."`,
`## 로직 흐름 요약 > "행이 있으면 \`@po_intRetVal = -9\` 설정 후 \`RETURN\`"`.

**관측**: §5.1이 파생 원천에서 제외한 `## 비즈니스 흐름 시각화`는 다이어그램만
담고 있지 않다 — 그 아래 에러 코드 범례가 허용 절의 사실을 더 풍부하게 재서술하고
있고, 모델이 그 재서술에 끌린다. 제외 판단 자체는 「다이어그램은 요구가 아니라 표현」
이라 옳지만, **절 단위 제외가 절 내부의 비균질성을 못 본다.** 지금은 검사가 잡고
재호출이 고치므로 계약을 바꾸지 않는다 — 같은 형태가 반복되면 그때 §5.1을 다시 본다.

`SUMMARY_ETC` 의 1건은 **재현되지 않았다**(프로브 첫 판정 0). 초안이 버려져 종류를
알 수 없다 — 「반려된 초안이 안 남는다」 후속 후보의 비용이 이 스윕에서 다시 나왔다.

### 행 단위 생존 — 14편 506행 전량

| | 표 행 | 파서가 본 행 | 결함 | 발화 | 침묵 | R7 의존 |
| --- | --- | --- | --- | --- | --- | --- |
| 14편 합계 | 506 | 506 | 0 | **506** | **0** | 0 |

**감사 하네스 결함 하나를 고쳤다.** 앞 스윕에서 「인용 추출 실패」로 넘겼던 것이
이번 6편에서 `Settle_Summary` 31행 중 12행(39%)까지 커졌다. 그 비율에서는 「침묵 0」이
그 행들의 증거가 아니다. 원인은 되살린 행이 칸을 `" | "` 로 다시 이어 원문 줄과 간격이
달라 치환이 조용히 실패한 것이었고, 원문 줄에서 직접 인용 끝을 찾는 방식으로 고쳤다.
고친 뒤 추출 실패 0, 즉 506행 전량이 실제로 검사를 받았다. **하네스가 조용히 빼먹는
것을 비율이 커지고 나서야 봤다 — 앞 스윕의 「283행 전량」은 그만큼 과장이었다**
(그때 실패 2행은 검사받지 않았다).

### 게이트

스윕 전 3517 · 스윕 후 3517 (실패 0 · 건너뜀 0 · warning CS 0). `Prd.md` 추가는 수를
흔들지 않는다 — 피어가 미리 재 준 예측대로다. `output/` 에서 바뀐 것은 `Prd.md` 뿐이고
`output.bak-*` 는 접촉 0.
...
- **범위 축소가 결함 면역이 된 경우.** 설계 §5.1 은 「다이어그램은 요구가 아니라 표현」이라며
  `## 비즈니스 흐름 시각화`를 파생 원천에서 빼고 PRD 에 Mermaid 를 넣지 않기로 했고, 그
  귀결로 「Mermaid 린팅도 필요 없다」를 적었다. 위의 정화기 결함이 드러난 뒤 재 보니
  `Prd.md` 8편의 mermaid 블록은 **0개**라 이 기능은 그 결함의 사정거리 밖이었다
  (`output/Procedures` 의 `Spec.md` 14편도 전부 flowchart 계열이라 sequenceDiagram 피해 0이다).
  당시엔 순수한 범위 축소로 적혔지 면역을 노린 것이 아니었다 — 얻은 것을 기록하되
  **의도한 설계였던 것처럼 적지 않는다.**
- **후속 후보(범위 밖) — 반려된 초안이 아무 데도 안 남는다.** `PrdDerivationService` 는
  ...
  **검사가 잡은 것을 사람이 나중에 검증하려면 반려된 판이 어딘가 남아야 한다.** 여기서
  ...
  올라가 있다(피어 `reset-b8`가 올렸고 `ad90c004`로 main 에 병합됐다 — 그 항목은 그 뒤
  실측으로 한 번 더 정밀해졌다: **골격 L1 반려본만은 남는다.** 골격 수리가 패치라 프롬프트에
  직전 골격을 통째로 싣고 그것이 `raw/prompt-context.md`에 기록되기 때문인데, 설계 의도가
  아니라 부수 효과이므로 우연이다). 정책으로 분류한 이유는 **두 파이프라인이 같은 규약을 써야 하기
