...
| ⑥ 이사 바이트 보존 | 297,421 (+570 링크) — **⚠ 실측 정정: +639(213 링크). 근거: 커밋 `066bfe11`.** **⚠ 재앵커(2026-09-07): 오라클이 `ffbf61db` → `0b809a3f` 로 옮겨졌고 기대값은 309,523 이다. 의도된 델타 둘 — ① 절 파일의 `synced-through` 제거(최종 리뷰 I1) ② 쌍둥이 상호 참조 11줄 (+1,135 바이트). 검증 스크립트는 ①만 오라클에서 빼므로 ②만큼 차이가 남는 것이 정상이다.** | |
| ⑦ mermaid | 9블록 | |
| R2 역검색 | 분할 후 ≥ 분할 전 | |
