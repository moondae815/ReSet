...
### 재생성 회차 (2026-08-25) — 계획의 비목표였으나 이어서 돌렸다

계획은 재생성을 비목표로 두고 끝냈으나, 같은 세션에서 이어 돌려 설계서 「미확정 사항」3번을
닫았다. 방법은 README의 「소비 SP 12개를 4그룹 병렬 재생성」이고, 캐시를 올린 상황이라 그 절이
단 조건대로 **공용 UDF 4종을 `INS_EXTRA` 단독 실행으로 먼저 데운 뒤** G1·G2·G3′·G4를 병렬로
돌렸다(G3′ = G3에서 `INS_EXTRA` 제외). 오프라인 스냅샷으로 돌아 DB 연결은 없었다.

```
소비 SP            12/12 완료 · 실패 0 · L1Exhausted 0
캐시 세대           {16: 31}  (31객체 전부 v16)
커버리지 맵 🟥      205 → 0   (전이 창 닫힘)
새 두 L1 검사       0건       (대조 재료: 트랜잭션 105행 · SET 103행)
L1 반려             14라운드 · 오류 항목 56건 — 전부 기존 검사
```

`reset-l1-check` 스킬이 경고한 대로 로그의 `(시도 N/6)`을 전부 읽었다. **거짓 고발로 재시도를
소진시키는 실패 모양은 나타나지 않았다** — 자세한 분해와 근거는 설계서 「미확정 사항」3번에 있다.

골든 테스트는 원래 형태로 되돌렸다 — `TransitionWindowSpecMissing` 상수를 지우고 메서드 이름을
`Requirement1_CurrentEdition_SpecMissingShouldBeZero`로, 단언을 `총계 == 0`으로 복원했다.
임시 단언의 실패 메시지가 그 지시를 스스로 담고 있었고, 실제로 그 메시지가 떠서 따랐다.

재생성 전 판은 `output.bak-cache16-20260825`로 백업해 두고 돌렸다. **재생성 결과를 검증한 뒤
그 백업은 지웠다**(2026-08-25) — 🟥 0 · 캐시 세대 `{16: 31}` · `dotnet test` 실패 0으로 새 판이
건강함을 확인했고, 그 백업을 참조하는 테스트가 없었기 때문이다. 골든 테스트가 과거 판으로 쓰는
것은 `output.bak-2026-08-22`이고 그것은 그대로 있다.

### 남은 것

없다. 계획의 8태스크와 그 뒤 재생성까지 닫혔다.
