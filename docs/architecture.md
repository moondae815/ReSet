...
| | [NonAggregateAssignmentExtractor](../src/ReSet.Core/Services/NonAggregateAssignmentExtractor.cs) | `SELECT @v = 컬럼 FROM ...`처럼 집계가 없는 변수 대입을 뽑는다. 집계 대입과 정반대다 — 비집계 SELECT는 일치 행이 0건이면 대입 자체가 일어나지 않아 변수가 앞선 값을 그대로 유지한다. 남는 값이 무엇인지는 판정될 때만 말한다: 이 문장 앞에 대입이 하나도 없다고 판정되면 "NULL이 그대로 남는다"고 확정하고, 아니면 "이 문장에 도달한 시점의 값"까지만 말한다. 담는 것은 컬럼 참조, 모든 분기가 컬럼 참조인 `IIF`/`CASE`, 그리고 그 둘을 `ISNULL(X, 리터럴)`/`COALESCE(X, 리터럴)`로 한 겹 감싼 것이다(`AssignmentExpressionUnwrapper`). 한 분기라도 리터럴·산술식이거나, `ELSE` 없는 `CASE`이거나, 식·FROM 절이 집계를 품거나, FROM이 아예 없거나, `WITH`를 단 문장이면 결론이 뒤집히거나 성립하지 않으므로 침묵한다 |
