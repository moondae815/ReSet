...
| | [SpecStatementFactsExtractor](../src/ReSet.Core/Services/SpecStatementFactsExtractor.cs) | 원본 프로시저 명세서의 DML 범위 표·갱신 절·지역 변수 표에서 단계 검사가 대조할 사실을 뽑는다 — 문장 종류별 개수, 문장별 확정 컬럼·조인 키, 선언돼야 할 지역 변수와 타입. `ValidateBatchStep`의 `statementFactsByProcedure` 인자가 되는 재료다(§4.4) |
...
* **두 조건을 한 번에**: 같은 코퍼스를 `AsIs`(오늘 그대로 — 캐시 16, 산출물에 「오류 코드」 표 없음)와 `SimulatedCache17`(원본 DDL에서 만든 코드→서수 사전을 주입한 상태) 두 조건으로 돌립니다. 캐시 인상은 전건 재생성을 부르는 비가역 결정이므로, 인상 **후에** 발화가 어떻게 변하는지를 인상 **전에** 재기 위한 장치입니다(§4.8의 캐시 17 보류).
* **결손을 머리말에 강제로 싣는다**: 대상 범위가 줄면 발화량도 줄어드는데, 결손(프로시저 디렉터리 미스·미해결 참조·측정 0인 Job·상한 초과 Job·Job 단위 예외·펜스 파싱 실패로 제외된 단계)을 적지 않으면 그 감소가 개선처럼 읽힙니다. `StepSweepReportWriter`는 이것을 표 아래 각주가 아니라 머리말에 두고, 발화가 0인 검사도 행을 냅니다 — **빠진 검사와 발화가 0인 검사는 다른 사실**입니다. 보고서에는 측정 시점의 커밋 해시와 단계 번들·명세서의 캐시 세대를 함께 싣습니다. 세대가 다른 산출물을 견주면 총량 비교가 곧바로 무의미해지기 때문입니다.
