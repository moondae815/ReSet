...
검사 B·C는 현재 **사실상 비활성**이다. `ReadAnchor`는 문장 바로 앞의 공백·주석만 보는데,
코퍼스 실물은 앵커 주석과 DML 사이에 `[Precise Error Tracking]` 규칙이 요구하는
`SET @v_currentStepId = ...;` 문이 끼어 있어 앵커가 326개 단계 파일 전수에서 0개 잡힌다.
C는 B가 잡은 앵커 위에서만 동작하므로 함께 비활성이다. 근거와 정정 이력은 `docs/known-defects.md`의 2026-08-24 항목("축 B 단계 검사(검사 A~E)
코퍼스 스윕 실측"·"검사 B·C의 앵커 방식 자체가 이 코퍼스에서 성립하지 않는다")에 있다.
...
* **기계 확정 표가 늘면 🟧이 🟥으로 옮겨간다**: 재료를 세는 곳(`CoverageMapComposer.ExtractorFactLines`)이 새 추출기를 세기 시작하면 그 잎은 즉시 "재료 있음"이 되지만, 기존 산출물은 아직 옛 프롬프트로 만들어져 그 표가 없으므로 앵커가 없습니다. 그래서 **표를 더한 직후에는 🟧이 줄고 🟥이 그만큼 튑니다** — 회귀가 아니라 재생성 전까지의 예정된 중간 상태입니다. 2026-08-25 현재 🟥 205건이 그 전이 창이고(트랜잭션 경계 105 + 변수 대입 100), 캐시 16 재생성이 끝나면 내려가야 합니다. 골든 테스트가 그 값을 임시 상수로 못 박아 두어, 0에 도달하면 상수를 지우고 원래 계약(총계 0)으로 돌아갑니다.
* **렌더와 누락 보고**: `CoverageMapHtmlWriter`가 문장 수 기준과 라인 가중 기준을 병기하고, 잎을 누르면 근거 패널이 재료·앵커의 출처를 보여 줍니다. 외부 자산을 참조하지 않으므로 산출물 디렉터리째 옮겨도 그대로 열립니다. 재료를 읽지 못해 빠진 객체는 조용히 넘기지 않고 화면에 남깁니다 — 참조 폐포에서 몇 개가 빠지면 맵은 멀쩡해 보이는데 대조 범위가 줄어든 것을 아무도 모르고, 감사와 대상 정의를 맞춰 둔 이유가 무너집니다.
