...
| | [ParameterColumnBindingExtractor](../src/ReSet.Core/Services/ParameterColumnBindingExtractor.cs) | 변수(파라미터·지역 변수)가 어느 `테이블.컬럼`과 **실제로 결합**되는지 문장 단위 별칭 해석으로 뽑는다 — 술어(비교·`IN`·`BETWEEN`·`LIKE`)·산술식 안 동반, 대입·INSERT 자리 대응·`SELECT @v =`·커서 `FETCH INTO` 자리 대응, 같은 문장의 조인 등식(컬럼=컬럼)으로 전파. 같은 함수 호출의 인자로만 동반하는 것은 결합이 아니다. L1 `CheckParameterColumnClaims`가 파라미터 표의 연결 컬럼 주장을 **기각할 근거**로만 쓰므로(결합이 하나라도 있으면 통과) 넓게 잡는다 — 좁게 잡으면 정상 서술이 결함이 된다(코퍼스 스윕 거짓 양성 6건이 가르쳐 준 모양). 전역 별칭 맵은 마지막 등록이 이기므로 쓰지 않는다 |
