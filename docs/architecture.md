...
* **검증 파이프라인 밖의 산출물**: 이 문서는 L1 자동 정화만 거치고 L2는 타지 않습니다. `FormatUnverifiedDocument`로 렌더링되어 '검증 없음'으로 표기됩니다. `output/settlement-process.md`(사람 소유 입력)와 `output/Policy/SettlementPolicy.md`·`settlement-codebook.json`·`steps/*.md`(생성물)로 저장되며, 다른 경로의 입력으로 이어지지 않는 종착 산출물입니다.
