...
| | [MetadataExporter](../src/ReSet.Core/Services/MetadataExporter.cs) | JSON 덤프, Raw 프롬프트 마크다운, 개별 DDL 및 테이블 스키마(`TableSchemas/*.md`) 내보내기 및 외부 코딩 에이전트용 가이드라인 번들(`agent/MigrationInstructions.md`) 생성 (SP 및 Job 모두 동일한 하위 경로로 일관성 유지). |
