...
| | [CliProviderBatchGuard](../src/ReSet.Core/Services/Clients/Cli/CliProviderBatchGuard.cs) | Actor/Critic/Consolidator 중 CLI 제공자가 지정된 상태로 ReSet.Cli 배치 모드가 실행되는 것을 판정하는 가드. CLI는 대화형 TUI 전용이라 무인 실행 중 권한 프롬프트 정지나 쿼터 소진이 발생하면 장시간 작업이 통째로 소실되므로, DB 연결 전에 실행을 즉시 중단시킵니다. |
