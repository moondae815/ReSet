...
| | [DmlScopeExtractor](../src/ReSet.Core/Services/DmlScopeExtractor.cs) | 원본 DDL의 문장마다 "무엇이 대상 범위를 정하는가"를 AST로 확정한다. 최상위 WHERE의 술어 컬럼·기준일 파라미터 적용 여부·조인 키·GROUP BY 키를 담는 `DmlScopeFact`와, 행을 가르는 리터럴 집합을 담는 `SetPredicateFact`를 낸다. 두 재료의 문장 집합은 DML 셋에 FROM이 있는 독립 SELECT를 더해 넷이다(2026-08-23 ③(b)) — `SetPredicateFact`의 `SELECT n` 행에서 술어가 가르는 것은 쓰는 대상 행이 아니라 읽는 행이다. 후자는 `IN`/`NOT IN`에 더해 우변이 리터럴인 `=`·`<>`, `ISNULL(...)`처럼 감싼 좌변, 파생 테이블 내부 술어까지 담고 `Scope`로 최상위와 파생 테이블을 가른다 — 감사에서 나온 대상 행 집합 결함 4건이 전부 이 재료가 없어 L1이 대조할 것조차 없던 자리였다. 우변이 파라미터·컬럼인 비교는 버전 11까지는 담지 않았으나(옮겨 적을 리터럴이 없고, 담으면 표가 기준일 비교와 조인 키로 뒤덮인다는 판단이었다) 지금은 **리터럴 집합으로 분해하지 않을 뿐 행 자체는 낸다** — 컬럼·연산·원소 수·리터럴 칸이 전부 `—`이고 「술어 원문」 칸이 그 필터의 유일한 기록이다(아래 §4.9의 세 번째 회차). GROUP BY는 UPDATE·DELETE의 최상위 절로 문법상 불가능해 그 두 연산은 "—"로 표시하고, INSERT는 UNION 갈래마다 키가 다르면 과소 포착(빈 목록)을 택해 거짓 합집합을 피한다. 사용자 함수 호출의 위치·인자도 `ReferencedFunctionCallFact`에 수집해 기계 확정 참조 표의 재료로 제공한다 — 이쪽 문장 집합은 `IF` 술어까지 더해 다섯이다. `SELECT n`은 네 방문자가 `HasFromClause` 하나를 공유해 표를 가로질러 맞물리지만, `IF n`은 표마다 채번 조건이 달라 맞물리지 않는다(아래 §4.9) |
...
* **기계 확정 표 원문 복사 지시**: 산문으로 사실을 요구하면 모델이 요약하거나 대체물을 채워 넣으므로, AST가 확정한 사실은 표로 프롬프트에 실어 "그대로 옮기라"고 지시하고 L1이 행 단위로 대조합니다. 「실행 의미」(DB 배치·집계 대입 0건 동작·`@@ROWCOUNT` 경계·커서 수명·CAST 반올림/절사 경로·비집계 대입 0건 동작·루프 내 재설정)와 「CASE 분기」(조건·결과 원문 그대로) 두 표가 이 계약의 최신 사례입니다. 전체 목록과 각 표의 상세 계약은 §4.12를 따릅니다.
