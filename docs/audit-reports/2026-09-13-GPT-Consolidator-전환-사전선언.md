...
| 스윕 A~E·P | 0 | 0 (미분류 7) |
| 배송 배너(단계 파일 3행) | 7 (하한 미달 2 · 목차 결함 5) | **3** (목차 결함 3 — 하한 미달 0) |
...
> **[정정 2026-09-13]** 위 표의 배너 수를 처음에 「하한 미달」 문구로만 세어 2 대 0 으로 적었다. 단계 파일 3행의 배너는
> 목차 결함(T36 · N6 · 목차 TargetTables 비어 있음)도 싣는다. 전부 세면 Batch10 **7** · Batch11 **3** 이고, Batch11 의 `S13`
> 배너(「본문이 `BatchControlTotal`에 쓰는데 목차는 `ControlTotal`만 선언」)는 아래 결함과 뿌리가 같다 — 목차가 계약 별칭을 적었다.
