...
> 형태로 남아 있다. 닫으려면 `SpecReturnCodeExtractor`를 넓혀야 하는데 그것은
> `PlanStructureEnricher`와 `FindMissingErrorCodes`도 함께 먹으므로 **사람 판단이 필요하다.**
> 재 둔 것: 서술형 패턴을 `` `(-?\d+)`(을|를|이|가|로|으로)?\s*(반환|설정|기록) ``으로
> 조이면 명세 28편에서 더해지는 것이 `-15`·`0`·`-5`~`-8`뿐이고 **잡음이 0**이다.
> 넓힐 때 「재료 = `0`이 아닌 코드가 하나 이상」으로 함께 좁혀야 한다 — 안 그러면
> `AcqManual`이 `0` 하나만 든 키를 얻어 그 단계의 모든 음수 대입이 발화한다.
>
