...
`-9`가 그 증거다. 측정된 241개 단계에서 `-9`의 분포는 이렇다.

| `-9` 분포 (측정된 241개 단계 기준) | 단계 수 |
|---|---:|
| `ErrorCodes`에 **선언**한 단계 | **124** |
| 그중 실제로 **싣는** 단계 | **86** |
| **선언만 하고 싣지 않는** 단계 | **38** (124 − 86) |

싣는 86개는 체인의 지급확정 보호검사를 실제로 갖는 단계들이다
(`POQSettleBatch1/agent/steps/S04.md:28`, `S05.md:36`, `S10.md:45`,
`POQSettlePrco20/agent/steps/S03.md:49` 등). **나머지 38개는 그 검사를 갖지 않으면서
`ErrorCodes`에만 `-9`를 달고 있다.** 그 38개의 내역은 §3의 세 갈래로 정확히 갈린다 —
**37개가 「정당한 미대입」이고, 1개(`Proc19/S11`)만 「진짜 결손」이다.**
`-9` 한 코드가 62건의 정당 미대입 중 37건, 곧 **60%를 혼자 만든다.**
...
측정된 241개 단계 중 **124개가 선언하고 86개만 싣는다.**
남는 **38개**는 그 보호검사를 갖지 않으면서 `ErrorCodes`에만 `-9`를 달고 있고,
그중 37개는 정당한 미대입, 1개(`Proc19/S11`)만 진짜 결손이다(§3-2).
**"선언했으니 실어야 한다"는 전제가 이 필드 위에서는 성립하지 않는다.**
...
> ⚠️ **다만 「재료가 있다」와 「리더가 그것을 본다」는 다르다 — 짜기 전에 리더의 시야를 먼저 확인하라.**
>
> 위 짝짓기의 왼쪽(원본 `Spec.md`)은 그대로 읽으면 되지만, 오른쪽 **「단계 DML 순서」는
> 리더가 만들어 주는 것이고 그 리더에게는 사각지대가 둘 있다.** 아래 좌표는 이 회차에
> 직접 확인했다(HEAD `118b9a6` 기준).
>
> **① 읽는 펜스가 ` ```sql `뿐이다.** `src/ReSet.Core/Services/StepSqlStatementReader.cs:83`의
> `FencePattern`이 ``` ```sql(?<sql>.*?)``` ``` 하나이고, `MechanicalValidator.cs:802`도 같은
> 패턴을 쓴다. ` ```text `·` ```pseudocode ` 펜스의 DML은 **리더에게 존재하지 않는다.**
> 코퍼스 실측: 단계 문서의 펜스는 `sql` 891 · `csharp` 156 · `pseudocode` 29 · `text` 22이고,
> **비-`sql` 펜스 안에 DML 14문장이 단계 문서 5건**(`Batch1/S02`, `Proc10/S03`, `Proc19/S18`,
> `Proc6/S32`, `Proc8/S05`)에 들어 있다.
>
> **② `SELECT … INTO`는 「보이지만 쓰기로 세지 않는다」.** 여기서 전해 들은 것과 실물이
> 달랐으므로 본 대로 적는다. `SELECT … INTO`를 **아무도 방문하지 않는 것이 아니다** —
> `SqlStaticParser.cs:140`이 `Visit(SelectStatement)`으로 청크에 담고,
> `DmlScopeExtractor.cs:1609`가 `ExplicitVisit(SelectStatement)`으로 독립 SELECT를 기록한다.
> 진짜 빈자리는 **분류**다. `src/` 전체에 `IntoClause`·`SelectInto` 처리가 **0건**이고,
> DML 사실을 만드는 방문자는 `UpdateSpecification`·`DeleteSpecification`·`InsertSpecification`
> 셋뿐이다(`DmlScopeExtractor.cs:1575, 1578, 1699` 및 `2051, 2054, 2057`). 그래서
...
>
> **이 보고서의 수치도 그 사각지대 위에 있다 — 다만 결론은 아니다.**
> §1의 `1,325 / 1,476`은 **정규식으로 잰 값**이고, 위 리더로 다시 재면 다를 수 있다.
> 그러나 **판정을 가른 151개는 하나씩 사람이 열어 원문 좌표로 확인했고**(§4),
> `-9`의 `124 / 86 / 38`도 직접 세어 검산했다. **그러므로 `1,325`는 근사이지만
> 결론 `87 / 62 / 2`와 채점표 2건은 근사가 아니다.** 후속 회차가 분모를 리더로 다시
> 재더라도 **세 갈래의 판정과 결손 2건은 그대로 쓸 수 있다.**
>
> **그리고 이 경고를 쓰다가 나도 같은 함정에 걸릴 뻔했다.** `public override void Visit`으로
> grep해서 "`SelectStatement`를 방문하는 것이 없다"는 결론에 닿았는데,
> `DmlScopeExtractor.cs:1609`는 `ExplicitVisit`이라 그 grep에 안 걸린다. **패턴이 아니라
> 호출 규약을 확인해야 한다.**

**(4) 이 문서가 밟은 함정 넷을 반복하지 말 것.**
① 산문은 판정 근거가 될 수 없다(§3-4). ② 넓은 정규식은 좁은 정규식만큼 위험하다(§4 —
