...
| POQSettleBatch11 | 미분류 | 3 | 3 |
| POQSettleBatch12 | 미분류 | 2 | 2 |
...
| :--- | ---: |
| 다중 레거시 SP 단계 수 | 1 |
| SP 표에는 있는데 단계에 없는 코드가 있는 단계 수 | 190 |
| 단계에는 있는데 SP 표에 없는 코드가 있는 단계 수 | 0 |
| 펜스 파싱 실패로 코드 집합 대조에서 제외한 단계 수 | 2 |
| 코드 앵커가 둘 이상의 문장에 붙은 단계 수 | 0 |

펜스 파싱 실패로 2개 단계를 코드 집합 대조에서 제외했다 - 위 두 코드 집합 지표(SP 표에는 있는데 단계에 없는 코드, 단계에는 있는데 SP 표에 없는 코드)의 분모가 그만큼 줄었다는 뜻이다. 이 값이 크면 두 지표가 코퍼스 전체를 대표하지 않는다.
...
| 분모 | 값 |
| :--- | ---: |
| 앵커가 서수로 해결된 문장 수 | 1550 |
| 앵커는 있으나 서수로 환산되지 않은 문장 수 | 0 |
| (Kind, Ordinal) 모호성 가드가 버린 문장 수 | 0 |
| 원본에 있는데 이행 최상위에 없는 조인 짝 수 (발화 아님) | 328 |
| 계보 원천을 가진 문장 수 | 5 |
| 스테이징만 읽어 검사 C 가 면제한 문장 수 | 4 |
| 자기 대상을 읽는 문장 수 | 722 |
| 자기 대상을 읽어 스테이징 면제가 취소된 문장 수 | 1 |
| 하위 범위 술어 컬럼을 가진 문장 수 | 210 |
| 하위 범위 술어 컬럼의 총수 | 3493 |
| 스테이징 원천의 총수 | 5 |
| 최상위 술어 - 원본과 대조까지 간 앵커 DML 문장 수 | 1385 |
| 최상위 술어 - 발화한 문장 수 | 2 |
| 최상위 술어 - 원본 DDL 이 없어 침묵(S1) | 0 |
| 최상위 술어 - 원본에 키가 없거나 모호해 침묵(S2) | 8 |
| 최상위 술어 - 원본 문장에 최상위 항이 없어 침묵(S3) | 117 |
| 최상위 술어 - 커서 그룹 면제(E2) | 30 |
