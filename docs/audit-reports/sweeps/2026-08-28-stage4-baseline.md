...
> 명세서 17개). **한 세대가 아니다.**
>
> ⚠️ **「표 보유」로 세지 마라. 정본 자는 `output/.sp_cache_index.json`의 `FormatVersion`이다.**
>
> 이 절의 초안은 「오류 코드 표 보유」로 세고 동료 세션의 기록(프로시저 5/14)과 어긋난 것을
> "그 사이 한 편이 더 끝났다"로 추론했다. **그 추론은 근거가 없었다.** 두 자로 나란히 재면
> 이렇다(같은 회차 실측).
>
> | | 표 보유 | `FormatVersion` 17 |
> |---|---:|---:|
> | 프로시저 | 6 / 14 | **6 / 14** |
> | 함수 | 0 / 10 | **14 / 17** |
>
> **프로시저의 두 6은 수만 같고 집합이 다르다.** 교집합 5, 대칭차 둘 —
> `EXCEPTION_PROC`는 표를 썼으나 게이트를 못 넘어 캐시에 안 실렸고(표 O · ver16),
> `PROC_ETC`는 오류 코드 사실이 0건이라 **표가 없는 것이 정상**이다(표 X · ver17).
> 수가 맞는다고 같은 것을 센 것이 아니다.
>
> **함수 축에서 「표 보유」는 재생성의 신호가 아니라 상수 0이다.** 함수 17개는 전부
> `ErrorCodeFact`가 0건이고(`DmlScopeExtractor.ExtractErrorCodes`가 인정하는 패턴이 함수에
> 사실상 없다), `CheckErrorCodes`가 `ErrorCodes.Count == 0`에서 조기 반환한다. 그래서 함수는
> 캐시 17로 재생성돼도 표가 **없어야 정상**이다 — 0/10은 틀린 값이 아니라 **신호를 안 담는
> 값**이다. 이 구별은 reset-b1이 `ErrorCodeTableCorpusTests`의 전수 실측으로 갈랐다.
>
> **객체 종류와 무관하게 성립하는 자는 캐시 인덱스의 `FormatVersion` 하나뿐이다.**
>
> ⚠️ **그리고 코퍼스는 지금도 움직이고 있다.** 이 정정을 쓰는 시점(08-28 11:50)에
> `dotnet run --project src/ReSet.Cli`(인자 없는 대화형 TUI)가 돌며 명세서를 검증 중이고,
> `INS_EXTRA`의 캐시 항목이 **08-28 02:50에 ver17로 기록**됐다. 즉 「재생성이 멈춰 있다」는
> 전제로 이 수치를 인용하면 안 된다 — **인용할 때 잰 시각을 함께 적어라.**
> 다만 계획서 20편은 무사하다(`BatchMigrationPlan.md` 중 08-26 이후 변경 **0건**).
> 개별 SP 분석 경로는 `output/Jobs`를 열지 않는다.
>
