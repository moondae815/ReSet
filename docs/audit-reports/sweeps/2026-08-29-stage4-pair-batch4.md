...
> ✅ **닫았다 (2026-08-29).** `ExitCode != 0` 갈래만 갈라 `MermaidCliError`를 낸다.
> 나머지 셋(시간 초과 · 예외 · CLI 비활성)은 그대로 Fallback으로 강등한다 — 그쪽은
> 「도구가 답을 못 줬다」이고 여기만 「도구가 정상 실행되어 파스 오류를 보고했다」다.
>
> **대가를 실측했다.** `mmdc`를 코퍼스 23편의 mermaid 블록 **60개 전부**에 돌렸다:
> **58 통과 · 2 실패**. 실패 둘(`POQSettleProc10`·`POQSettleBatch4`)이 전부 같은
> 부류이므로, 새로 반려되는 것은 **23편 중 2편(~9%)**이고 시정도 한 부류다.
> `sequenceDiagram`은 60개 중 3개뿐인데 **그중 2개가 틀렸다** — 모델이 그 종류를
> 쓰면 거의 틀린다.
>
> 시정 문구는 이미 있는 버킷 3이 받는다. 그 체크리스트가 `->`·`- ->`만 들고 있어
> **측정된 부류(`--->`는 flowchart에는 유효하고 sequenceDiagram에는 없다)를 한 줄
> 더했다.**
...
- ~~**예외처리 7/10의 내용** — Critic이 무엇을 지적했는지는 이 조사가 세지 않았다.~~
  → ✅ **답했다**: `2026-08-29-critic-exception-axis.md`. 축 4 규칙 11개 중 넷만 울렸고
  그중 **E2(원본 오류 코드 보존)가 절반**이다. 3단계가 고친 항목(NOLOCK·SNAPSHOT·신규 SP)은
  **네 시도 전부에서 발화 0**이다. **필수 수정 넷이 전부 축 4이고 넷 다 채택본에 남았다.**
...
> ✅ **(1)(2) 둘 다 확인됐고, (1)의 근거가 한 겹 바뀌었다** (`2026-08-29-critic-exception-axis.md` §6).
> Critic은 ②가 겨누는 자리를 **이미 보고 있다** — 채점 기준 4의 E2가 같은 문장이고 아홉 번
> 울렸다. 그런데 **못 닫는다**: 시도 6이 그 수정을 하러 돌았는데 같은 결함이 반복됐고,
> 채택된 시도 5에는 Critic이 놓친 발명(`S04`의 `-2`)까지 무경고로 실렸다.
> **②의 정당화는 「Critic이 못 보니까」가 아니라 「Critic이 점수로만 말하니까」다.**
> 역방향 검사(대입 코드 ∉ 명세 코드)의 실측 오탐은 레거시 출신 168단계에서 **0**이다.
...
#
#    ⚠️ 정정(2026-08-29) — 이 자리에 처음 적은 `"Score[A-Za-z]+":[0-9]+`는 **0을 낸다.**
#    로그의 실제 표기가 `"ScoreException": 7`로 **콜론 뒤에 공백**이라 그 패턴은 60건
#    중 하나도 못 잡는다. 내가 이 보고서를 쓸 때 그 grep이 빈 출력을 냈는데 paste
#    포맷 문제로 오인하고 python 파서로 넘어갔다 - **레시피만 틀린 채 남았다.**
#    다음 사람이 0을 받고 「점수가 로그에 없다」로 읽을 자리였다.
grep -oE '"Score[A-Za-z]+":\s*[0-9]+' $C/logs-batch4/*.log

#    요약 WRN 줄(`L2 AI 리뷰 결함 발견`)은 본문이 잘리므로 지적 내용을 그것으로 읽지
#    말 것. 응답 JSON을 파싱해야 한다 - 파서는
#    `2026-08-29-critic-exception-axis.md` §11에 있다.
