...
**이 열은 세 라운드에 걸쳐 틀렸다 - 라운드 셋에서 셀 일곱 개다**(§4-2, §6-6과 같은 수를 다르게 센 것이다). 그래서 이번에는 점 수정을 버리고 14행을 **전수로 다시
...
이 행의 표기 갈래는 세 번 세어 세 번 다른 수가 나왔다(8 → 9 → 11). **세 수 중 어느 것도
아래 최종 목록과 맞지 않는다** — 목록은 12행이고, 그 안에서 쓰기 문장에 나오는 표기가 9,
DDL 전용이 2, 의사코드가 1이다. 개수는 **무엇을 세느냐**에 따라 달라지므로(쓰기 문장만인가,
DDL과 산문까지인가) 수 대신 목록을 남긴다. 아래는 전부 **같은 논리 표 하나**를 가리킨다.
...
### 8-2. 이 검사가 못하는 것 넷 · 잘못하는 것 넷 — 닫힌 목록이 아니다
...
5. **3부 표기(`DB.스키마.표`)를 인식하지 못한다 — 거짓 고발.** 이름 조각
   (`QualifiedTableNameFragment`)이 한정자를 <b>하나</b>만 받으므로
   `INSERT INTO SETTLE_POQ_DB.batch.BatchStepJournal (…)`은 결속으로 인식되지 않는다.
   저널 대상으로는 0건이지만 **배치 제어 계열 표에는 실재한다** — 직접 세어 보면
   3부 표기 언급이 58회(`SETTLE_POQ_DB.dbo.BatchStepCheckpoint` 46 ·
   `SETTLE_POQ_DB.POQSettleBatch.StepRun` 4 · `SETTLE_POQ_DB.POQBatch.POQSettleStepRun` 4 ·
   `SETTLE_POQ_DB.dbo.TBatchTaskRun` 4)이고 그중 **쓰기 문장이 25개**다
   (`UPDATE` 19 · `INSERT` 4 · `MERGE` 2). 계획서가 저널을 이 표기로 쓰기 시작하면
   그날 이행한 계획서가 반려된다.
6. **테이블 힌트를 끼우면 인식하지 못한다 — 거짓 고발.**
   `UPDATE batch.BatchStepJournal WITH (ROWLOCK) SET …`는 표 이름 바로 뒤에 `SET`을
   요구하는 헤더 정규식에 걸리지 않는다. 저널을 이렇게 쓰는 자리는 0건이지만
   코퍼스 전체에 잠금·격리 힌트가 **239회** 있고, 이 저장소에는 힌트 표를 따로 대조하는
   검사(`LockHintTableMissing`)가 있을 만큼 살아 있는 어법이다.
7. **한 MERGE의 둘째 `WHEN MATCHED` 가지를 보지 않는다 — 거짓 고발.**
   `WHEN MATCHED AND … THEN UPDATE SET …` 다음에 오는 둘째 `WHEN MATCHED` 가지에서만
   결속하면 놓친다(첫 매치 하나만 본다). 코퍼스 0건이다.
8. **절 경계를 원문 전체에서 찾는다 — 거짓 음성 방향.** `UPDATE`의 SET 절을 뜯을 때
   경계를 펜스가 아니라 문서 원문에서 찾으므로, 펜스가 `;`·`FROM`·`WHERE` 없이 끝나면
   절이 뒤따르는 산문으로 샐 수 있다. 형제 검사(`CheckUpdateSetTargets`)와 같은 관례라
   이 검사 고유의 결함은 아니고, 방향도 "인정을 넓히는" 쪽이라 거짓 고발은 아니다.

**이 목록은 닫혀 있지 않다.** 위 5~7은 §8-3이 "예방적"이라 부른 확대 둘과 같은 잠복
부류이며, 근거는 오히려 더 세다(3부 표기는 이미 제어 계열 표에 실재한다). 코드로 닫지
않은 이유는 이름 조각이 공용이라 형제 검사 전체의 판정이 함께 움직이기 때문이다 —
넓히려면 그 검사들의 발화 집합을 다시 재는 것이 조건이다.
