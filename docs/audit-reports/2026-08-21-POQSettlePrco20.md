...
**축 A 교차가 8/8 정합이 됐다.** 기계 확정 「참조 함수」 표를 도입한 뒤 두 번째 실행이고,
직전에 남아 있던 🟡 1건(링크 대소문자)과 ⚪ 1건이 모두 닫혔다.

숫자만 보면 🔴이 1로 같고 🟠이 하나 늘었지만, **같은 결함이 아니다**. 8/20의 🔴은
`EXCEPTION_PROC`의 자기참조 누락이었고 이번에 닫혔다. 이번 🔴·🟠은 `INS_EXTRA`에서
직전에 🟡이던 두 건을 이번 단위가 배포 구성 전제를 적고 등급을 올린 것이다(4-2절).
...
|---|---|---|---|---|---|---|
| 🔴 | `UP_UTIL_SETTLE_INS_EXTRA` | `object_definition.sql:30-41` | `Spec.md:44`, `:90`, `:104`, `:303`, `:312` | `IF EXISTS(SELECT PLTID FROM TSettleMst WITH(NOLOCK) WHERE … OutState IN (1,5) …) → RETURN -9`. `BEGIN TRAN`보다 앞에 있는 게이트다 | `:303`의 `NOLOCK` 일반 서술이 이 사전 검사를 열거하지 않고, 이 검사를 다루는 네 자리(`:44`·`:90`·`:104`·`:312`)는 `NOLOCK`을 전혀 언급하지 않는다 | 더티 리드로 거짓 양성이면 그 날짜 배치의 재계산이 통째로 스킵되고, 거짓 음성이면 보호돼야 할 행이 삭제·재계산된다. 둘 다 최종 정산 금액이 달라진다. **전제: 동시 실행·동시 갱신 상황** |
...
결함 행에 적는다"고 정한다. 이번 단위는 그 전제를 명시했다 — 동시 실행·동시 갱신 상황에서
`NOLOCK` 게이트가 오판하면 거짓 양성일 때 그 날짜 배치가 통째로 스킵되고, 거짓 음성일 때
보호돼야 할 행이 삭제·재계산된다.

**상위는 이 등급을 조정하지 않았다.** 두 판정이 갈린다는 사실을 여기 적는 것이 규칙이고,
어느 쪽이 옳은지는 배포 구성(이 배치가 단독 실행인지 동시 실행인지)이 정한다.
그것을 아는 사람이 닫아야 한다.
...
- **`INS_EXTRA`의 🔴·🟠 등급은 배포 구성에 달려 있다.** 단위가 전제를 명시했고 상위는
  조정하지 않았다. 이 배치가 단독 실행인지 동시 실행인지 아는 사람이 닫아야 한다.
