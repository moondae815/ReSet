...
| `실행 의미` | (재생성 후 채운다) | 종류(`DB 배치`·`집계 대입`·`@@ROWCOUNT`·`커서 수명`)별 확정 문장 — 아래 별도 절 |
...
**`실행 의미` 표.** 종류 칸(`DB 배치`·`집계 대입`·`@@ROWCOUNT`·`커서 수명` —
`ExecutionSemanticsFacts`의 `*Kind` 상수)이 첫 칸이자 행 식별 키의 일부다.
`MechanicalValidator.CheckExecutionSemantics`는 종류·라인·대상·확정 사실 **네 칸 모두**가
표 어딘가의 셀과 일치해야 통과시킨다. `DB 배치`만 라인이 `-`, 대상이 `(객체 전체)`인
고정값이다 — 줄에 매이지 않는 사실이라서다. **표가 있어도 네 종류 전부가 행을 낸 것은
