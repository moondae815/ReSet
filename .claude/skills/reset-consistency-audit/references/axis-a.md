...
방향을 판정한다 — CAST만 다루고 `CONVERT`는 보지 않는다. 컬럼·변수·파라미터·리터럴 중
잎 타입을 하나라도 모르면(화이트리스트 다섯 계열 — money/smallmoney, decimal/numeric,
int 계열, float/real — 밖의 타입, "(모호)" 컬럼, 함수 호출·서브쿼리 등 다루지 않는 식
모양, 파생 테이블·CTE·임시 테이블·테이블 변수 별칭과 동명인 한정자 등) 그 CAST는 표에서
조용히 생략된다(`Log.Debug`만 남고 예외·경고는 없다). **따라서 특정 CAST 식 하나의 행이
표에 없는 것은 결함이 아니다** — `CASE`가 없는 객체에 `CASE 분기` 표가 없는 것과 같은
종류의 "정상"이다. 감사자가 "이 CAST가 표에 없으니 결함"으로 오판하지 말고, 결함은
"표에 실린 행이 원문과 다르다"거나 "산문이 표의 사실을 뒤집었다"일 때만 성립한다는 것을
기억해라.
...
| **상수·계수·반올림 자릿수·부호** | **DDL 원문** — 단, `CAST(... AS INT)`의 반올림/절사 방향은 `### 실행 의미 (기계 확정 — 수정 금지)` 표(`식 타입 경로` 종류)가 있으면 그 표가 기준값이다(위 SP 단위 절 참고) — SP·함수 양쪽에 같은 표가 실린다. 그 밖의 상수·계수·자릿수·부호는 여전히 DDL 원문이 기준값이다 |
