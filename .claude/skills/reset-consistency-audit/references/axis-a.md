...
양쪽 프롬프트 경로에 배선돼(`AiService.cs:445,1448,2670,2833`) 함수 명세서에도 같은 표가
...
**`DML 범위` 표에 `GROUP BY` 열이 늘었다(이 배치의 다른 과제, Task 8).** 이 문서를 쓰는
시점에는 그 열이 아직 반영되지 않았다 — 열이 하나 늘었다는 사실만 여기 적고, 빈 칸
표기나 값 규약은 단정하지 않는다. 감사 시 산출물의 `DML 범위` 표 헤더 자체에서 실제 열
이름과 표기 규약을 직접 읽어라.
