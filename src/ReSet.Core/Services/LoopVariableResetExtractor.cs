...
namespace ReSet.Core.Services
{
    ...
    ///    변수를 읽으면 재설정이 아니라 누적이거나 계산이다. 실측 4건이 이 모양이고,
    ///    그중 **이 조건만으로 걸러지는 것은 2건**이다(`WORKDAY2:31` · `SUMMARY_ETC:135`
    ///    누적). 나머지 2건(`SUMMARY_ETC:77·129` 문자열 조립)은 조건 1에도 걸리므로 위
    ///    6건에 이미 세어져 있다 - 그래서 11 = 3 + 6 + 2다. 누적을 담으면 **종류 칸
    ///    "루프 내 재설정"이 첫 칸부터 거짓**이 된다. 직전 반복의 값이 남는 것이
    ///    누적에서는 버그가 아니라 요점이다.
    ///    `+=` 같은 복합 대입도 오른쪽이 리터럴이지만 직전 값을 읽으므로 제외한다.
    /// 3. **앞선 최상위 문장이 반복을 벗어나지 않는다.** `WHILE 1=1` + `IF ... BREAK`는
    ///    흔한 커서 관용구고, 그 뒤 문장에는 "반복마다"가 성립하지 않는다. 이 가드는
    ///    **코퍼스에서 실제로 발동한다** - `SUMMARY_ETC`의 74~79행 `IF @@ERROR <> 0 ...
    ///    RETURN`에서 훑기가 멈춰 135행 `SET`에 닿지 않고, `PROC_ETC`도 137~141행에서
    ///    같은 이유로 멈춘다. 다만 그렇게 가려진 `SET`은 조건 2(누적)에도 걸리므로
    ///    **이 가드 때문에 사라지는 행은 0건**이다(가드를 뺀 쌍둥이로 코퍼스를 훑어 3행
    ///    그대로임을 확인했다). 그래도 두는 이유는, 확인할 수 있는 조건을 확인하지 않은
    ///    채 강한 문장을 실으면 그것이 바로 거짓 행이 새는 길이기 때문이다. 앞 라운드에서
    ///    같은 부류의 문장이 8행 중 7행에서 거짓이었다.
    /// </summary>
    public static class LoopVariableResetExtractor
    {
        ...
        ///
        /// [가운데 절은 출처만 말한다] 수정 라운드 1 - "DECLARE 시점의 값도 직전 반복이
        /// 남긴 값도 아닙니다"라고 적었더니 값을 부정하는 말로도 읽혔다. 이 문장이 실리는
        /// 코퍼스 3행 전부에서 재설정값과 DECLARE 초기값이 똑같은 `0`이라
        /// (`PROC_ETC` 22·32·33행 DECLARE, 69·113·114행 재설정), 값으로 읽으면 3행 중
        /// 3행에서 거짓이었다. 그래서 "이 지점의 값을 **정하는 것이** 무엇인가"만 말한다 -
        /// 두 값이 같든 다르든 참이다. 값을 부정하지 않으므로 지역 변수 표의
        /// "초기값 0"과도 부딪히지 않는다.
        ///
        /// [마지막 절이 "이 지점의 값"에 머무는 것도 의도적이다] "이 재설정을 빠뜨리면
        ...
        private const string ResetSentence =
            ...
            + "반복마다 다시 실행됩니다. 대입값이 상수이므로 이 지점의 값을 정하는 것은 "
            + "DECLARE의 초기값이 아니라 매 반복 실행되는 이 문장입니다. "
        ...
        private sealed class LoopExitDetector : TSqlFragmentVisitor
        {
            ...
            // 심각도 20 이상이면 연결이 끊긴다. 낮은 심각도는 그냥 지나가므로 여기서
            // 멈추는 것이 과할 때가 있지만, 틀리는 방향이 "행을 덜 담는" 쪽이다.
            public override void Visit(RaiseErrorStatement node) => Found = true;
        }
    }
}
