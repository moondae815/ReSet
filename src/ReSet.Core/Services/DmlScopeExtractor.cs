...
namespace ReSet.Core.Services
{
    ...
    public static class DmlScopeExtractor
    {
        ...
        private sealed class LockHintVisitor : TSqlFragmentVisitor
        {
            ...
            /// 가리는 규칙은 하나다: <b>이 방문자가 문장 노드에서 출발하는 네 자리</b>
            /// (DML 셋 · `HasFromClause`가 참인 독립 SELECT)와 `IF` 술어에서 출발하는
            /// 훑기만 존재한다. "문장 번호를 집는 자리"로 세지 않는 이유는 `IF`도
            ...
            /// (라) MERGE 문 전체. `MergeStatement`는 네 방문자 어디에도 출발점이 없어 대상·
            ///      USING 원천·ON 술어·WHEN 절이 통째로 빠진다 - 이 표만이 아니라 네 표가
            ///      같다. 거짓 행 대신 <see cref="ExtractUncoveredStatements"/>가 그 문장을
            ///      세어 프롬프트에 "표가 담지 않는다" 공지를 내고, 코퍼스 트립와이어가
            ///      MERGE 유입 시 깨진다(2026-08-23, 코퍼스 0건).
            ///
