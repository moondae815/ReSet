...
4. 비즈니스 흐름을 직관적으로 이해할 수 있는 Mermaid Flowchart 다이어그램을 필수로 포함해 마크다운으로 구성해 주십시오. 
   ...
   - 연결선(화살표) 위에 조건 텍스트를 적을 때(예: -->|텍스트|), 텍스트 부분에 절대 큰따옴표 기호(쌍따옴표)나 괄호, 특수기호를 사용하지 마십시오. (예: 화살표 중간에 '존재' 또는 '성공'을 표시하려면, 기호 없이 반드시 -->|존재| 또는 -->|성공| 과 같이 순수 텍스트만 적어야 하며, -->|""성공""| 이나 -- ""성공"" --> 와 같이 따옴표를 쓰면 절대 안 됩니다.)
   - 노드 내부 텍스트에는 골뱅이(@) 변수명 기호를 절대 포함하지 마십시오. (예: @po_intRetVal 대신 ""출력 실패 코드 설정"" 또는 ""실패 코드 반환""과 같이 자연어로 순화하여 기술) 단, '@@ERROR' 등 SQL 내장 시스템 에러 코드에 한해서는 가독성을 위해 예외적으로 기입을 허용하되, 특수문자로 인해 린팅이 깨지지 않도록 노드 정의 전체를 반드시 이중 큰따옴표로 감싸십시오.
...
13. SP 소스코드 내부의 자연어 개발 주석과 실제 쿼리 실행 연산식 사이에 모순(불일치)이 감지되면, 실제 쿼리 코드를 최우선 기준으로 판정해 명세서를 작성하고, `## 개요` 섹션 하단에 `[🚨 주석 불일치 경고] {{모순내용}}` 형식으로 구체적인 경고 문구를 포함시키십시오. (예: 개발 주석에는 '정상거래만 조회'로 적혀있으나 실제 쿼리 WHERE 조건절에서는 취소거래 조건(OutState = 9)도 배제하지 않고 함께 조회하는 모순이 있는 경우, 이를 지적하고 실제 쿼리 기준으로 로직을 분석 및 기록해야 합니다.) 특히 DDL의 메인 INSERT/UPDATE/SELECT 바로 직전에 쓰여 있는 설명 주석의 기준일자나 필터 범위 조건이 실제 WHERE 절(예: A.YMD = @pi_strYMD 가 아닌 A.REQYMD = @pi_strYMD 로 조회하거나 취소상태도 포함 등)과 어떻게 다른지 엄밀히 대조하여 누락 없이 경고를 작성하십시오.
...
5. 다이어그램 및 시각화 가독성 (ScoreReadability):
   ...
   - 노드 한글 텍스트에 이중 큰따옴표(""text"")를 적절히 사용하였는지, 노드 ID가 영문자/숫자 조합의 고유 식별자이며 예약어 충돌이 없는지, 화살표 위에 특수기호/괄호/큰따옴표를 배제했는지, 그리고 노드 내부에 @ 나 $ 같은 변수 기호를 배제했는지 (단, @@ERROR 는 예외 허용) 등의 문법 및 린팅 요건을 충족하는가?
