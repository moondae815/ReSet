...
3. Integration and Interface Definition (ScoreInterface):
   ...
   - Verify that a local variable the source procedure DECLAREs with an initial value still carries that `DECLARE` in the SQL the step sends, spelled with the type the source declared. A step that passes that value as a binding parameter instead has dropped the type contract - the binding list carries the original procedure's parameters only, and a bound value has no declared type: the driver picks one, and a `DECIMAL(2,1)` arriving as `FLOAT` changes what `CAST(... AS INT)` truncates. A type stated only in a comment is not pinned.
