...
4. 비즈니스 흐름을 직관적으로 이해할 수 있는 Mermaid Flowchart 다이어그램을 필수로 포함해 마크다운으로 구성해 주십시오. 
   ...
   - 노드 내부 텍스트에는 골뱅이(@)나 달러($) 같은 특수 변수명을 직접 사용할 때 이스케이프 처리가 복잡해지므로, 다이어그램 내부에서는 변수 기호를 빼고 일반 명칭으로 적으십시오. (예: 변수명 po_intRetVal 대신 '출력값 리턴' 또는 'po_intRetVal'로 기술) 단, '@@ERROR' 등 SQL 내장 시스템 에러 코드에 한해서는 가독성을 위해 예외적으로 기입을 허용하되, 특수문자로 인해 린팅이 깨지지 않도록 노드 정의 전체를 반드시 이중 큰따옴표로 감싸십시오.
