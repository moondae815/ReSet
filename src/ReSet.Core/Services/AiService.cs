...
2. The document must use only 4 mandatory H2 headers:
   ...
   - ## 통합 데이터 정합성 검증 SQL 세트: Include validation SQL templates checking data integrity.
     * EVERY block in this section MUST declare its own role in the FIRST comment line of its ```sql block, as `-- <block name> · 역할: 게이트` or `-- <block name> · 역할: 기록`. 게이트 means the step runs it INSIDE its transaction before committing and rolls that transaction back when `ViolationCount` is non-zero. 기록 means the step runs it AFTER the work is durable, writes one row to `batch.BatchValidationIssue`, and the batch continues. Write the role in the SQL comment, not in the prose above the block - a step that copies the block into its own section copies the comment with it, and the prose stays behind. Choose 게이트 ONLY when the predicate is exactly as narrow as the original procedure's own guard; a predicate that is even slightly wider stops production runs on data the legacy procedure commits, so it belongs in 기록. The first line of the block's SQL looks exactly like this: `-- SQL-07 지급·회수 상태 및 예정일 검증 · 역할: 기록`
...
5-1. [Local Variable Type Contract] A local variable the source procedure DECLAREs *with an initial value* keeps its DECLARE in the SQL this step sends. Write it inside the ```sql block as `DECLARE @<name> <declared type> = <initial value>;`, spelled exactly as the machine-derived local variable table gives it. Do NOT demote it to a binding parameter of `execute(...)` - the binding list carries the step's own parameters and the values the orchestration itself supplies (run id, step code, chunk bounds, and their kin), and a constant the source declared with its initial value is not one of them. Binding a run id or a chunk bound IS correct and the examples below do it; this rule is about the source's own declared constants, nothing else. A bound value has no declared type: the driver picks one, and a `DECIMAL(2,1)` that arrives as `FLOAT` changes what `CAST(... AS INT)` truncates. Stating the type in a comment does not pin it.
...
```pseudocode
// 원본이 초기값과 함께 선언한 상수는 바인딩 목록에 넣지 않는다 (rule 5-1).
...
```
The binding list above carries one name because this step needs nothing else - the shadow
example higher up binds `p_runId`, `p_from` and `p_to`, which are not the original procedure's
parameters at all, and that is correct. What never belongs in a binding list is the rate:
passing it there would leave its type to the driver, and a `DECIMAL(5,2)` that arrives as
`FLOAT` truncates differently. Writing the type in a comment does not pin it.
...
" + ConsolidatedPlanRules;
        ...
        private static string RenderVerificationSetBlock(string? verificationSet)
        {
            ...
            block.AppendLine("- Each block above declares its role in its first SQL comment - `역할: 게이트` or `역할: 기록`.");
            block.AppendLine("  HONOUR IT. A 기록 block runs AFTER this step's work is durable and writes one row to");
            block.AppendLine("  `batch.BatchValidationIssue`; the batch continues. NEVER promote it into a commit-time gate");
            block.AppendLine("  that rolls this step back when `ViolationCount` is non-zero - its predicate was not written to");
            block.AppendLine("  be that narrow, so a normal production day would stop the whole batch instead of recording");
            block.AppendLine("  one issue row. If you believe this step truly cannot commit while that block reports a");
            block.AppendLine("  violation, say so in prose and name the block; do NOT silently change how it is used.");
...
3. Integration and Interface Definition (ScoreInterface):
   ...
   - Verify that a local variable the source procedure DECLAREs with an initial value still carries that `DECLARE` in the SQL the step sends, spelled with the type the source declared. A step that passes that value as a binding parameter instead has dropped the type contract - the binding list is for the step's own parameters and the values the orchestration itself supplies (run id, step code, chunk bounds), not for a constant the source declared with its initial value, and a bound value has no declared type: the driver picks one, and a `DECIMAL(2,1)` arriving as `FLOAT` changes what `CAST(... AS INT)` truncates. A type stated only in a comment is not pinned. Do NOT penalize a binding list for carrying a run id, a step code, or chunk bounds - those are the orchestration's own values, they are correct there, and demanding their removal contradicts the rules that put them there.
...
4. Exception Handling, Transaction and Isolation Policy (ScoreException):
   ...
   - Check that no statement the step sends branches on its own outcome. Penalize `GOTO` error labels, `IF @@ERROR <> 0` checks, `IF @@ROWCOUNT` branches (an upsert that tests its own UPDATE's row count belongs in the application), and `BEGIN TRY`/`END CATCH` wrappers written into the step's own SQL - control flow and error handling belong to the application code. A quotation of the original legacy procedure is exempt.
