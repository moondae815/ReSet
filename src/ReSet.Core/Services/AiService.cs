...
{checklistSb}";
        ...
        /// - 규칙 6-1은 실패 지점 충실도만 남겼다. `TRY...CATCH`·`GOTO` 조항은 C#
        ///   예외 처리로 자동 대체되므로 뺐고, 원본 코드 재사용은 규칙 9와
        ...
        /// - 규칙 11(INSERT 전용 롤백)의 `ROLLBACK TRAN` 철자는 §2의 표에 없어
        ///   그대로 남겼다. 같은 종류의 잔존이며 다음 회차의 몫이다.
        /// </summary>
        /// <summary>
...
4. Exception Handling, Transaction and Isolation Policy (ScoreException):
   - Check that every step runs under SNAPSHOT isolation (Penalize heavily if `ALTER DATABASE SET READ_COMMITTED_SNAPSHOT ON` is proposed). Do NOT penalize a step for not spelling out where the isolation level is set - that is the implementing round's choice.
