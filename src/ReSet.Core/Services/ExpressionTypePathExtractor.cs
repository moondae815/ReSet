...
namespace ReSet.Core.Services
{
    ...
    public static class ExpressionTypePathExtractor
    {
        ...
        private sealed class CastVisitor : TSqlFragmentVisitor
        {
            ...
            ///
            /// [한정자가 없는 컬럼은 이 게이트를 타지 않는다 - 그리고 그 이유는
            /// "(모호)"가 감당해서가 아니다] `BuildColumnTypeMap`(ExecutionSemanticsFacts.cs:
            /// 41-64)의 "(모호)" 표시는 **의존성 집합 안에서** 같은 컬럼명이 테이블마다
            /// 타입이 다를 때만 붙는다. 파생 테이블·CTE·임시 테이블·테이블 변수는
            /// 의존성이 아니라서 애초에 그 맵에 어떤 항목도 넣지 않는다 - 그러므로 그런
            /// 비의존성 원천에서 온, 한정자 없는 동명 컬럼의 모호성은 "(모호)"가 감당하지
            /// 못한다(감당한다고 적혀 있었다면 그것은 거짓 근거다). 그런데도 이 게이트를
            /// 걸지 않은 것은 보증이 아니라 **관측**이다 - 코퍼스 실측(수정 라운드 2)에서
            /// `CAST` 인자 안 한정자 없는 컬럼 35건(5개 객체)을 표본으로 확인했고 전부
            /// 실제 테이블 컬럼이었으며, 파생 테이블/CTE 별칭이 `CAST` 인자로 직접 쓰인
            /// 사례는 관측되지 않았다. 반면 프래그먼트 단위로 "unsafe 참조가 하나라도
            /// 있으면 한정 없는 컬럼도 침묵"시키는 봉합을 걸면 `UP_UTIL_SETTLE_COMM_UPD`
            /// 하나에서만 파생 테이블 하나 때문에 정상 22건이 통째로 사라진다(정확성
            /// 이득 0, 손실만 있음 - 그래서 채택하지 않았다). **이 전제는 코퍼스가 바뀌면
            /// 깨질 수 있다** - 다음 사람은 이것을 "이미 안전하다"가 아니라 "여기에
            /// 코퍼스 관측에 기댄 조건부 전제가 있다"로 읽어야 한다.
            /// </summary>
