...
namespace ReSet.Core.Services
{
    public class CacheManager : ICacheManager
    {
        ...
        //    CASE 분기 표가 프롬프트 네 갈래 전부에 새로 실렸다. 요약이 곧 결함이었다
        //    (2026-08-22 축 A 감사, UIF_SettleYMD 🟠 3건). DML 범위 표에는 GROUP BY
        //    칸이 붙었다. 스키마 표 과소 포함도 고쳐져 주석에만 등장하는 컬럼과 별칭
        //    한정 표기(예: X.PRODUCTNAME)가 다시 실린다. 과소 포함이 "그 컬럼은 없다"는
        //    잘못된 서술을 14개 명세서에 남긴 결함이었다(UP_UTIL_SETTLE_PROC_ETC 실측).
