...
namespace ReSet.Core.Services
{
    public class CacheManager : ICacheManager
    {
        ...
        //     (2) 잠금 힌트 표와 DML 범위 표의 문장 칸에 `SELECT n`·`IF n`이 실린다.
        //     잠금 힌트는 DML 밖 독립 SELECT와 `IF` 술어 안의 스캔까지, DML 범위 표는
        //     독립 SELECT(커서 원천 질의 포함)까지 문장 집합이 넓어졌다. 두 표의 범위
        //     칸에는 `하위 질의` 값이 더해졌다(술어 안에서 다시 열린 질의가 훑는 자리).
