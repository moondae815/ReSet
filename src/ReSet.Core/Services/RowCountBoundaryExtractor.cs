...
namespace ReSet.Core.Services
{
    ...
    public static class RowCountBoundaryExtractor
    {
        ...
        /// 거짓 문장은 아니지만 완전하지도 않다. (1) 첫 절("분기가 건너뛰어지면")은
        /// 직전 IF에 ELSE가 있으면 도달 불가능하다 - IF/ELSE는 항상 THEN·ELSE 중
        /// 하나를 실행하므로 "분기가 건너뛰어짐"이라는 전제 자체가 ELSE 없는 IF에서만
        /// 성립한다. (2) "분기는 실행됐으나 그 안 마지막 문장이 행에 영향을 주지 않는"
        /// 경우(예: 마지막 문장이 DML이 아니거나 0행에 영향)는 두 절 어디에도 명시적으로
        /// 없다. 그래도 고치지 않는다 - 이 추출기의 다른 모든 사실은 로컬 Docker SQL
        /// Server 2022 16.0.4255.1 실행으로 확정했는데(위 클래스 주석), 이 문장에 뉘앙스를
        /// 더하려면 같은 기준의 실행 확인이 새로 필요하고 이 작업 범위에서는 그 확인을
        /// 하지 않았다. 이 배치에서 Critical 셋이 모두 "주석/브리프가 말하는 것과 실제가
        /// 어긋남"이었다 - 검증 없이 문장을 고쳐 같은 범주의 결함을 새로 만들 위험이,
        /// Minor로 분류된 이 정밀도 격차를 그대로 두는 비용보다 크다고 판단했다. 정적
        /// 분석은 애초에 "ELSE가 실행됐는지"와 "분기가 아예 안 돎"을 구분할 수 없으므로
        /// (둘 다 AST만으로는 불투명) 이 문장이 틀린 값을 단정하지는 않는다.
        /// </summary>
