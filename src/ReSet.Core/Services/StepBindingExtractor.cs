...
namespace ReSet.Core.Services
{
    ...
    /// 단계 본문 386 편에서 이 일곱 이름의 호출은 **506** 이고 그중 바인딩 객체를 진 것이
    ...
    public static class StepBindingExtractor
    {
        ...
        ///
        /// **일곱이 전량이라는 보증은 지금도 없다.** 위 실측은 「오늘 코퍼스에 이 일곱만
        /// 있다」는 관측이지 「이 일곱뿐이어야 한다」는 도출이 아니다. 여덟 번째 이름이
        ...
        ///
        /// **[왜 줄 앵커(`^\s*``` `)로 안 고쳤나 - 이월 판정, 2026-09-07]** 이 정규식은
        /// 줄 앵커가 없어 짝짓기가 어긋나면 뒤가 조용히 사라지고, 위험 방향이 오탐이 아니라
        /// **침묵**이다. 그런데도 지금 안 고친 것은 비순환 대조가 오늘 어긋남을 못 찾았기
        /// 때문이다: 386 편에서 **정규식 펜스 매칭 1386 == 줄 상태 기계 펜스 쌍 1386,
        /// 어긋난 파일 0**. Task 4·5 의 발화 기대값 3 이 걸린 경로를 물결 중에 다시 쓰는
        /// 대가가 더 크다고 판정했다. **다음에 이것을 닫을 때는 재작성이 아니라 위
        /// 항등식(정규식 쌍 수 == 줄 상태 기계 쌍 수)을 검사로 심어라** - 재작성 없이
        /// 침묵을 발화로 바꿀 수 있다.
        /// </summary>
        ...
        /// 단계 본문 386 편, 일곱 이름 기준): 중괄호가 **여러 줄에 걸친 호출이 21 자리**이고
