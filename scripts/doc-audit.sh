...
#     있는데도 근거없음으로 나올 수 있다.
#   - 2026-09-07 분할 이후 architecture.md 지표(arch)에는 체계적 편향이 하나 있다.
#     절 파일(docs/architecture/*.md)에 사는 링크는 허브보다 한 겹 깊어 상대경로가
#     "../src" 에서 "../../src" 로 늘고, wc -c 는 매치된 줄 전체를 재므로(부분
#     문자열이 아니다) 그 링크를 담은 매치 줄마다 3바이트가 arch 에 더해진다. 이
#     편향은 항상 arch 를 키우는 방향이라 판정을 "중복"쪽으로 기울인다 — 위에서
#     말한 위험한 방향(지우면 안 되는 서술을 지우게 만드는 쪽)과 같은 방향이다.
#     실측(2026-09-07)으로 오늘 뒤집힌 항목은 없었지만 CSharpReflectionRunner는
#     문턱까지 여유가 2바이트뿐이라 편향 한 단위(+3)보다 작다 — 문턱 근처 항목을
#     볼 때는 이 편향을 먼저 빼고 위 "근소한 차이" 규칙을 적용해야 한다.
...
#     최종 판단은 사람이 한다.
#   - architecture.md가 어떤 클래스를 백틱 이름으로만 언급하며 본문 문단에서 길게
#     서술하면서도 그 문단이 파일을 링크하지 않는 경우가 있다. 경로 대조는 그
#     경로가 실제로 등장하는 줄만 세므로, 이런 문단은 존재해도 카운트에 들어오지
#     않아 근거없음이 나온다 — 이 실패 방향은 위 "묶음 레이블" 항목(하나의 링크가
#     여러 클래스를 덮는 경우)과 다르다. 이것은 그 클래스를 가리키는 링크 자체가
#     그 문단에 아예 없는 경우다.
set -eu
